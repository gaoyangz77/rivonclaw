// @ts-nocheck -- esbuild emits JavaScript syntax into this generated TypeScript module.
...
// vendor/openclaw/packages/markdown-core/src/reasoning-tag-parser.ts
...
var REASONING_TAG_NAME_SET = new Set(REASONING_TAG_NAMES);
var DISABLE_HTML_MARKDOWN = {
  disable: { null: ["htmlFlow", "htmlText"] },
};
function scanReasoningTags(text3, final = true) {
  const tags = [];
  let cursor = 0;
  while (cursor < text3.length) {
    ...
    const parsed = parseReasoningTagAt(text3, start, final);
    if (parsed.kind === "tag") {
...
function parseReasoningTagAt(text3, start, final) {
  let cursor = skipTagWhitespace(text3, start + 1);
  let isClose = false;
  if (text3.charAt(cursor) === "/") {
    isClose = true;
    cursor = skipTagWhitespace(text3, cursor + 1);
  }
  const nameStart = cursor;
  while (cursor < text3.length && isTagNameCharacter(text3.charCodeAt(cursor))) {
    cursor += 1;
  }
  const partialName = text3.slice(nameStart, cursor).toLowerCase();
  if (!partialName) {
    return cursor === text3.length && !final ? { kind: "pending" } : invalidTag(text3, start);
  }
  if (!REASONING_TAG_NAME_SET.has(partialName)) {
    const canBecomeKnown = REASONING_TAG_NAMES.some((name) => name.startsWith(partialName));
    return cursor === text3.length && !final && canBecomeKnown
      ? { kind: "pending" }
      : invalidTag(text3, start);
  }
  if (cursor === text3.length) {
    return final ? invalidTag(text3, start) : { kind: "pending" };
  }
  const boundary = text3.charAt(cursor);
  if (!isTagWhitespace(boundary) && boundary !== "/" && boundary !== ">") {
    return invalidTag(text3, start);
  }
  let quote;
  let lastSignificant = "";
  for (; cursor < text3.length; cursor += 1) {
    const char = text3.charAt(cursor);
    if (quote) {
      if (char === quote) {
        quote = void 0;
      }
...
function invalidTag(text3, start) {
  const nested = text3.indexOf("<", start + 1);
  return { kind: "invalid", next: nested === -1 ? text3.length : nested };
}
function skipTagWhitespace(text3, start) {
  let cursor = start;
  while (cursor < text3.length && isTagWhitespace(text3.charAt(cursor))) {
    cursor += 1;
  }
  return cursor;
}
...
function reduceReasoningText(text3, codeSpans, state, options) {
  const output = [];
  const emit = (kind, value) => {
    if (!value) {
      return;
    }
    const previous2 = output.at(-1);
    if (previous2?.kind === kind) {
      previous2.text += value;
    } else {
  ...
  const scan = scanReasoningTags(text3.slice(start), options.final);
  const tags = [];
  ...
  for (let tagIndex = 0; tagIndex < tags.length; tagIndex += 1) {
    ...
    const beforeTag = text3.slice(cursor, tag.index);
    append(beforeTag);
    ...
    if (options.mode === "visible") {
      ...
      const after = text3.slice(tagEnd);
...
function stripReasoningTagsFromMarkdown(text3, options) {
...
function stripReasoningTagsFromText(text3, options) {
  if (!text3) {
    return text3;
  }
