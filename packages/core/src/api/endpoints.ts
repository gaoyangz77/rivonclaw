...
export function getCsRelayHttpUrl(): string {
  const explicitOverride =
    typeof process !== "undefined" ? process.env.CS_RELAY_HTTP_URL : undefined;
...
export function getReleaseFeedUrl(locale: string): string {
  ...
  if (useStaging)
    return `https://${firstPartyDomain(DEFAULTS.domains.staging, DEFAULTS.domains.stagingCn)}/releases`;
...
/** Update one self-built Feishu/Lark application's bot ability settings. */
export function getFeishuApplicationAbilityUrl(domain: string, appId: string): string {
