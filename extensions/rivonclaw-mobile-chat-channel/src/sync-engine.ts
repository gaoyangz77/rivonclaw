...
export class MobileSyncEngine {
    ...
    this.saveChain = this.saveChain
      ...
      .then(async () => {
        try {
          ...
          console.error(
            `[MobileSync:${this.pairingId.slice(0, 8)}] Failed to save sync state:`,
            err.message,
          );
        }
      });
  ...
  public async resetSession() {
    ...
    if (storePath) {
      try {
        ...
        console.error(
          `[MobileSync:${this.pairingId.slice(0, 8)}] Failed to update session store:`,
          err.message,
        );
      }
    }
  ...
  private async handleIncoming(msg: any) {
    switch (msg.type) {
      ...
      case "delivery_failed":
        ...
        if (msg.messageId) {
          console.warn(
            `[MobileSync:${this.pairingId.slice(0, 8)}] delivery_failed for ${msg.messageId}: ${msg.reason}`,
          );
        }
        break;
  ...
  private async processIncomingPayload(msg: any): Promise<boolean> {
    ...
    try {
      ...
      if (repliesDelivered) {
        console.error(
          "[MobileSync] Post-dispatch error (replies already sent, suppressed):",
          dispatchErr.message,
        );
      } else {
        throw dispatchErr;
