...
function resolveSqliteDir(repoRoot) {
  const resolve = (dir) =>
    fs.realpathSync(
      path.dirname(
        createRequire(path.join(repoRoot, dir, "package.json")).resolve(
  ...
  if (desktop !== storage)
    throw new Error("Desktop and storage resolve different better-sqlite3 installations");
  const modules = fs.realpathSync(path.join(repoRoot, "node_modules"));
  if (!modules.startsWith(`${fs.realpathSync(repoRoot)}${path.sep}`))
    throw new Error("Refusing shared node_modules outside this worktree");
  if (!desktop.startsWith(`${modules}${path.sep}`))
    throw new Error(`Refusing to rebuild outside this worktree: ${desktop}`);
  return desktop;
}
...
function run(command, args, cwd) {
  ...
  if (result.status !== 0)
    throw new Error(`Native build failed (exit ${result.status}): ${command} ${args.join(" ")}`);
}
...
async function main() {
  ...
  if (Number(sqliteVersion.split(".")[0]) >= 13) {
    ...
    if (result.error) throw result.error;
    if (result.status !== 0)
      throw new Error(`Electron N-API SQLite verification failed (exit ${result.status})`);
    return;
  }
  const platform = `${process.platform}-${process.arch}`;
  const binding = (abi) =>
    path.join(sqliteDir, "lib/binding", `node-v${abi}-${platform}`, "better_sqlite3.node");
  ...
  if (
    ...
    fs.existsSync(nodeBinding) &&
    fs.existsSync(electronBinding) &&
    !fs.existsSync(buildDir)
  ) {
    console.log(
      `[rebuild-native] Both exact ABI prebuilds exist: ${process.versions.modules}, ${electronAbi}`,
    );
    return;
  }
  ...
  run(
    process.execPath,
    [path.join(path.dirname(rebuildEntry), "cli.js"), "-f", "-o", "better-sqlite3"],
    desktopDir,
  );
  ...
  fs.rmSync(buildDir, { recursive: true, force: true });
  console.log(
    `[rebuild-native] Ready: Node ABI ${process.versions.modules}, Electron ABI ${electronAbi} (${platform})`,
  );
}
