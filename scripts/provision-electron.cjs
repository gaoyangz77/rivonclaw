...
function provisionElectron() {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [path.join(packageDir, "install.js")], {
      env,
      stdio: "inherit",
    });
  ...
  if (versions.electron !== version)
    throw new Error(`Electron binary ${versions.electron} != package ${version}`);
  console.log(
    `[provision-electron] Electron ${versions.electron}, Node ${versions.node}, ABI ${versions.modules}: ${binary}`,
  );
}
