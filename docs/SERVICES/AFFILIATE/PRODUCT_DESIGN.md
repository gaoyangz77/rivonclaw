...
“团队与渠道”中的保护名单导入采用显式的两阶段语义：

1. Panel 在本地解析 Excel，并用卖家账号下包含归档记录的完整 BD 列表匹配 `bd_name`。
2. 非空但未匹配的名称不会被跳过；用户必须选择创建新 BD、恢复同名归档 BD、映射到已有 BD、仅保护不分配，或明确排除这些行。
3. 创建和恢复通过用户级幂等批量接口完成。BD 名称按 NFKC、空白折叠和小写规范化后，在同一 `userId` 下唯一。
4. 从保护名单创建或恢复的 BD 默认不接收新达人、仅人工处理，并标记为“资料待完善”。只有人工检查并保存资料后才进入已配置状态。
5. BD ID 返回并重新映射到 Excel 行后，保护记录才会分片写入；Excel 原文件不会上传或持久化。

空 `bd_name` 始终表示“保护该 Creator，但不分配人工 BD”。这与“填写了一个系统不认识的 BD 名称”是两种不同语义。
...
Agent-facing API 的安全边界是用户级 `CreatorRelationship`，不是店铺。
`creatorRelationshipId` 和 Creator identity 由 trusted run context 注入；触发本次
agenda 的店铺以 `triggerShopId` / `triggerPlatformShopId` 作为 provenance 提供，但
不能被用作 Relationship 历史或工具读取范围的隐式过滤器。模型输入只接受业务过滤
条件、卖家自有店铺/商品目标，以及它明确选择的二级业务对象 ID。
...
Affiliate Agent 的授权边界由 run harness 和 Backend 建立，不由模型选择。
每次 run 必须注入并锁定：
...
这些字段可以作为运行上下文帮助 Agent 理解当前对象，但不出现在可由模型填写的 tool
schema 中。Backend 在执行前仍从认证用户、work item 和 Relationship 重新解析并覆盖这些
常量，不能信任模型回传的同名值。

每个 Affiliate Agent tool 必须声明一种 `ToolDataScope`：

| Scope | 语义 | 允许方式 |
| --- | --- | --- |
| `CREATOR_RELATIONSHIP` | 只读取或操作当前 Relationship 及其 Creator，可跨该卖家的所有授权店铺 | 必须绑定 trusted `creatorRelationshipId` |
| `SELLER_CATALOG` | 当前卖家账号下跨店铺的非 Creator 商品目录 | Backend 必须验证所有 shop/product 都属于认证 `userId` |
| `SELLER_WIDE` | 当前卖家账号下跨 Creator 的业务数据 | Affiliate Agent run profile 禁止 |

关系、消息时间线、Collaboration、Sample Application、渠道联系人和发送动作均属于
`CREATOR_RELATIONSHIP`。商品工具使用 Affiliate 专用别名
`affiliate_get_product`、`affiliate_search_products`，属于 `SELLER_CATALOG`；搜索
结果必须携带 `shopId` / `shopName`，已知商品读取必须使用来自卖家目录或 Relationship
业务记录的 `shopId + productId`。
通用 `ecom_get_product` / `ecom_search_products` 不向 Affiliate Agent 暴露。

Affiliate Agent 不可获得：

- seller-wide WhatsApp / Email 账号列表；
- BI catalog、BI query 或跨 Creator 历史绩效数据；
- proposal approval/decision 工具；
- raw Provider route、conversation、thread 或 message ID。

`collaborationRecordId`、`sampleApplicationRecordId`、`productId` 等一对多二级业务 ID
可由 Agent 在工具结果中选择，但 Backend 必须验证 Collaboration/Sample 属于认证用户
和当前 Relationship/Creator，并从记录自身解析目标店铺；不能要求它同时属于触发本次
run 的店铺。商品和新 Target Collaboration 的目标店铺可由 Agent 从卖家自有目录中
选择，Backend 必须验证 `shopId + productId` 属于认证用户。验证失败必须在 proposal
创建和 Provider side effect 之前拒绝。Target Collaboration 的 Creator
ID/OpenID/username 仍由 trusted context 注入，Agent 只选择目标店铺、商品和商业参数。

Relationship 是长生命周期 session boundary。一个 run 可以看到同一 Creator 在多家
店铺中的 Collaboration、Sample、消息和商品上下文，也可以形成多个带各自 `shopId` 的
action step。`triggerShopId` 只解释“哪家店的事件唤醒了 Agent”，不能锁定 session，
不能阻止跨店读取，也不能成为 proposal 的隐式目标店铺。Sample/Collaboration action
从所选业务记录解析店铺；新 Target Collaboration 从显式选择的卖家自有
`shopId + productId` 解析店铺；消息回复从触发 lifecycle event / channel route 解析
店铺。选定 action route 后仍然禁止静默跨店 fallback。
...
MySQL history warehouse
  ...
  analytics-only storage for conversation and messaging-effectiveness analysis
  never used as operational state, Agent history input, delivery routing, or provider fallback
...
所有 Agent-facing tool 的服务端 binding key 必须是 trusted
`creatorRelationshipId`；它不是模型可填写的参数。
...
正确入口由 harness 绑定 Relationship 和 Creator identity，并把 trigger shop 作为
agenda provenance；模型只填写过滤条件、卖家自有目标店铺/商品或二级业务对象：
...
Forbidden as tool input:
  userId
  creatorRelationshipId
  creatorId / creatorOpenId / creatorUsername
...
推荐把输入 schema 设计成由 trusted context 限定的 relationship query：
...
```text
Agent call:
  get_creator_chat_history({ channelFilter })

Service internals:
  trusted creatorRelationshipId + optional shop/channel business filters
...
如果 Agent 不传 `shopIds`，返回该 Relationship 在卖家所有授权店铺中的合并时间线；
如果传入店铺过滤，它只能来自卖家自有店铺并由 Backend 校验。`triggerShopId` 可以帮助
Agent 理解本次 agenda 的来源，但不会自动过滤历史，也不会阻止 Agent 查看或选择另一家
店作为新的 Target Collaboration 目标。
...
Agent 可以基于 `channel=PLATFORM_CHAT`、`shop=6号` 理解来源，也可以在后续动作里表达
`channelPreference=PLATFORM_CHAT`；`shopId` 仍由 trusted run context 固定。
Agent 不应该看到、保存、选择或回传 raw provider route id。

例如，Agent 可以根据输出说：

```text
这条消息来自 TikTok Shop / 6号，所以回复时优先使用 PLATFORM_CHAT。
```
...
同一个 relationship 可以同时拥有多个 agenda item。例如 A 合作等待达人产出内容，B 合作需要 Agent 审核样品，C 合作已批准样品并等待 TikTok FBT 发货；三个事项都必须保留，不能以更新时间或优先级压成一个状态。
...
- `SAMPLE_REVIEW`
- `SAMPLE_PLATFORM_FULFILLMENT_WAIT`
- `CONTENT_PENDING`
- `CONTENT_POSTED`
- `CLOSED`
...
- `SAMPLE_AWAITING_PLATFORM_SHIPMENT`
- `SAMPLE_IN_TRANSIT`
- `SAMPLE_DELIVERED`
...
样品申请被批准后，TikTok 会创建 sample order，并由 FBT 自动履约。系统必须投影为 `WAITING_EXTERNAL / WAIT_PLATFORM_UPDATE`，不得创建 `SHIP_SAMPLE` 人工任务或重复创建仓配订单。旧的 `SHIP_SAMPLE`、`SAMPLE_SHIPMENT` 和 `SAMPLE_AWAITING_SHIPMENT` 记录按 legacy repair 语义统一重算为平台等待态。

Creator 入站附件在 Desktop harness 创建 Agent run 之前执行类型预检。当前可理解白名单是安全图片（JPEG/PNG/GIF/WebP）、文本/JSON/XML/CSV/Markdown、PDF、Word OOXML（DOCX/DOCM）和 PowerPoint OOXML（PPTX/PPTM）；宏只作为压缩文档读取文本，绝不执行。视频、音频、旧 Office 二进制、压缩包、可执行文件、未知 part 或缺失 Provider 历史均直接转人工，不依赖提示词让模型自行判断。
...
- The latest internal user turn contains only `Agent Working Agenda` plus trusted CreatorRelationship/Creator constants and trigger-shop provenance. It is a wake-up reason, not a fact snapshot, and the trigger shop is not a read boundary.
...
- `affiliate_list_creator_collaborations`: CreatorRelationship-owned Mongo operational Collaboration projections across every linked seller shop, with per-shop readiness and history coverage.
- `affiliate_list_creator_sample_applications`: CreatorRelationship-owned Mongo operational Sample Application projections across every linked seller shop, with per-shop readiness and history coverage. An empty result is authoritative only when every relevant shop reports complete coverage.
- `affiliate_list_shops`: the authorized seller shop catalog used to choose an explicit shop for seller-catalog or new Target Collaboration work.
- `affiliate_get_product` / `affiliate_search_products`: seller-catalog product facts for an explicitly selected authorized shop; they never expose another Creator's information.
...
The CreatorRelationship and Creator identity are injected from the Affiliate session. Relationship tools may span its linked shops but cannot read another Creator. Exact Collaboration/Sample tools derive their shop from the relationship-owned Mongo record. Seller-catalog tools accept only an explicitly selected shop owned by the same seller. Action execution resolves its shop per step instead of inheriting one run-wide shop.
...
- `AffiliateCollaboration`：店铺级 Provider collaboration metadata；它可能是 open/general
  collaboration，本身不证明当前 Creator 参与。
- `AffiliateCollaborationRecord`：Creator-specific participation projection，必须同时关联
  Relationship/Creator、店铺和 parent collaboration。Agent 的 list/get collaboration
  工具只返回当前 Relationship 的 record，不得把 general campaign 当成 Creator 的合作。
