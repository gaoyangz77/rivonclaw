...
Agent-facing API 的规则更简单：`creatorRelationshipId`、Creator identity 和本次
`focusShopId` 由 trusted run context 注入；模型输入只接受业务过滤条件和它明确选择的
二级业务对象 ID。
...
| Scope | 语义 | 允许方式 |
| --- | --- | --- |
| `CREATOR_RELATIONSHIP` | 只读取或操作当前 Relationship 及其 Creator | 必须绑定 trusted `creatorRelationshipId` |
| `CURRENT_SHOP` | 只读取当前 run 的 focus shop | 必须绑定 trusted `focusShopId` |
| `SELLER_WIDE` | 当前卖家账号下跨 Creator/店铺 | Affiliate Agent run profile 禁止 |

关系、消息时间线、Collaboration、Sample Application、渠道联系人和发送动作均属于
`CREATOR_RELATIONSHIP`。商品工具使用 Affiliate 专用别名
`affiliate_get_product`、`affiliate_search_products`，属于 `CURRENT_SHOP`。
...
可由 Agent 在工具结果中选择，但 Backend 必须验证它属于认证用户、精确 focus shop 和
当前 Relationship/Creator；验证失败必须在 proposal 创建和 Provider side effect 之前
拒绝。Target Collaboration 的 Creator ID/OpenID/username 由 trusted context 注入，
Agent 只选择业务参数。

Relationship 仍是长生命周期 session boundary，但每个 work-item run 只允许一个
`focusShopId`。同一 Relationship 的下一次 run 可以聚焦另一家店；当前 run 不得从
routing shops fallback，也不得在 active session 中切换 focus shop。
...
正确入口由 harness 绑定 Relationship 和 focus shop；模型只填写过滤条件或二级业务
对象：
...
Service internals:
  trusted creatorRelationshipId + trusted focusShopId + channelFilter
...
当前店铺由 `focusShopId` 固定注入，不允许 Agent 选择“6 号店”或其他 routing shop。
