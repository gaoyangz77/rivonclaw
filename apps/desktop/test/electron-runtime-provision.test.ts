...
describe("Electron runtime packaging guards", () => {
  let root: string;
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "electron-runtime-"));
  });
  afterEach(() => {
  ...
  it.each(["checkout", "checkout with spaces #100%"])(
    "loads the native build's ESM dependency from %s",
    (name) => {
      ...
      write(
        ...
        fs.readFileSync(new URL("../../../scripts/rebuild-native.cjs", import.meta.url), "utf8"),
      );
      ...
      write(
        ...
        JSON.stringify({ name: "better-sqlite3", version: "13.0.3" }),
      );
      write(
        "node_modules/electron/package.json",
        JSON.stringify({ name: "electron", version: "42.11.3" }),
      );
      write(
        "node_modules/node-abi/package.json",
        JSON.stringify({ name: "node-abi", type: "module", exports: "./index.mjs" }),
      );
      write(
        "node_modules/node-abi/index.mjs",
        `
    ...
    `,
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(checkout, "scripts/rebuild-native.cjs")],
        {
          cwd: checkout,
          encoding: "utf8",
          timeout: 10_000,
  ...
  it("discovers both legacy js and new mjs workspace chunks, not declarations", () => {
    ...
    expect(findWorkspaceBundles(root).map((file: string) => path.basename(file))).toEqual([
      "workspace-new.mjs",
      "workspace-old.js",
    ]);
  });
  ...
  it("removes inherited host runtime and OpenClaw state overrides", () => {
    expect(
      isolatedEnvironment({
        PATH: "path",
        Path: "windows-host",
        path: "lower-case-host",
        NODE_PATH: "host",
        NODE_OPTIONS: "--require host",
        OPENCLAW_CONFIG_PATH: "real-user",
        FS_SAFE_NATIVE_MODE: "required",
        HOME: "real-home",
      }),
    ).toEqual({});
  });
  ...
  it.each([".openclaw-lifecycle-pending", "dist/openclaw-install-guard"])(
    "rejects unfinished package marker %s",
    (marker) => {
