...
function canvasFixture(code?: string) {
  ...
  if (code !== undefined) {
    ...
    fs.writeFileSync(
      path.join(canvas, "package.json"),
      JSON.stringify({ name: "@napi-rs/canvas", version: "1.0.0", main: "index.js" }),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: `@napi-rs/canvas-darwin-${process.arch}`, version: "1.0.0" }),
    );
  }
  return root;
}
...
describe("external native runtime smoke", () => {
  it("rejects a missing Canvas payload", () => {
    expect(() => runExternalNativeRuntimeSmoke(canvasFixture())).toThrow(
      "Missing packaged Canvas runtime",
    );
  });
  it("propagates native loading failures", () => {
    expect(() =>
      runExternalNativeRuntimeSmoke(canvasFixture('throw new Error("native load failed");')),
    ).toThrow("native load failed");
  });
  it.each([0, 1])(
    "renders and checks the output rather than only the manifest (bytes=%i)",
    (bytes) => {
      const root = canvasFixture(
        `exports.createCanvas = () => ({ getContext: () => ({ fillRect() {} }), toBuffer: () => Buffer.alloc(${bytes}) });`,
      );
      if (bytes) expect(() => runExternalNativeRuntimeSmoke(root)).not.toThrow();
      else
        expect(() => runExternalNativeRuntimeSmoke(root)).toThrow(
          "Packaged Canvas rendering failed",
        );
    },
  );
});
...
describe("actual packaged plugin inventory contract", () => {
  it("reads top-level QR method registrations from aggregate runtime inspection", () => {
    expect(() =>
      assertLoadedPluginInventory(inventory("loaded"), ["openclaw-weixin"], true),
    ).not.toThrow();
  });
  it("rejects dependency-health errors even when channel registration succeeded", () => {
    expect(() =>
      assertLoadedPluginInventory(inventory("error"), ["openclaw-weixin"], true),
    ).toThrow("missing dependency");
  });
  it("rejects missing plugins and missing QR registrations", () => {
    expect(() => assertLoadedPluginInventory([], ["openclaw-weixin"], true)).toThrow(
      "did not discover",
    );
    expect(() =>
      assertLoadedPluginInventory(inventory("loaded", [methods[0]]), ["openclaw-weixin"], true),
    ).toThrow(methods[1]);
  });
  it("checks every external plugin rather than only the vendor allowlist", () => {
    expect(() =>
      assertLoadedPluginInventory(inventory("loaded"), ["openclaw-weixin", "rivonclaw-cs"], true),
    ).toThrow("rivonclaw-cs");
  });
});
