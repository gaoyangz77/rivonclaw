...
const {
  ...
  VENDOR_ARCHIVE_ENV,
} = require("../scripts/vendor-plugin-size.cjs");
...
beforeEach(() => {
  root = fs.mkdtempSync(path.join(os.tmpdir(), "plugin-size-test-"));
});
afterEach(() => {
  vi.restoreAllMocks();
...
function fixture(vendor = path.join(root, "vendor")) {
  for (const base of ["dist", "dist-runtime"]) {
    ...
    write(path.join(modules, "host/package.json"), {
      name: "host",
      version: "1.0.0",
      main: "index.js",
...
describe("mirrored plugin dependency size", () => {
  it("hardlinks only identical relative files, preserving nested versions, paths and module resolution", () => {
    ...
    for (const relative of ["dep/index.js", "host/node_modules/dep/index.js"]) {
      ...
      expect(fs.realpathSync(path.join(source, relative))).not.toBe(
        fs.realpathSync(path.join(destination, relative)),
      );
    }
  ...
  it("does not change executable permissions or follow private file symlinks", () => {
    ...
    if (process.platform !== "win32") {
      ...
      expect(fs.statSync(path.join(destination, "host/tool.js")).ino).not.toBe(
        fs.statSync(path.join(source, "host/tool.js")).ino,
      );
    }
  });
  ...
  it("keeps the original file intact when hardlink creation fails", () => {
    ...
    expect(() => deduplicateMirroredPluginDependencies(vendor)).toThrow(
      "Cannot hardlink mirrored runtime file",
    );
    expect(fs.readFileSync(file)).toEqual(before);
    expect(
      fs.readdirSync(path.dirname(file)).some((name) => name.includes("rivonclaw-hardlink")),
    ).toBe(false);
  });
  ...
  it("tar records a hardlink, extraction preserves it, and private imports still resolve", async () => {
    ...
    for (const base of ["dist", "dist-runtime"])
      write(
        path.join(vendor, base, "extensions/feishu/node_modules/dep/large.js"),
        "x".repeat(256 * 1024),
      );
    ...
    await tar.t({
      ...
      onReadEntry: (entry: any) => {
        if (entry.type === "Link") links.push(entry.path);
      },
    });
...
describe("private source-map pruning", () => {
  it("removes test/build cache payloads without deleting runtime caches or data files", () => {
    ...
    expect(fs.readFileSync(path.join(modules, "runtime/cache/data.json"), "utf8")).toBe(
      "runtime data",
    );
  ...
  it("prunes maps from both runtime roots without stripping canonical SDK output", () => {
    ...
    for (const dir of [source, destination])
      write(path.join(dir, "dep/index.js.map"), { version: 3, sources: [], mappings: "" });
    write(path.join(vendor, "dist/plugin-sdk/index.d.ts"), "export {};");
  ...
  it("removes only valid JS/TS source maps through the existing prune boundary", () => {
    ...
    for (const name of ["index.js.map", "index.mjs.map", "index.cjs.map", "index.d.ts.map"]) {
      write(path.join(source, "host", name), {
        version: 3,
    ...
    write(
      path.join(source, "host/index.js"),
    ...
    const fn = script.slice(
      script.indexOf("function stripNonRuntimeFiles("),
      script.indexOf("\nfunction stripOtherDarwinArch("),
    );
    const strip = runInNewContext(`${fn}; stripNonRuntimeFiles`, {
      vendorDir: vendor,
      isSelectedPluginNodeModules,
      stripPrivateSourceMaps,
    });
    expect(strip(source).files).toBe(4);
    for (const name of ["LICENSE", "index.d.ts", "asset.js.map", "location.map"])
      expect(fs.existsSync(path.join(source, "host", name))).toBe(true);
    expectPrivateResolution(vendor);
  ...
  it("uses the same AppleDouble-free tar environment in production", () => {
    ...
    const archive = fs.readFileSync(
      new URL("../scripts/archive-vendor-runtime.cjs", import.meta.url),
      "utf8",
    );
...
describe("root and private runtime dependency deduplication", () => {
  it("archives shared root/private files once and preserves imports after extraction", async () => {
    ...
    const env = { ...process.env, ...VENDOR_ARCHIVE_ENV };
    execFileSync("tar", ["-cf", before, "-C", vendor, "node_modules", "dist", "dist-runtime"], {
      env,
    });
    deduplicateRuntimeDependencies(vendor);
    execFileSync("tar", ["-cf", after, "-C", vendor, "node_modules", "dist", "dist-runtime"], {
      env,
    });
    ...
    expectPrivateResolution(extracted);
    expect(fs.statSync(path.join(extracted, "node_modules/dep/large.js")).ino).toBe(
      fs.statSync(path.join(extracted, "dist/extensions/feishu/node_modules/dep/large.js")).ino,
    );
  });
  ...
  it("shares identical package files across all three copies without changing private resolution", () => {
    ...
    expect(result.savedBytes).toBeGreaterThan(0);
    const files = [
      path.join(vendor, "node_modules/dep/index.js"),
      path.join(source, "dep/index.js"),
      path.join(destination, "dep/index.js"),
    ];
  ...
  it("keeps identical bytes separate across package versions and names", () => {
    ...
    deduplicateRuntimeDependencies(vendor);
    expect(
      new Set([current, nested, scoped].map((dir) => fs.statSync(path.join(dir, "shared.js")).ino))
        .size,
    ).toBe(3);
    expectPrivateResolution(vendor);
  });
  ...
  it("does not share different bytes, native binaries, permissions or external hardlinks", () => {
    ...
    for (const file of [
      ...
      "data.json",
      "external.js",
      ...(process.platform === "win32" ? [] : ["mode.js"]),
    ]) {
      expect(fs.statSync(path.join(target, file)).ino).not.toBe(
        fs.statSync(path.join(source, "dep", file)).ino,
      );
    }
