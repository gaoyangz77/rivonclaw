...
const {
  preparePluginManifest,
  packageExternalPlugins,
} = require("../scripts/package-external-plugins.cjs");
const roots: string[] = [];
...
function fixture(
  name = "@rivonclaw/rivonclaw-event-bridge",
  dependencies: Record<string, string> = { "@rivonclaw/plugin-sdk": "workspace:*" },
) {
  ...
  write(path.join(root, "repo/vendor/openclaw/package.json"), {
    exports: { "./plugin-sdk/test": "./dist/test.mjs" },
  });
...
describe("packaged external plugin manifests", () => {
  ...
  it.each([
    ...
    expect(() => preparePluginManifest(source, dest, {})).toThrow(
      "Claimed bundled dependency still imported",
    );
  });
  ...
  it("requires all copied PDF.js assets before removing its dependency", () => {
    const { source, dest } = fixture("@rivonclaw/rivonclaw-cloud-tools", {
      "pdfjs-dist": "5.7.284",
    });
  ...
  it("ships real dynamic imports and their complete closure in packaged-only node_modules", () => {
    ...
    write(
      path.join(dest, "dist/index.mjs"),
      'await import("qrcode-terminal"); await import("silk-wasm");',
    );
    for (const [name, version] of [
      ["qrcode-terminal", "0.12.0"],
      ["silk-wasm", "3.7.1"],
    ]) {
      write(path.join(source, "node_modules", name, "package.json"), {
        name,
        version,
        main: "index.js",
      ...
      write(
        path.join(source, "node_modules", name, "index.js"),
        'module.exports = require("child");',
      );
    }
    write(path.join(source, "node_modules/child/package.json"), {
      name: "child",
      version: "1.0.0",
      main: "index.js",
    });
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(source, "package.json"), "utf8")).dependencies,
    ).toHaveProperty("@tencent-weixin/openclaw-weixin");
  });
  ...
  it("fails instead of hiding a real missing external", () => {
    ...
    expect(() =>
      packageExternalPlugins(path.join(root, "resources"), path.join(root, "repo")),
    ).toThrow("Missing runtime dependency");
  });
});
