...
const {
  copySelectedPluginDependencies,
  shouldCopyVendorNative,
} = require("../scripts/copy-vendor-deps.cjs");
...
describe("selected vendor plugin payload", () => {
  ...
  beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), "vendor-plugin-deps-"));
  });
  afterEach(() => {
  ...
  it("preserves source-installed versions, transitive conflicts and cycles after source removal", () => {
    ...
    const a = pkg(path.join(root, "node_modules/.store/a"), "a", "1.0.0", {
      b: "1.0.0",
      shared: "^1.0.0",
    });
    const b = pkg(path.join(root, "node_modules/.store/b"), "b", "1.0.0", {
      a: "1.0.0",
      shared: "^2.0.0",
    });
    ...
    expect(
      JSON.parse(
        fs.readFileSync(path.join(resolvePackage("shared", copiedB), "package.json"), "utf8"),
      ).version,
    ).toBe("2.0.0");
  ...
  it("rejects a wrong hoisted version rather than silently shipping it", () => {
    pkg(path.join(root, "dist/extensions/feishu"), "@openclaw/feishu", "2026.9.3", {
      dep: "1.0.0",
    });
  ...
  it("ignores optional peers found only in the surrounding build checkout", () => {
    ...
    const plugin = pkg(
      path.join(payload, "dist/extensions/feishu"),
      "@openclaw/feishu",
      "2026.9.3",
      {},
      {
  ...
  it("does not satisfy required dependencies from the surrounding build checkout", () => {
    const payload = path.join(root, "packed/vendor/openclaw");
    pkg(path.join(payload, "dist/extensions/feishu"), "@openclaw/feishu", "2026.9.3", {
      encoding: "^0.1.0",
    });
    pkg(path.join(root, "node_modules/encoding"), "encoding", "0.1.13");
    expect(() => materializeSelectedPluginDependencies(payload, ids)).toThrow(
      "Missing runtime dependency",
    );
    expect(() => assertSelectedPluginDependencies(payload, ids)).toThrow(
      "Missing packaged runtime dependency",
    );
  });
  ...
  it("still rejects optional dependencies linked outside the payload", () => {
    ...
    expect(() => materializeSelectedPluginDependencies(payload, ids)).toThrow(
      "escapes vendor payload",
    );
    expect(() => assertSelectedPluginDependencies(payload, ids)).toThrow("escapes vendor payload");
  });

  it.each([false, true])(
    "preserves direct/transitive version conflicts through the real prune walkers (direct first: %s)",
    (directFirst) => {
      const deps = directFirst
        ? { shared: "11.0.2", sdk: "1.0.0" }
        : { sdk: "1.0.0", shared: "11.0.2" };
      const plugin = pkg(
        path.join(root, "dist-runtime/extensions/feishu"),
        "@openclaw/feishu",
        "2026.9.3",
        deps,
      );
      ...
      const pruning = fs.readFileSync(
        new URL("../scripts/prune-vendor-deps.cjs", import.meta.url),
        "utf8",
      );
      const functions = ["removeSymlinksAndNestedNodeModules", "stripNonRuntimeFiles"]
        .map((name) => {
        ...
        .join("\n");
      const junk = pkg(
        path.join(root, "dist-runtime/extensions/unselected/node_modules/junk"),
        "junk",
        "1.0.0",
      );
      runInNewContext(
        `${functions}\nremoveSymlinksAndNestedNodeModules(target); stripNonRuntimeFiles(target);`,
        {
          ...
          path,
          vendorDir: root,
          target: path.join(root, "dist-runtime"),
          isSelectedPluginNodeModules,
          dirSize: () => 0,
          fileCount: () => 0,
      ...
      expect(
        JSON.parse(
          fs.readFileSync(path.join(resolvePackage("shared", sdk), "package.json"), "utf8"),
        ).version,
      ).toBe("10.9.1");
      expect(
        JSON.parse(
          fs.readFileSync(path.join(resolvePackage("shared", plugin), "package.json"), "utf8"),
        ).version,
      ).toBe("11.0.2");
    },
  );
  ...
  it("copies required npm dependencies named after Node builtins", () => {
    ...
    expect(() => assertSelectedPluginDependencies(root, ids)).toThrow(
      /Missing packaged runtime dependency buffer|Runtime dependency escapes vendor payload/,
    );
  });
  ...
  it.each(["protobufjs", "@larksuiteoapi/node-sdk@1.73.0>protobufjs"])(
    ...
    (selector) => {
      ...
      const sdk = pkg(
        path.join(root, "node_modules/@larksuiteoapi/node-sdk"),
      ...
      lock(
        { "@larksuiteoapi/node-sdk@1.73.0": { dependencies: { protobufjs: "8.7.2" } } },
        { [selector]: "8.7.2" },
      );
      materializeSelectedPluginDependencies(root, ids);
      expect(
        JSON.parse(fs.readFileSync(path.join(sdk, "package.json"), "utf8")).dependencies.protobufjs,
      ).toBe("^7.2.6");
      const copied = resolvePackage("@larksuiteoapi/node-sdk", plugin);
      expect(
        JSON.parse(fs.readFileSync(path.join(copied, "package.json"), "utf8")).dependencies
          .protobufjs,
      ).toBe("8.7.2");
...
`;
  ...
  it.each(["LF", "BOM + CRLF"])(
    "reads pnpm 12's project document, not bootstrap resolutions (%s)",
    (format) => {
      const plugin = pkg(
        path.join(root, "dist/extensions/feishu"),
        "@openclaw/feishu",
      ...
      expect(
        JSON.parse(
          fs.readFileSync(path.join(resolvePackage("sdk", plugin), "package.json"), "utf8"),
        ).dependencies.dep,
      ).toBe("8.7.2");
      fs.rmSync(path.join(root, "node_modules"), { recursive: true });
      fs.rmSync(path.join(root, "pnpm-lock.yaml"));
  ...
  it("handles locked alias replacements and peer-suffixed resolutions", () => {
    ...
    lock(
      { "sdk@1.0.0(peer@1.0.0)": { dependencies: { legacy: "replacement@2.0.0(peer@1.0.0)" } } },
      { legacy: "npm:replacement@2.0.0" },
    );
    materializeSelectedPluginDependencies(root, ids);
  ...
  it.each(["3.25.76", "4.4.3"])(
    "accepts the installed Zod %s peer context without requiring unrelated contexts to match",
    (version) => {
      const plugin = pkg(
        path.join(root, "dist/extensions/feishu"),
        "@openclaw/feishu",
      ...
      pkg(
        path.join(root, "node_modules/zod-to-json-schema"),
        "zod-to-json-schema",
        "3.25.2",
        {},
      ...
      expect(
        JSON.parse(
          fs.readFileSync(path.join(resolvePackage("zod", schema), "package.json"), "utf8"),
        ).version,
      ).toBe(version);
      expect(() => assertSelectedPluginDependencies(root, ids)).not.toThrow();
    },
  ...
  it("materializes locally built patched mjs sidecars and copies private dependencies", () => {
    const built = pkg(path.join(root, "dist/extensions/feishu"), "@openclaw/feishu", "2026.9.3", {
      dep: "1.0.0",
    });
    const overlay = pkg(
      path.join(root, "dist-runtime/extensions/feishu"),
    ...
    pkg(path.join(packed, "dist/extensions/feishu"), "@openclaw/feishu", "2026.9.3", {
      dep: "1.0.0",
    });
    pkg(path.join(packed, "dist-runtime/extensions/feishu"), "@openclaw/feishu", "2026.9.3", {
      dep: "1.0.0",
    });
  ...
  it("preserves unselected CJS plugin entries and chunks before link cleanup, including a partial-prune retry", () => {
    const extra = {
      openclaw: {
        ...
        channel: { configuredState: { specifier: "./configured-state.cjs" } },
      },
    };
    const built = pkg(
      path.join(root, "dist/extensions/msteams"),
      "@openclaw/msteams",
      "2026.9.3",
      {},
      extra,
    ...
    const overlay = pkg(
      path.join(root, "dist-runtime/extensions/msteams"),
      "@openclaw/msteams",
      "2026.9.3",
      {},
      extra,
    ...
    for (const name of [
      "index.cjs",
      "setup-entry.cjs",
      "configured-state.cjs",
      "chunk.cjs",
      "lazy.mjs",
    ]) {
      write(
        path.join(built, name),
        name === "index.cjs" ? 'module.exports = require("./chunk.cjs");' : "module.exports = 42;",
      );
    ...
    expect(fs.readFileSync(path.join(overlay, "index.js"), "utf8")).toContain(
      "preserve upstream ESM wrapper",
    );
  });

  it.each(["index.cjs", "setup-entry.cjs", "configured-state.cjs"])(
    "rejects an unselected plugin's missing %s",
    (missing) => {
      const extra = {
        openclaw: {
          extensions: ["./index.cjs"],
      ...
      for (const name of ["index.cjs", "setup-entry.cjs", "configured-state.cjs"])
        if (name !== missing) write(path.join(dir, name), "module.exports = {};");
      expect(() => assertBundledPluginEntries(root)).toThrow(
        `Missing retained plugin entry: dist-runtime/msteams/./${missing}`,
      );
    },
  );
