...
export default function globalSetup() {
  // Kill current and pre-rebrand instances from previous test runs.
  ...
  if (process.platform === "darwin") {
    ...
    try {
      execSync("killall -9 'TK Copilot' RivonClaw EasyClaw 2>/dev/null || true", {
        stdio: "ignore",
      });
    } catch {}
  }
  if (process.platform === "win32") {
    try {
      execSync(
        'taskkill /F /IM "TK Copilot.exe" 2>nul & taskkill /F /IM RivonClaw.exe 2>nul & taskkill /F /IM EasyClaw.exe 2>nul & exit 0',
        { stdio: "ignore" },
      );
    } catch {}
  }
