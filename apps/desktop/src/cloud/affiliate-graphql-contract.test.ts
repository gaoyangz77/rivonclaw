...
describe("affiliate desktop GraphQL contracts", () => {
  it.each([
    ...
    ["affiliate work-item resolution", RESOLVE_AFFILIATE_WORK_ITEM_MUTATION],
  ...
  it("keeps Agent work-item resolution output to a boolean receipt", () => {
    const compactMutation = RESOLVE_AFFILIATE_WORK_ITEM_MUTATION.replace(/\s+/g, " ");
    const payload = backendSchema.getType("ResolveAffiliateWorkItemPayload");

    expect(compactMutation).toContain("resolveAffiliateWorkItem(input: $input) { success }");
    expect(payload?.toString()).toBe("ResolveAffiliateWorkItemPayload");
    if (payload == null || !("getFields" in payload)) return;
    expect(Object.keys(payload.getFields())).toEqual(["success"]);
    expect(compactMutation).not.toMatch(
      /prediction|expectedSales|proposal|executionResult|affiliateCollaboration/u,
    );
  });
  ...
  it.each([
    ...
    const selections = [...source.matchAll(/creatorProfile\s*\{([^}]*)\}/g)].map(
      (match) => match[1],
    );
