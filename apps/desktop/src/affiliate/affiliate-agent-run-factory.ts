...
function buildSampleReviewRun(input: AffiliateAgentRunFactoryInput): AffiliateAgentRunRequest {
  ...
  return {
    message: [
      ...
      "Affiliate Prediction uses expected sales units. expectedSalesUnits is a statistical expected-value forecast for this creator/product/sample context; it is not a guarantee of actual sales.",
      ...
      "Write operatorSummary for a busy ecommerce seller, not a statistician. Explain the business meaning in plain language, for example: \"The expected-sales model estimates around 0 units for this creator and product, below the shop's minimum of 2, so rejecting the sample is recommended.\" Do not include raw model internals unless the merchant explicitly asks.",
