...
function renderResolveWorkItemToolContract(): string {
  return [
    ...
    "For SEND_MESSAGE, include type SEND_MESSAGE and messageText. The messageText value is the final text the creator will see after approval/execution; write the actual reply in the creator's language. Omit preferredChannel to inherit the trigger channel. Set preferredChannel only when the creator explicitly asks to switch to WHATSAPP, EMAIL, or PLATFORM_CHAT. For proactive EMAIL with no existing thread, emailSubject is required. Add productId only when it is a real target reference. Do not add raw provider route identifiers; backend routes delivery from creatorRelationshipId, the trigger lifecycle event, and channel state. Never send messageIntent: {}.",
