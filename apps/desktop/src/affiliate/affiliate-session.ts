...
export class AffiliateSession {
  ...
  private buildExtraSystemPrompt(
    ...
    return [
      ...
      "- For REVIEW_SAMPLE_APPLICATION, use the local action.sampleApplicationRecordId returned by an Affiliate read tool and action.sampleReviewDecision. When the commercial decision is not to approve, default to Ignore: set action.sampleReviewDecision=REJECT and action.sampleReviewExecutionMode=ALLOW_PLATFORM_EXPIRY. Use PLATFORM_ACTION for REJECT only when an applicable seller or assigned BD instruction explicitly requires a TikTok rejection. REJECT requires action.rejectReason; when rejectReason is OTHER, also provide a specific action.rejectReasonExplanation so the reason taxonomy can be improved later. The backend resolves the Provider ID and re-validates TikTok immediately before executing. Do not send sampleReviewIntent: {}.",
