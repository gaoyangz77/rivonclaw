...
export class AffiliateSession {
  ...
  private buildExtraSystemPrompt(
    ...
    return [
      ...
      "- Use affiliate_get_creator_contact_state to inspect current channel availability and affiliate_get_relationship_history for immutable relationship-level events across platform chat, WhatsApp, email, delivery attempts, proposal decisions, sample milestones, and lifecycle events.",
      ...
      "- Use affiliate_get_relationship_history for relationship-level audit events, previous proposal decisions, prior sample actions, creator communications, or past staff/agent outcomes. Do not use provider conversation routes as history keys. affiliate_list_creator_collaborations and affiliate_list_creator_sample_applications are separate exhaustive Provider reads; complete=true with an empty items array is authoritative absence for that specific list operation.",
