...
describe("archived CS session recovery", () => {
  ...
  it.each([archived, archived.replace(" [code=INVALID_REQUEST]", "")])(
    "restores with the current local session ID and retries once: %s",
    async (message) => {
      const bridge = createBridge();
      bridge.setShopContext(defaultShop);
      mockRpcRequest
      ...
      expect(mockRpcRequest.mock.calls[3]).toEqual([
        "sessions.patch",
        {
          key,
          archived: false,
          expectedSessionId: localSessionId,
        },
      ]);
      ...
      expect(mockEmitCsDispatchEvent).not.toHaveBeenCalledWith(
        expect.objectContaining({ outcome: "failed" }),
      );
    },
  );

  it.each(["describe", "restore", "retry"])(
    "reports a %s failure without looping or claiming success",
    async (stage) => {
      const bridge = createBridge();
      bridge.setShopContext(defaultShop);
      ...
      expect(
        mockRpcRequest.mock.calls.filter(([method]) => method === "sessions.describe"),
      ).toHaveLength(1);
      expect(
        mockRpcRequest.mock.calls.filter(([method]) => method === "sessions.patch"),
      ).toHaveLength(stage === "describe" ? 0 : 1);
      expect(mockRpcRequest.mock.calls.filter(([method]) => method === "agent")).toHaveLength(
        stage === "retry" ? 2 : 1,
      );
      expect(mockEmitCsDispatchEvent).toHaveBeenCalledWith(
        expect.objectContaining({
          outcome: "failed",
          errorMessage: expect.objectContaining({ message: failure }),
        }),
      );
      expect(mockEmitCsDispatchEvent).not.toHaveBeenCalledWith(
  ...
  it("does not restore a replaced local session or bypass its generation check", async () => {
    ...
    expect(mockEmitCsDispatchEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        outcome: "failed",
        errorMessage: expect.objectContaining({ message: changed }),
      }),
    );
    expect(mockEmitCsDispatchEvent).not.toHaveBeenCalledWith(
      expect.objectContaining({ outcome: "accepted" }),
  ...
  it.each([
    ...
    { session: { sessionId: "" } },
    { session: { sessionId: "  " } },
  ])("does not restore or retry without a local session ID: %j", async (description) => {
    const bridge = createBridge();
    bridge.setShopContext(defaultShop);
    mockRpcRequest
      .mockResolvedValueOnce({ ok: true })
