        ...
        unpaidOrderReachoutStages: [
          {
            ...
            enabled: true,
            delayMinutes: 12,
            messageTemplate: "Order {{order_id}} has {{product_count}} item(s).",
          },
        ],
...
describe("DesktopRootStore shop ingestion", () => {
  ...
  it("preserves existing service fields omitted by partial shop push payloads", () => {
    rootStore.replaceShopsFromGraphQL([customerServiceShop("shop-1", "Shop 1")], "initial");
    ...
    expect(cs?.unpaidOrderReachoutStages[0]?.messageTemplate).toBe(
...
describe("DesktopRootStore payment ingestion", () => {
  ...
  it("emits field-level replace patches when a known payment changes status", async () => {
    ...
    expect(batches).toHaveLength(1);
    expect(batches[0].map((patch) => patch.path).sort()).toEqual([
      "/payments/0/paidAt",
      "/payments/0/status",
    ]);
    applyPatch(panel, batches[0]);
