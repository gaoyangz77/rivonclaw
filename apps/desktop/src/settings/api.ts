...
//
// The route applies a small, deliberate allowlist: only the CS BI event
// types we ingest into dedicated CH tables are accepted. Anything else is a
// silent 204 drop. This prevents a misconfigured plugin from flooding the
// stream with noise. `cs.*` vs `ecom.*` follows the CH table scoping: `cs.*`
// carries conversation-scoped signals, `ecom.*` carries e-commerce-domain
// tool-call observations (which a CS conversation is only one source of).
const CS_EVENT_ALLOWLIST = new Set([
  "cs.message",
  "cs.token_snapshot",
