...
runOrSkip("vendor patch 0008 behavioral: Pi SDK validateConfig compatibility shim", () => {
  ...
  it('auth: "token" provider does not prevent custom models from loading', async () => {
    ...
    const modelsJson = {
      providers: {
        codex: {
          ...
          models: [{ id: "gpt-5.4", name: "gpt-5.4", input: ["text", "image"] }],
        },
        "test-custom": {
    ...
    writeFileSync(join(tmpDir, "models.json"), JSON.stringify(modelsJson, null, 2));

    // Create an in-memory auth storage (no disk auth needed for this test)
    const authStorage = (
      piDiscovery.AuthStorage as { inMemory: (data: object) => unknown }
    ).inMemory({});
    ...
    const testCustomModels = all.filter((m) => m.provider === "test-custom");
    ...
    const gpt54Mini = testCustomModels.find((m) => m.id === "gpt-5.4-mini");
    ...
    const modelsJson = {
      providers: {
        "oauth-provider": {
          ...
          auth: "oauth",
          api: "openai-completions",
          models: [{ id: "oauth-model-1", name: "OAuth Model", input: ["text"] }],
        },
        "test-custom-2": {
          baseUrl: "https://api.example.com/v1",
          api: "openai-completions",
  ...
  it('auth: "aws-sdk" provider does not prevent custom models from loading', async () => {
    ...
    const modelsJson = {
      providers: {
        ...
        "test-custom-3": {
          ...
          api: "openai-completions",
          apiKey: "test-key-789",
          models: [{ id: "vision-model", name: "Vision Model", input: ["text", "image"] }],
        },
      },
    };
    writeFileSync(join(tmpDir, "models.json"), JSON.stringify(modelsJson, null, 2));
    ...
    const testCustomModels = all.filter((m) => m.provider === "test-custom-3");
    expect(testCustomModels.length).toBeGreaterThanOrEqual(1);
