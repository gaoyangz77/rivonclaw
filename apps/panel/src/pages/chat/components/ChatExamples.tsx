...
export function ChatExamples({
  ...
  return (
    <div className="chat-examples" data-tutorial-id="chat-examples">
      <button className="chat-examples-toggle" onClick={onToggleExpanded}>
        <svg
          className={`chat-examples-chevron ${chatExamplesExpanded ? "chat-examples-chevron-down" : ""}`}
          width="18"
          height="18"
          viewBox="0 0 16 16"
          fill="none"
      ...
      {chatExamplesExpanded && (
        <>
          ...
          <div className="chat-examples-grid">
            {(
              ["example1", "example2", "example3", "example4", "example5", "example6"] as const
            ).map((key) => {
