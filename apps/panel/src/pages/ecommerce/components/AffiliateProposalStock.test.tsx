...
} from "./AffiliateProposalStock.js";
...
describe("Affiliate proposal stock", () => {
    ...
    steps: [
      {
        ...
        sampleReviewIntent: {
          ...
          rejectReasonExplanation:
            "Long rejection explanation with evidence, kept outside the decision metric.",
        },
      },
    ],
  ...
  it("renders long reasons outside the short decision label", () => {
    ...
    const reason = screen.getByText(
      rejection.steps[0].sampleReviewIntent!.rejectReasonExplanation!,
    );
  ...
  it("keeps review actions and revision editor outside the scroll region", () => {
    ...
    render(
      <AgentWorkBundleCard
        ...
        reviewLayout
        variant="embedded"
        shopLabel={{ text: "5号", sensitive: false }}
        onApprove={approve}
  ...
  it("does not expose decision actions for finished work", () => {
    render(
      <AgentWorkBundleCard
        proposal={{ ...rejection, status: GQL.ActionProposalStatus.Approved }}
        reviewLayout
        variant="embedded"
        shopLabel={{ text: "5号", sensitive: false }}
        onApprove={vi.fn()}
  ...
  it("shows Seller SKUs or product names beside stock without exposing platform IDs", () => {
    ...
    expect(rows.map((row) => row.textContent)).toEqual([
      "ROPE-GOLD-1821",
      "ROPE-GOLD-20370",
      "Holylegend full product name with size and material234",
    ]);
    expect(screen.queryByText(/platform-/)).toBeNull();
    expect(within(rows[0]).getByText("ROPE-GOLD-18").getAttribute("data-tk-private")).toBe("text");
    expect(
      within(rows[2])
        .getByText("Holylegend full product name with size and material")
        .getAttribute("data-tk-private"),
    ).toBe("text");
  ...
  it("shows missing SKU data without inventing a product-level quantity", () => {
    render(
      <AffiliateProposalStock
        items={[{ id: "missing", skuId: null, label: null, quantity: null }]}
      />,
    );
  ...
  it("keeps SKU subrows under each Sample without duplicating the Sample's decision", () => {
    ...
    for (const application of applications) {
      ...
      expect(
        within(application).getAllByText(
          i18n.t("ecommerce.affiliateWorkspace.sampleDecisionBundle.agentDecision"),
        ),
      ).toHaveLength(1);
    }
