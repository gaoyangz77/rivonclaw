...
const STACK_COLOR_VARS = Array.from({ length: 8 }, (_, index) => `var(--affiliate-stack-${index + 1})`);
...
function activeChartPayload(state: unknown): Record<string, unknown> | undefined {
  ...
  const activePayload = (state as { activePayload?: Array<{ payload?: Record<string, unknown> }> }).activePayload;
...
function ExploreKpi({ label, value, kind }: { label: string; value: number | null | undefined; kind: "number" | "money" | "rate" }) {
  const { i18n } = useTranslation();
  const formatted = kind === "money"
...
export function AffiliateExploreTab({ shops }: { shops: AffiliateAnalyticsShop[] }) {
  ...
  const activeDraft: AffiliateExplorerDraft = shopIds === draft.shopIds ? draft : { ...draft, shopIds };
  ...
  const dimensionLabel = (id: string) =>
    affiliateCatalogLabel(t, "dimensions", id, catalog?.dimensions.find((item) => item.id === id)?.label);
  ...
  const stackedChart = executed && chartMetric
    ? buildAffiliateStackedChartData(chartRows, executed.dimensions, chartMetric)
    : null;
  const resultChanged = executed != null && JSON.stringify(activeDraft) !== JSON.stringify(executed);
  ...
  const filterDimensionOptions = (catalog?.dimensions ?? [])
    ...
    .filter((dimension) => dimension.id === filterDimension
      || isAffiliateGroupingLegal(draft.dimensions, [...draft.filters.map((filter) => filter.dimension), dimension.id], groupingSets))
    ...
    <div className="affiliate-explore" data-tutorial-id="affiliate-analytics-explore">
      ...
      <section className="affiliate-query-composer">
        <div className="affiliate-composer-row">
          <AffiliateShopScopeControl shops={shops} selected={shopIds} onChange={(next) => setDraft((current) => ({ ...current, shopIds: next }))} />
          <label>
            <span>{t("ecommerce.affiliateAnalytics.startDate")}</span>
            <input type="date" value={draft.startDateGe} onChange={(event) => setDraft((current) => ({ ...current, startDateGe: event.target.value }))} />
          </label>
          <label>
            <span>{t("ecommerce.affiliateAnalytics.endDate")}</span>
            <input type="date" value={formatInputEndDate(draft.endDateLt)} onChange={(event) => setDraft((current) => ({ ...current, endDateLt: endDateLtFromInclusive(event.target.value) }))} />
          </label>
          <label>
            ...
            <Select
              ...
              options={GRANULARITIES.map((item) => ({ value: item, label: t(`ecommerce.affiliateAnalytics.granularities.${item}`) }))}
              onChange={(value) => setDraft((current) => ({ ...current, granularity: value as AffiliateGranularity }))}
            />
          </label>
        </div>
          ...
          <div className="affiliate-composer-block">
            ...
            <div className="affiliate-chip-list">
              {GROUP_PRESETS.filter((preset) => preset.dimensions.every(
                ...
                return (
                  <button type="button" key={preset.key} className={active ? "active" : ""} disabled={disabled} onClick={() => selectPreset(preset)}>
          ...
          <div className="affiliate-composer-block affiliate-filter-builder">
            ...
            <div className="affiliate-filter-fields">
              ...
              <input
                ...
                onKeyDown={(event) => { if (event.key === "Enter") void searchValues(); }}
              />
              <button type="button" className="btn btn-secondary" onClick={() => void searchValues()} disabled={valueState.loading}>
            ...
            <div className="affiliate-value-results">
              {valueState.data?.getEcommerceBiDimensionValues.items.map((item) => (
                <button type="button" key={item.value} onClick={() => addFilter(item.value, item.label)}>
            ...
            <div className="affiliate-manual-filter">
              <input value={manualFilterValue} placeholder={t("ecommerce.affiliateAnalytics.explore.exactValue")} onChange={(event) => setManualFilterValue(event.target.value)} />
              <button type="button" onClick={() => addFilter(manualFilterValue)}>{t("ecommerce.affiliateAnalytics.add")}</button>
            </div>
          </div>
        </div>
        ...
        <div className="affiliate-breadcrumbs">
          {draft.filters.map((filter) => (
            <button
              ...
              onClick={() => setDraft((current) => ({ ...current, filters: removeAffiliateFilter(current.filters, filter.dimension) }))}
            >
        ...
        <div className="affiliate-run-row">
          <label>
            ...
            <Select
              ...
              options={[
                ...
                ...[...draft.dimensions, ...draft.metrics].map((field) => ({ value: field, label: fieldLabel(field) })),
              ]}
          ...
          <label>
            ...
            <Select
              ...
              onChange={(value) => setDraft((current) => ({ ...current, sortDirection: value as "ASC" | "DESC" }))}
            />
          </label>
          <label>
          ...
          >
            {queryState.loading ? t("ecommerce.affiliateAnalytics.running") : t("ecommerce.affiliateAnalytics.run")}
          </button>
        </div>
      </section>
      ...
      {queryState.error && (
        <section className="affiliate-state is-error">
          ...
          <button className="btn btn-secondary" type="button" onClick={() => void execute(activeDraft)}>
      ...
      {result && executed && (
        <>
          <section className="affiliate-result-status" data-tutorial-id="affiliate-analytics-results">
            <div>
              <strong>{executed.datasetId === PLATFORM_DATASET ? t("ecommerce.affiliateAnalytics.platformTitle") : t("ecommerce.affiliateAnalytics.sampleTitle")}</strong>
              <span>{executed.startDateGe} → {formatInputEndDate(executed.endDateLt)}</span>
          ...
          <section className="affiliate-query-kpis">
            {executed.metrics.slice(0, 6).map((metric) => (
              <ExploreKpi
                ...
                kind={metric.includes("GMV") || metric.includes("COMMISSION")
                  ? "money"
                  : metric.includes("RATE") || metric.includes("COVERAGE") ? "rate" : "number"}
              />
            ))}
          </section>
          {executed.metrics.some((metric) => metric.endsWith("_NATIVE") && summary[metric] == null) && (
            <div className="affiliate-currency-warning">{t("ecommerce.affiliateAnalytics.explore.mixedCurrency")}</div>
          )}
          <section className="affiliate-panel affiliate-explore-chart">
            <header>
              ...
              <div className="affiliate-segmented">
                {CHART_MODES.map((mode) => (
                  <button key={mode} type="button" className={chartMode === mode ? "active" : ""} onClick={() => setChartMode(mode)}>
                    {t(`ecommerce.affiliateAnalytics.chartModes.${mode}`)}
                  </button>
                ))}
              </div>
            </header>
              ...
              <div className="affiliate-chart-large">
                <ResponsiveContainer width="100%" height="100%">
                  {resolvedChartMode === "LINE" ? (
                    <LineChart data={chartRows} onClick={(state) => drillFromPayload(activeChartPayload(state))}>
                      <CartesianGrid strokeDasharray="3 6" vertical={false} />
                      <XAxis dataKey={executed.dimensions.includes("DATE") ? "DATE" : categoryDimension} />
                      <YAxis tickFormatter={(value) => formatNumber(Number(value), i18n.language, true)} />
                      <Tooltip />
                      <Line type="monotone" dataKey={chartMetric} stroke={executed.datasetId === PLATFORM_DATASET ? "var(--affiliate-platform)" : "var(--affiliate-sample)"} strokeWidth={3} />
                    </LineChart>
                  ) : (
                    <BarChart
                      data={resolvedChartMode === "STACKED" && stackedChart ? stackedChart.rows : chartRows}
                      layout="vertical"
                      onClick={(state) => drillFromPayload(activeChartPayload(state))}
                    >
                      <CartesianGrid strokeDasharray="3 6" horizontal={false} />
                      <XAxis type="number" />
                      <YAxis type="category" dataKey={resolvedChartMode === "STACKED" && stackedChart ? "category" : categoryDimension} width={120} />
                      <Tooltip />
                      <Legend />
                      {resolvedChartMode === "STACKED" && stackedChart
                        ? stackedChart.series.map((series, index) => (
                          <Bar key={series.key} dataKey={series.key} name={series.label} stackId="affiliate" fill={STACK_COLOR_VARS[index % STACK_COLOR_VARS.length]} />
                        ))
                        : <Bar dataKey={chartMetric} fill={executed.datasetId === PLATFORM_DATASET ? "var(--affiliate-platform)" : "var(--affiliate-sample)"} />}
                    </BarChart>
                  )}
                </ResponsiveContainer>
              </div>
          ...
          <section className="affiliate-panel affiliate-result-table">
                    ...
                    <tr>
                      {displayedColumns.map((column) => (
                        <th key={column.key}>
                          <button
                            type="button"
                            title={column.metric && RATE_COMPONENTS[column.metric]
                            ...
                            onClick={() => setDraft((current) => ({
                              ...
                              sortDirection: current.sortField === column.key && current.sortDirection === "DESC" ? "ASC" : "DESC",
                            }))}
                          >
                            {column.dimension ? dimensionLabel(column.key) : metricLabel(column.key)}
                          </button>
                        </th>
                      ))}
                    </tr>
                      ...
                      <tr key={rowIndex}>
                        {displayedColumns.map((column) => (
                          <td key={column.key}>
                            {column.dimension && row[column.key] != null ? (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  const next = { ...executed, filters: upsertAffiliateFilter(executed.filters, { dimension: column.dimension!, operator: "IN", values: [value], labels: { [value]: value } }) };
                                  setDraft(next);
                                  void execute(next);
                                }}
                              >
                            ...
                            ) : metricDisplay(row[column.key] == null ? null : numberValue(row[column.key]), column.key, i18n.language)}
                          </td>
                        ))}
                      </tr>
                    ))}
            ...
            {result.pageInfo.hasMore && (
              <button className="btn btn-secondary affiliate-load-more" type="button" disabled={queryState.loading} onClick={() => void execute(executed, true)}>
