...
export function developerFormFrom(source: DeveloperFormSource): DeveloperForm {
  return {
    ...
    escalationChannelId: source.escalationChannelId ?? "",
    escalationRecipientId: source.escalationRecipientId ?? "",
  };
}

export function isDeveloperFormDirty(form: DeveloperForm, source: DeveloperFormSource): boolean {
  return (
    form.displayName.trim() !== source.displayName ||
    ...
    form.escalationChannelId.trim() !== (source.escalationChannelId?.trim() ?? "") ||
    form.escalationRecipientId.trim() !== (source.escalationRecipientId?.trim() ?? "")
  );
}
...
export function writeDeveloperInputFrom(
  ...
  return {
    ...
    escalationChannelId: form.escalationChannelId.trim() || null,
    escalationRecipientId: form.escalationRecipientId.trim() || null,
  };
}
...
export const AffiliateTeamPage = observer(function AffiliateTeamPage() {
  ...
  const [pendingAccountTransfer, setPendingAccountTransfer] =
    useState<PendingAccountTransfer | null>(null);
  ...
  const [protectionImportProgress, setProtectionImportProgress] =
    useState<ProtectionImportProgress | null>(null);
  const [protectionImportPhase, setProtectionImportPhase] = useState<ProtectionImportPhase>("IDLE");
  const [protectionImportOperationId, setProtectionImportOperationId] = useState<string | null>(
    null,
  );
  const [developerProvisionRevision, setDeveloperProvisionRevision] = useState(0);
  const [developerResolutionGroups, setDeveloperResolutionGroups] = useState<
    BusinessDeveloperResolutionGroup[]
  >([]);
  ...
  const [protectionResolution, setProtectionResolution] = useState<
    "" | GQL.AffiliateCreatorProtectionResolution
  >("");
  ...
  useEffect(() => {
    ...
    fetchChannelStatus(false)
      .then((snapshot) => {
        if (!cancelled) setEscalationChannelSnapshot(snapshot);
      })
      .catch(() => {
        if (!cancelled) setEscalationChannelSnapshot(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    const route = form.escalationChannelId.trim();
    const separator = route.indexOf(":");
    if (separator <= 0 || separator === route.length - 1) {
      setEscalationRecipientOptions([]);
      return;
    }
    let cancelled = false;
    fetchAllowlist(route.slice(0, separator), route.slice(separator + 1))
      .then((data) => {
        if (cancelled) return;
        setEscalationRecipientOptions(
          data.allowlist.map((recipientId) => ({
            value: recipientId,
            label: data.labels[recipientId]
              ? `${data.labels[recipientId]} (${recipientId})`
              : recipientId,
          })),
        );
      })
      .catch(() => {
        if (!cancelled) setEscalationRecipientOptions([]);
      });
    return () => {
      cancelled = true;
    };
  }, [form.escalationChannelId]);

  const escalationChannelOptions = useMemo(() => {
    const options = [
      { value: "", label: t("common.none") },
      ...buildAccountsList(entityStore.channelAccounts, escalationChannelSnapshot, t).map(
        ({ channelId, channelLabel, account }) => ({
    ...
    if (
      form.escalationChannelId &&
      !options.some((option) => option.value === form.escalationChannelId)
    ) {
      options.push({
        value: form.escalationChannelId,
        label: `${form.escalationChannelId} (${t("crons.channelDisconnected")})`,
      });
    }
    return options;
  }, [entityStore.channelAccounts, escalationChannelSnapshot, form.escalationChannelId, t]);
  const visibleEscalationRecipientOptions = useMemo(() => {
    const options = [...escalationRecipientOptions];
    if (
      form.escalationRecipientId &&
      !options.some((option) => option.value === form.escalationRecipientId)
    ) {
      options.push({ value: form.escalationRecipientId, label: form.escalationRecipientId });
    }
    return options;
  }, [escalationRecipientOptions, form.escalationRecipientId]);
  ...
  useEffect(() => {
    if (developersQuery.data)
      workspace.replaceAffiliateBusinessDevelopers(
        developersQuery.data.affiliateBusinessDevelopers,
      );
  }, [developersQuery.data, workspace]);
  useEffect(() => {
    if (settingsQuery.data)
      workspace.setAffiliateOperationalSettings(settingsQuery.data.affiliateOperationalSettings);
  }, [settingsQuery.data, workspace]);
  useEffect(
    () => setProtectionPage(0),
    [deferredProtectionSearch, protectionDeveloperId, protectionResolution],
  );
  useEffect(() => {
    if (whatsappQuery.data)
      workspace.replaceAffiliateWhatsAppAccounts(whatsappQuery.data.whatsAppAccountBindings);
  }, [whatsappQuery.data, workspace]);
  useEffect(() => {
    if (emailQuery.data)
      workspace.replaceAffiliateEmailAccounts(emailQuery.data.emailAccountBindings);
  ...
  const protectionImportBusy =
    importState.loading || ensureDevelopersState.loading || protectionImportProgress !== null;

  const activeDevelopers = workspace.businessDevelopers.filter(
    (developer) => !developer.archivedAt,
  );
  ...
  const detailHealthyChannelCount = [...detailWhatsapp, ...detailEmail].filter(
    (account) => account.status.toLowerCase() === "connected",
  ).length;
  const detailChannelContacts =
    channelContactsQuery.data?.affiliateCreatorChannelContacts.items ?? [];
  const detailRegionLabel = detailDeveloper
    ...
    : "";
  const unassignedWhatsapp = activeWhatsappAccounts.filter(
    (account) => !account.businessDeveloperId,
  );
  ...
  const protectedCreatorCount =
    (protectionData?.resolvedCount ?? 0) + (protectionData?.unresolvedCount ?? 0);
  const appliedProtectionCount = protectionData?.resolvedCount ?? 0;
  const protectionTotalPages = Math.max(
    1,
    Math.ceil((protectionData?.totalCount ?? 0) / PROTECTION_PAGE_SIZE),
  );
  ...
  const protectionPreviewCounts = protectionRows.reduce(
    (counts, row) => {
  ...
  const archiveBlocked = Boolean(
    ...
    detailSummary.creatorRelationshipCount +
      ...
      detailSummary.emailAccountCount >
      0,
  );
  ...
  const refreshChannelData = useCallback(async () => {
    ...
    setDetailSummary((current) => {
      ...
      return (
        pageResult.data?.affiliateBusinessDeveloperPage.items.find(
          (item) => item.developer.id === current.developer.id,
        ) ?? current
      );
    });
  }, [
    channelContactsQuery,
    detailSummary?.developer.id,
    developerPageQuery,
    emailQuery,
    whatsappQuery,
    workspace,
  ...
  async function handleSetPreferredAccount(channel: "WHATSAPP" | "EMAIL", accountId: string) {
    ...
    try {
      ...
      showToast(
        t("ecommerce.affiliateTeam.preferredAccountSaved", {
          defaultValue: "Preferred sender updated.",
        }),
        "success",
      );
    } catch (error) {
  ...
  function requestAccountOwnerChange(
    channel: "WHATSAPP" | "EMAIL",
    accountId: string,
    nextOwnerId: string,
  ) {
  ...
  async function changeAccountOwner(
    ...
    try {
      if (channel === "WHATSAPP") {
        if (nextOwner === UNASSIGNED_ID)
          await unassignWhatsapp({ variables: { accountBindingId: accountId } });
        else
          await assignWhatsapp({
            variables: { accountBindingId: accountId, businessDeveloperId: nextOwner },
          });
      } else {
        if (nextOwner === UNASSIGNED_ID)
          await unassignEmail({ variables: { accountBindingId: accountId } });
        else
          await assignEmail({
            variables: { accountBindingId: accountId, businessDeveloperId: nextOwner },
          });
  ...
  async function processProtectionFile(file: File) {
    ...
    try {
      ...
      if (!templateValidation.valid) {
        throw new Error(
          t("ecommerce.affiliateTeam.creatorUpdateTemplateUnsupported", {
      ...
      const groups: BusinessDeveloperResolutionGroup[] =
        buildAffiliateProtectionDeveloperResolutionSeeds(parsed, authoritativeDevelopers).map(
          (seed) => ({
            ...seed,
  ...
  function addManualProtection() {
    ...
    const duplicate = protectionRows.some(
      (row) => row.username?.toLowerCase() === identity.toLowerCase(),
    );
    if (duplicate) {
    ...
    const developer =
      manualDeveloperId === UNASSIGNED_ID
        ? null
        : workspace.getBusinessDeveloper(manualDeveloperId);
    setProtectionRows((rows) => [
      ...rows,
      {
  ...
  function updateDeveloperResolutionGroup(
    ...
    setDeveloperResolutionGroups((groups) =>
      groups.map((group) =>
        group.clientKey === clientKey ? { ...group, ...patch, error: patch.error ?? null } : group,
      ),
    );
  }

  async function confirmDeveloperResolutions() {
    const unresolved = developerResolutionGroups.some(
      (group) => !group.resolution || (group.resolution === "MAP" && !group.mappedDeveloperId),
    );
    ...
    const provisionTargets = new Map<
      string,
      {
        clientKey: string;
        displayName: string;
    ...
    try {
      ...
      const operationBaseKey =
        developerProvisionRevision === 0
      ...
      const provisionResultByKey = new Map(provisionResults.map((item) => [item.clientKey, item]));
      ...
      for (const target of targets) {
        ...
        if (
          !provisioned?.developer ||
          provisioned.disposition === GQL.AffiliateBusinessDeveloperProvisionDisposition.Rejected
        ) {
          const reason = provisioned?.errorMessage ?? t("ecommerce.updateFailed");
          target.groupKeys.forEach((groupKey) =>
            rejectedGroups.set(groupKey, {
              message: reason,
        ...
        setDeveloperResolutionGroups((groups) =>
          groups.map((group) => {
            ...
            return {
              ...group,
              archivedDeveloperId:
                rejection.errorCode === "ARCHIVED_CONFLICT"
      ...
      const resolvedRows = protectionRows.map((row): ProtectionPreviewRow => {
        ...
        if (group.resolution === "MAP") {
          const mapped = activeDevelopers.find(
            (developer) => developer.id === group.mappedDeveloperId,
          );
          return {
            ...row,
  ...
  async function submitProtectionRows() {
    const operationId =
      protectionImportOperationId ?? globalThis.crypto?.randomUUID?.() ?? `import-${Date.now()}`;
      ...
      for (const [batchIndex, batch] of batches.entries()) {
        ...
        try {
          ...
          if (controller.signal.aborted) {
            throw new Error(
              t("ecommerce.affiliateTeam.protectionImportBatchTimeout", {
                batch: batchIndex + 1,
        ...
        if (
          payload.results.length !== batch.entries.length ||
          resultByIndex.size !== batch.entries.length ||
          batch.entries.some((_, index) => !resultByIndex.has(index))
        ) {
          throw new Error(
            "Affiliate Creator update response did not contain exactly one result per row",
          );
        }
        ...
        for (let index = 0; index < batch.entries.length; index += 1) {
          ...
          if (
            item.status === GQL.AffiliateCreatorBulkUpdateStatus.Applied ||
            item.status === GQL.AffiliateCreatorBulkUpdateStatus.NoOp
          ) {
            completedRowNumbers.add(sourceRow.rowNumber);
          } else if (
            item.status === GQL.AffiliateCreatorBulkUpdateStatus.Rejected ||
  ...
  function requestRemovePersistedProtection(protection: GQL.AffiliateCreatorProtection) {
    const creator = protection.username
      ? `@${protection.username}`
  ...
  async function removePersistedProtection(protectionId: string) {
    try {
      ...
      showToast(
        t("ecommerce.affiliateTeam.protectionRemoved", { defaultValue: "Protection removed" }),
        "success",
      );
    } catch (error) {
      showToast(error instanceof Error ? error.message : t("ecommerce.updateFailed"), "error");
  ...
  function pendingConfirmationMessage(): string {
    switch (pendingConfirmation?.kind) {
      ...
      case "ARCHIVE_DEVELOPER":
        return t("ecommerce.affiliateTeam.archiveConfirm", {
          name: pendingConfirmation.displayName,
        });
      case "MOVE_ACCOUNT":
    ...
    try {
      ...
      if (result.data?.completeAffiliateOperationalOnboarding) {
        workspace.setAffiliateOperationalSettings(
          result.data.completeAffiliateOperationalOnboarding,
        );
      }
  ...
  const pageTabs: Array<{
    ...
    },
    {
      ...
      summary: t(
        onboardingComplete
          ? "ecommerce.affiliateTeam.setupReady"
          : "ecommerce.affiliateTeam.setupRequired",
      ),
  ...
  return (
    <AffiliatePageFrame className="affiliate-team-page">
        ...
        actions={
          <div className="affiliate-team-header-actions">
            ...
            {pageTab === "TEAM" && (
              ...
              >
                {t("ecommerce.affiliateWorkspace.whatsapp.proxyPoolButton", {
                  defaultValue: "Proxy pool",
                })}
              </button>
            )}
            {pageTab === "TEAM" && (
      ...
      >
        <section
          className="affiliate-team-overview"
          aria-label={t("ecommerce.affiliateTeam.operationsOverview")}
        >
          <div
            className={`affiliate-team-overview-status ${onboardingComplete ? "is-ready" : "needs-setup"}`}
          >
            <span className="affiliate-team-overview-icon">
          ...
          <div>
            <span>{t("ecommerce.affiliateTeam.humanDevelopers")}</span>
            <strong>{activeDevelopers.length}</strong>
          </div>
          <div>
        ...
        <section
          className="affiliate-team-responsibility"
          data-tutorial-id="affiliate-team-responsibilities"
        >
          <div className="affiliate-team-responsibility-head">
            <div>
              ...
              <p>
                {t("ecommerce.affiliateTeam.responsibilityHint", {
                  defaultValue: "Review each BD's workload and outreach accounts.",
                })}
              </p>
            </div>
            <span className="affiliate-team-owner-count">
              {t("ecommerce.affiliateTeam.ownerCount", { count: developerTotalCount })}
            </span>
          </div>
          <div className="affiliate-team-workspace">
            <div className="affiliate-bd-table-toolbar">
              <label className="affiliate-bd-search">
                <span>
                  {t("ecommerce.affiliateTeam.tableSearch", { defaultValue: "Search BDs" })}
                </span>
                <input
                  className="input"
            ...
            {developerSummaries.length > 0 ? (
                    ...
                    <thead>
                      <tr>
                        ...
                        <th className="is-numeric">
                          {t("ecommerce.affiliateTeam.managedCreators", {
                          ...
                          >
                            <td>
                              <div className="affiliate-bd-identity">
                                <span className="affiliate-bd-avatar">
                                  <UserIcon />
                                </span>
                                <span>
                                  <strong>
                                    ...
                                    {developer.profileStatus ===
                                      GQL.AffiliateBusinessDeveloperProfileStatus
                                        .NeedsConfiguration && (
                                  ...
                                  <small>
                                    {developer.regions.length > 0
                                      ? developer.regions
                                          ...
                                          .join(", ")
                                      : t("ecommerce.affiliateTeam.allRegions", {
                            ...
                            <td>
                              <span
                                className={`affiliate-bd-availability ${developer.acceptingCreators && !developer.archivedAt ? "is-accepting" : "is-paused"}`}
                              >
                                {developer.archivedAt
                                  ? t("ecommerce.affiliateTeam.archivedStatus", {
                                      defaultValue: "Archived",
                                    })
                                  : developer.acceptingCreators
                                    ? t("ecommerce.affiliateTeam.acceptingStatus", {
                                        defaultValue: "Accepting",
                                      })
                                    : t("ecommerce.affiliateTeam.pausedStatus", {
                                        defaultValue: "Paused",
                                      })}
                              </span>
                            </td>
                            <td className="is-numeric">
                              <strong>{summary.creatorRelationshipCount}</strong>
                            </td>
                            <td className="is-numeric">
                              <ChannelCount
                                total={summary.whatsappAccountCount}
                                unhealthy={summary.unhealthyWhatsappAccountCount}
                              />
                            </td>
                            <td className="is-numeric">
                              <ChannelCount
                                total={summary.emailAccountCount}
                                unhealthy={summary.unhealthyEmailAccountCount}
                              />
                            </td>
                            <td>
                              {developer.agentAssistanceMode ===
                ...
                <div className="affiliate-bd-pagination">
                  ...
                  <div>
                    <button
                      ...
                      title={t("ecommerce.affiliateTeam.previousPage", {
                        defaultValue: "Previous page",
                      })}
                      aria-label={t("ecommerce.affiliateTeam.previousPage", {
                        defaultValue: "Previous page",
                      })}
                    >
                      <ChevronRightIcon />
                    ...
                    <span>
                      {developerPage + 1} / {developerTotalPages}
                    </span>
                    <button
              ...
              <div className="affiliate-team-empty-developers">
                <span className="affiliate-team-empty-developers-icon">
                  <UserPlusIcon />
                </span>
                <h3>
                  {deferredDeveloperSearch
                    ? t("ecommerce.affiliateTeam.noSearchResults", {
                        defaultValue: "No matching business developers",
                      })
                    : t("ecommerce.affiliateTeam.emptyDevelopersTitle")}
                </h3>
                <p>
                  {deferredDeveloperSearch
                    ? t("ecommerce.affiliateTeam.noSearchResultsHint", {
                        defaultValue: "Try a different name or clear the archived filter.",
                      })
                    : t("ecommerce.affiliateTeam.emptyDevelopersHint")}
                </p>
                {!deferredDeveloperSearch && (
                  <button className="btn btn-primary" type="button" onClick={beginCreateDeveloper}>
                    <UserPlusIcon />
                    {t("ecommerce.affiliateTeam.addDeveloper")}
                  </button>
                )}
              </div>
            )}
      ...
      >
        <div
          className="affiliate-creator-management-toolbar"
          data-tutorial-id="affiliate-team-assignments"
        >
          <div>
        ...
        <section className="affiliate-protection-boundary">
          <div className="affiliate-protection-boundary-head">
            <div className="affiliate-protection-boundary-title">
              <span className="affiliate-protection-boundary-icon">
                <InfoIcon />
              </span>
              <div>
                ...
                <p>
                  {t(
                    onboardingComplete
                      ? "ecommerce.affiliateTeam.protectionBoundaryReady"
                      : "ecommerce.affiliateTeam.onboardingHint",
                  )}
                </p>
              </div>
            </div>
          ...
          {!onboardingComplete && (
            <div className="affiliate-onboarding-completion">
              <label className="affiliate-onboarding-confirm">
                <input
                  type="checkbox"
                  checked={confirmedProtectionBoundary}
                  onChange={(event) => setConfirmedProtectionBoundary(event.target.checked)}
                />
              ...
              <button
                ...
                disabled={!confirmedProtectionBoundary || onboardingState.loading}
              >
          ...
          <div className="affiliate-protection-directory">
            <div className="affiliate-protection-directory-head">
              <div>
                ...
                <span>
                  {t("ecommerce.affiliateTeam.protectionDirectorySummary", {
              ...
              <div className="affiliate-protection-directory-filters">
                <input
                  ...
                  placeholder={t("ecommerce.affiliateTeam.searchProtectedCreators", {
                    defaultValue: "Search username or OpenID",
                  })}
                />
                <Select
                ...
                <Select
                  ...
                  onChange={(value) =>
                    setProtectionResolution(value as "" | GQL.AffiliateCreatorProtectionResolution)
                  }
                  options={[
                    { value: "", label: t("common.all", { defaultValue: "All" }) },
                    {
                      value: GQL.AffiliateCreatorProtectionResolution.Resolved,
                      label: t("ecommerce.affiliateTeam.protectionResolved", {
                        defaultValue: "Matched Creator",
                      }),
                    },
                    {
                      value: GQL.AffiliateCreatorProtectionResolution.Unresolved,
                      label: t("ecommerce.affiliateTeam.protectionUnresolved", {
              ...
              {(protectionData?.businessDeveloperCounts ?? []).map((entry) => (
                <span key={entry.businessDeveloperId ?? UNASSIGNED_ID}>
                  {entry.businessDeveloperId
                    ? (workspace.getBusinessDeveloper(entry.businessDeveloperId)?.displayName ??
                      entry.businessDeveloperId)
                    : t("ecommerce.affiliateTeam.protectedOnly")}
                  : {entry.count}
                </span>
              ))}
            </div>
            ...
            {protectionTotalPages > 1 && (
              <div className="affiliate-pagination">
                <button
                  className="btn btn-secondary btn-sm"
                  type="button"
                  disabled={protectionPage === 0}
                  onClick={() => setProtectionPage((page) => Math.max(0, page - 1))}
                >
                  ‹
                </button>
                <span>
                  {protectionPage + 1} / {protectionTotalPages}
                </span>
                <button
                  className="btn btn-secondary btn-sm"
                  type="button"
                  disabled={protectionPage + 1 >= protectionTotalPages}
                  onClick={() => setProtectionPage((page) => page + 1)}
                >
                  ›
                </button>
              </div>
      ...
      >
        <section className="affiliate-team-policy-section is-open">
          ...
          <div className="affiliate-team-policy-content">
      ...
      >
        <div
          className="affiliate-protection-import-steps"
          aria-label={t("ecommerce.affiliateTeam.protectionImportSteps")}
        >
          {(
            [
            ...
            const currentIndex =
              protectionImportView === "ADD" ? 0 : protectionImportView === "RESOLVE" ? 1 : 2;
            ...
            {protectionComposerMode === "FILE" ? (
              <>
                <input
                  ...
                  accept=".csv,.xlsx,.xls"
                  hidden
                  onChange={handleProtectionFile}
                />
                <div className="affiliate-protection-template-card">
                  <span className="affiliate-protection-template-card-icon">
                    <DownloadIcon />
                  </span>
                  <div>
                ...
                >
                  <span className="affiliate-protection-dropzone-icon">
                    <DownloadIcon />
                  </span>
                  <div>
                    <h3>
                      {protectionImportPhase === "PARSING"
                        ? t("ecommerce.affiliateTeam.protectionImportParsing")
                        : t("ecommerce.affiliateTeam.protectionImportDropTitle")}
                    </h3>
              ...
              <div className="affiliate-protection-manual-card">
                ...
                <div className="affiliate-protection-manual">
                  <label>
                    ...
                    <input
                      value={manualCreator}
                      onChange={(event) => setManualCreator(event.target.value)}
                      placeholder={t("ecommerce.affiliateTeam.creatorIdentityPlaceholder")}
                    />
                  </label>
                  <label>
                    <span>{t("ecommerce.affiliateTeam.assignProtectionDeveloper")}</span>
                    <Select
                      value={manualDeveloperId}
                      options={protectionOwnerOptions}
                      onChange={setManualDeveloperId}
                    />
                  </label>
                  <label>
                    <span>{t("ecommerce.affiliateTeam.note")}</span>
                    <input
                      ...
                      placeholder={t("ecommerce.affiliateTeam.notePlaceholder")}
                    />
                  </label>
                </div>
        ...
        {protectionImportView === "RESOLVE" && (
          <div className="affiliate-protection-import-body is-resolve">
            <div className="affiliate-protection-developer-intro">
              <span className="affiliate-protection-developer-intro-icon">
                <UserPlusIcon />
              </span>
              <div>
                <strong>
                  {developerResolutionGroups.length > 0
                    ...
                    : t("ecommerce.affiliateTeam.confirmMatchedDevelopersTitle")}
                </strong>
                <p>
                  {developerResolutionGroups.length > 0
                    ? t("ecommerce.affiliateTeam.resolveDeveloperHint")
                    : t("ecommerce.affiliateTeam.confirmMatchedDevelopersHint")}
                </p>
              </div>
            </div>
            ...
            <div className="affiliate-protection-confirmation-summary">
              ...
              <div
                className={protectionAssignmentSummary.attentionRowCount > 0 ? "has-attention" : ""}
              >
            ...
            {protectionAssignmentSummary.assigned.length > 0 && (
              <section className="affiliate-protection-matched-developers">
                <div className="affiliate-protection-matched-developers-head">
                  ...
                  <span>
                    {t("ecommerce.affiliateTeam.matchedDeveloperCount", {
                ...
                <div className="affiliate-protection-matched-developer-list">
                  {protectionAssignmentSummary.assigned.map((assignment) => (
                    <div key={assignment.businessDeveloperId}>
                      ...
                      <span>
                        {t("ecommerce.affiliateTeam.affectedCreatorRows", {
            ...
            {developerResolutionGroups.length > 0 && (
              <div className="affiliate-protection-developer-list">
                {developerResolutionGroups.map((group) => {
                  const mappingOptions = activeDevelopers.filter(
                    (developer) =>
                      !group.mapSearch ||
                      developer.displayName
                        .toLocaleLowerCase()
                        .includes(group.mapSearch.toLocaleLowerCase()),
                  );
                  return (
                    <section
                    ...
                    >
                        ...
                        <div>
                          <span>
                            {group.archivedDeveloperId
                              ...
                              : t("ecommerce.affiliateTeam.missingDeveloperFound")}
                          </span>
                      ...
                      <label className="affiliate-protection-developer-action">
                        ...
                        >
                          <option value="">
                            {t("ecommerce.affiliateTeam.chooseImportDecision")}
                          </option>
                          {!group.archivedDeveloperId && (
                            <option value="CREATE">
                              {t("ecommerce.affiliateTeam.createDeveloper")}
                            </option>
                          )}
                          {group.archivedDeveloperId && (
                            <option value="RESTORE">
                              {t("ecommerce.affiliateTeam.restoreDeveloper")}
                            </option>
                          )}
                          <option value="MAP">{t("ecommerce.affiliateTeam.mapDeveloper")}</option>
                          <option value="UNASSIGNED">
                            {t("ecommerce.affiliateTeam.importProtectionOnly")}
                          </option>
                          <option value="EXCLUDE">
                            {t("ecommerce.affiliateTeam.excludeAffectedRows")}
                          </option>
                        </select>
                      </label>
                      ...
                      {group.resolution === "MAP" && (
                        <div className="affiliate-protection-developer-map">
                          ...
                          >
                            <option value="">
                              {t("ecommerce.affiliateTeam.selectExistingDeveloper")}
                            </option>
                      ...
                      {group.error && (
                        <p className="affiliate-protection-developer-error">{group.error}</p>
                      )}
                    </section>
                  );
                })}
              ...
              <div>
                <strong>
                  {developerResolutionGroups.length > 0
                    ? t("ecommerce.affiliateTeam.importDecisionSummary", {
                        create: developerResolutionGroups.filter(
                          (group) => group.resolution === "CREATE",
                        ).length,
                        restore: developerResolutionGroups.filter(
                          (group) => group.resolution === "RESTORE",
                        ).length,
                        unassigned: developerResolutionGroups.filter(
                          (group) => group.resolution === "UNASSIGNED",
                        ).length,
                      })
                ...
                <span>
                  {developerResolutionGroups.length > 0
                    ...
                    : t("ecommerce.affiliateTeam.confirmationRequiredBeforeImport")}
                </span>
              </div>
              <div>
                <button
                ...
                <button
                  ...
                  disabled={
                    ...
                    developerResolutionGroups.some(
                      (group) =>
                        !group.resolution ||
          ...
          <div className="affiliate-protection-import-body is-preview">
            <div className="affiliate-protection-preview-toolbar">
              <div>
                ...
                <span>
                  {protectionImportFileName ||
                    t("ecommerce.affiliateTeam.protectionImportManualSource")}
                </span>
              </div>
              <button
                className="btn btn-secondary btn-sm"
            ...
            <div className="affiliate-protection-preview-summary">
              <div>
                <span>{t("ecommerce.affiliateTeam.protectionPreviewTotal")}</span>
                <strong>{protectionRows.length}</strong>
              </div>
              <div>
                <span>{t("ecommerce.affiliateTeam.protectionPreviewAssigned")}</span>
                <strong>{protectionPreviewCounts.ASSIGNED}</strong>
              </div>
              <div>
                <span>{t("ecommerce.affiliateTeam.protectionPreviewOnly")}</span>
                <strong>{protectionPreviewCounts.PROTECTION_ONLY}</strong>
              </div>
              <div
                className={
                  protectionPreviewCounts.ERROR +
                    protectionPreviewCounts.EXCLUDED +
                    protectionPreviewCounts.NEEDS_DEVELOPER_DECISION >
                  0
                    ? "has-attention"
                    : ""
                }
            ...
            <div className="affiliate-protection-preview-footer">
              <span>
                {t("ecommerce.affiliateTeam.protectionPreviewRange", {
                  from:
                    protectionRows.length === 0
                      ? 0
                      : protectionPreviewPage * PROTECTION_PREVIEW_PAGE_SIZE + 1,
                  to: Math.min(
                    (protectionPreviewPage + 1) * PROTECTION_PREVIEW_PAGE_SIZE,
              ...
              <div className="affiliate-pagination">
                <button
                  className="btn btn-secondary btn-sm"
                  type="button"
                  disabled={protectionPreviewPage === 0}
                  onClick={() => setProtectionPreviewPage((page) => Math.max(0, page - 1))}
                >
                  ‹
                </button>
                <span>
                  {protectionPreviewPage + 1} / {protectionPreviewTotalPages}
                </span>
                <button
                  className="btn btn-secondary btn-sm"
                  type="button"
                  disabled={protectionPreviewPage + 1 >= protectionPreviewTotalPages}
                  onClick={() => setProtectionPreviewPage((page) => page + 1)}
                >
                  ›
                </button>
              </div>
            ...
            {protectionImportProgress && (
              <div className="affiliate-protection-import-live-progress" aria-live="polite">
                <div>
                  <strong>
                    {t(
                      "ecommerce.affiliateTeam.protectionImportBatchProgress",
                      protectionImportProgress,
                    )}
                  </strong>
                  <span>
            ...
            <div className="affiliate-protection-import-footer">
              ...
              <button
                ...
                disabled={
                  protectionImportBusy || protectionRows.every((row) => row.error || row.excluded)
                }
              >
                {protectionImportProgress
                  ? t(
          ...
          <>
              ...
              <div className="affiliate-bd-command-identity">
                ...
                <div>
                  <div className="affiliate-bd-command-chips">
                    <span
                      className={`affiliate-bd-availability ${detailDeveloper.acceptingCreators && !detailDeveloper.archivedAt ? "is-accepting" : "is-paused"}`}
                    >
                    ...
                    <span>
                      {detailDeveloper.agentAssistanceMode ===
                      GQL.AffiliateAgentAssistanceMode.HumanOnly
                        ? t("ecommerce.affiliateTeam.humanOnly")
                        : t("ecommerce.affiliateTeam.aiAssisted")}
                    </span>
              ...
              {detailTab === "CHANNELS" && (
                ...
                >
                  <div className="affiliate-bd-command-section-head">
                    ...
                    <span>
                      {detailSummary.whatsappAccountCount + detailSummary.emailAccountCount}
                    </span>
                  </div>
                  <div className="affiliate-bd-channel-grid">
                    ...
                    <ChannelWorkspaceCard
                      ...
                      onConnect={() =>
                        setConnectChannel((value) => (value === "EMAIL" ? null : "EMAIL"))
                      }
                      t={t}
                    />
                  </div>
                  <section className="affiliate-bd-escalation-routing">
                    <div className="affiliate-bd-command-section-head">
                      <div className="affiliate-bd-section-title">
                        <span className="affiliate-bd-section-icon">
                          <InfoIcon />
                        ...
                        <div>
                          <strong>
                            {t("ecommerce.affiliateWorkspace.escalations.routingTitle")}
                          </strong>
                          <span>{t("ecommerce.affiliateWorkspace.escalations.routingHint")}</span>
                        </div>
                      </div>
                    </div>
                    <div className="affiliate-bd-escalation-routing-fields">
                      <label>
                        <span>{t("ecommerce.affiliateWorkspace.escalations.routingChannel")}</span>
                        <Select
                          value={form.escalationChannelId}
                      ...
                      <label>
                        <span>
                          {t("ecommerce.affiliateWorkspace.escalations.routingRecipient")}
                        </span>
                        <Select
                          value={form.escalationRecipientId}
                          options={visibleEscalationRecipientOptions}
                          onChange={(value) => setForm({ ...form, escalationRecipientId: value })}
                          disabled={!form.escalationChannelId}
                        />
                      </label>
                      <button
                        className="btn btn-primary"
                        ...
                        disabled={
                          writeState.loading ||
                          Boolean(form.escalationChannelId && !form.escalationRecipientId)
                        }
                      >
                        {writeState.loading ? t("common.loading") : t("common.save")}
                      </button>
                    </div>
                  </section>
                  <div className="affiliate-bd-contact-ledger">
                    <div className="affiliate-bd-command-section-head">
                      <div className="affiliate-bd-section-title">
                        ...
                        <div>
                          <strong>
                            {t("ecommerce.affiliateTeam.creatorContacts", {
                              defaultValue: "Creator contacts on these accounts",
                            })}
                          </strong>
                          <span>
                            {t("ecommerce.affiliateTeam.creatorContactsHint", {
                              defaultValue:
                                "Each row is one concrete sender account → Creator contact route.",
                            })}
                          </span>
                        </div>
                      </div>
                    ...
                    {channelContactsQuery.loading && !channelContactsQuery.data ? (
                      ...
                      <div className="affiliate-bd-contact-list">
                        {detailChannelContacts.map((contact) => {
                          const account =
                            contact.channel === GQL.AffiliateMessageChannel.Whatsapp
                          ...
                          const address =
                            ...
                            t("ecommerce.affiliateTeam.unknownContact", {
                              defaultValue: "Provider identity",
                            });
                          return (
                            <div className="affiliate-bd-contact-row" key={contact.id}>
                              <span
                                className={`affiliate-bd-contact-channel is-${contact.channel.toLowerCase()}`}
                              >
                                {contact.channel === GQL.AffiliateMessageChannel.Whatsapp
                                  ? "WhatsApp"
                                  : "Outlook"}
                              ...
                              <div>
                                <strong>
                                  {account?.displayName ||
                                    accountAddress ||
                                    t("ecommerce.affiliateTeam.unknownAccount", {
                                ...
                                <small>
                                  {accountAddress ||
                                    t("ecommerce.affiliateTeam.unknownAccount", {
                                      defaultValue: "Unavailable sender",
                                    })}
                                </small>
                              </div>
                              <span
                                className={`affiliate-channel-health ${contact.status.toLowerCase()}`}
                              >
                                {contact.status}
                              </span>
                            </div>
                      ...
                      <div className="affiliate-bd-channel-card-empty">
                        {t("ecommerce.affiliateTeam.noCreatorContacts", {
                          defaultValue:
                            "No Creator contact has been added on this BD's accounts yet.",
                        })}
                      </div>
                    )}
                  </div>
                ...
                >
                  <div className="affiliate-bd-command-section-head">
                    <div className="affiliate-bd-section-title">
                      ...
                      <div>
                        <strong>
                          {t("ecommerce.affiliateTeam.detailOverview", {
            ...
            {detailTab === "SETTINGS" && (
              <div className="affiliate-bd-detail-footer">
                {archiveBlocked && (
                  <span className="affiliate-bd-archive-note">
                    <InfoIcon />
                    {t("ecommerce.affiliateTeam.archiveBlockedHint", {
                      defaultValue:
                        "Move all creators and outreach accounts before archiving this BD.",
                    })}
                  </span>
                )}
                <div>
                  {!detailDeveloper.archivedAt && (
                    <button
                      ...
                      type="button"
                      onClick={handleArchiveDeveloper}
                      disabled={archiveState.loading || archiveBlocked}
                    >
                      {t("ecommerce.affiliateTeam.archive")}
                    </button>
                  )}
                  {!detailDeveloper.archivedAt && (
                    <button
        ...
        {pendingAccountTransfer && (
          <>
            <div className="affiliate-account-transfer-content">
              <div
                className={`affiliate-account-transfer-summary is-${pendingAccountTransfer.channel.toLowerCase()}`}
              >
                ...
                <div>
                  <small>
                    {pendingAccountTransfer.channel === "WHATSAPP" ? "WhatsApp" : "Outlook"}
                  </small>
                  <strong>
                    {pendingAccountTransfer.displayName ||
                      pendingAccountTransfer.phoneNumber ||
                      pendingAccountTransfer.emailAddress ||
                      t("ecommerce.affiliateTeam.unnamedAccount")}
                  </strong>
                  {(pendingAccountTransfer.phoneNumber || pendingAccountTransfer.emailAddress) &&
                    pendingAccountTransfer.displayName && (
                      <span>
                        {pendingAccountTransfer.phoneNumber || pendingAccountTransfer.emailAddress}
                      </span>
              ...
              <p className="affiliate-account-transfer-warning">
                <InfoIcon />
                {t("ecommerce.affiliateTeam.transferConfirm")}
              </p>
            </div>
            <div className="affiliate-account-transfer-footer">
              ...
              <button
                className="btn btn-primary"
                type="button"
                onClick={confirmAccountTransfer}
      ...
      <Modal
        ...
        portal
      >
        {detailDeveloper && connectChannel === "WHATSAPP" && (
        ...
        {detailDeveloper && connectChannel === "EMAIL" && (
      ...
      <Modal
        ...
        title={t("ecommerce.affiliateTeam.unassignedAccountsTitle", {
          defaultValue: "Unassigned outreach accounts",
        })}
        maxWidth={920}
      ...
      >
        <div className="affiliate-unassigned-body">
          <p>
            {t("ecommerce.affiliateTeam.unassignedAccountsHint", {
              defaultValue:
                "Assign each account to the BD who will use it for creator communication.",
            })}
          </p>
          <ChannelAccountRows
            channel="WHATSAPP"
            accounts={unassignedWhatsapp}
            ownerOptions={ownerOptions}
            onOwnerChange={requestAccountOwnerChange}
            t={t}
          />
          <ChannelAccountRows
            channel="EMAIL"
            accounts={unassignedEmail}
            ownerOptions={ownerOptions}
            onOwnerChange={requestAccountOwnerChange}
            t={t}
          />
          {unassignedChannelCount === 0 && (
            <div className="affiliate-channel-empty">
              {t("ecommerce.affiliateTeam.noUnassignedAccounts", {
                defaultValue: "All outreach accounts are assigned.",
              })}
            </div>
          )}
        </div>
...
export function DeveloperEditor({
  ...
  return (
    <div className="affiliate-developer-editor">
      <label>
        <span>{t("ecommerce.affiliateTeam.internalName")}</span>
        <input
          className="input"
          value={form.displayName}
          onChange={(event) => setForm({ ...form, displayName: event.target.value })}
        />
        <small className="form-hint">{t("ecommerce.affiliateTeam.internalNameHint")}</small>
      </label>
      <label>
        <span>{t("ecommerce.affiliateTeam.creatorDisplayName")}</span>
        <input
          className="input"
          value={form.creatorDisplayName}
          onChange={(event) => setForm({ ...form, creatorDisplayName: event.target.value })}
          placeholder={t("ecommerce.affiliateTeam.creatorDisplayNamePlaceholder")}
        />
        <small className="form-hint">{t("ecommerce.affiliateTeam.creatorDisplayNameHint")}</small>
      </label>
      <label>
        <span>{t("ecommerce.affiliateTeam.workMode")}</span>
        <Select
          value={form.agentAssistanceMode}
          onChange={(value) =>
            setForm({ ...form, agentAssistanceMode: value as GQL.AffiliateAgentAssistanceMode })
          }
          options={[
            {
              value: GQL.AffiliateAgentAssistanceMode.AiAssisted,
              label: t("ecommerce.affiliateTeam.aiAssisted"),
            },
            {
              value: GQL.AffiliateAgentAssistanceMode.HumanOnly,
              label: t("ecommerce.affiliateTeam.humanOnly"),
            },
          ]}
        />
      </label>
      ...
      <fieldset>
        <legend>{t("ecommerce.affiliateTeam.regions")}</legend>
        <div className="affiliate-region-grid">
          {SHOP_REGIONS.map((region) => (
            <label key={region}>
              ...
              />
              <span>{formatShopRegionLabel(region, t)}</span>
            </label>
          ))}
        </div>
      </fieldset>
      <label className="affiliate-developer-toggle">
        <input
          type="checkbox"
          checked={form.acceptingCreators}
          onChange={(event) => setForm({ ...form, acceptingCreators: event.target.checked })}
        />
        <span>{t("ecommerce.affiliateTeam.acceptingCreators")}</span>
      </label>
      <label>
        <span>{t("ecommerce.affiliateTeam.workingStyle")}</span>
        <textarea
          ...
          onChange={(event) => setForm({ ...form, businessPrompt: event.target.value })}
          placeholder={t("ecommerce.affiliateTeam.workingStylePlaceholder")}
        />
        <small className="affiliate-bd-prompt-hint">
          <InfoIcon />
          {t("ecommerce.affiliateTeam.workingStylePlaceholder")}
        </small>
      </label>
      <div className="affiliate-developer-editor-actions">
...
function DeveloperProfileEditor({
  ...
  return (
    <div className="affiliate-bd-inline-editor">
      <label className="affiliate-bd-inline-name">
        <span>{t("ecommerce.affiliateTeam.internalName")}</span>
        <input
          className="input"
          value={form.displayName}
      ...
      <DeveloperDeviceBinding form={form} setForm={setForm} myDeviceId={myDeviceId} t={t} />
      <label className="affiliate-bd-inline-toggle">
        <input
          type="checkbox"
          checked={form.acceptingCreators}
          onChange={(event) => setForm({ ...form, acceptingCreators: event.target.checked })}
      ...
      <label className="affiliate-bd-inline-prompt">
        <span>{t("ecommerce.affiliateTeam.workingStyle")}</span>
        <textarea
          className="input"
          rows={5}
 ...
 */
function DeveloperDeviceBinding({
  form,
  setForm,
  myDeviceId,
  t,
...
function ChannelCount({ total, unhealthy }: { total: number; unhealthy: number }) {
  return (
    <span className={`affiliate-bd-channel-count ${unhealthy > 0 ? "has-warning" : ""}`}>
      <strong>{total}</strong>
  ...
  return (
    <>
      {accounts.map((account) => {
        ...
        const statusLabel =
          channel === "WHATSAPP"
            ? t(`ecommerce.affiliateWorkspace.whatsapp.status.${account.status}`, {
                defaultValue: account.status,
              })
            : t(`ecommerce.affiliateWorkspace.email.status.${account.status.toLowerCase()}`, {
                defaultValue: account.status,
              });
        const preferred = preferredAccountId === account.id;
        return (
          <div className="affiliate-channel-row" key={account.id}>
            <span className="affiliate-channel-kind">
              {channel === "WHATSAPP" ? "WhatsApp" : "Outlook"}
            </span>
            <div className="affiliate-channel-account-identity">
              <span className="affiliate-channel-account-avatar">
                <UserIcon />
              </span>
              <div>
                <strong>
                  {account.displayName || address || t("ecommerce.affiliateTeam.unnamedAccount")}
                </strong>
                {address && account.displayName && <small>{address}</small>}
                {account.lastError && (
                  <small className="affiliate-channel-error-reason">{account.lastError}</small>
                )}
              </div>
            </div>
            <span className={`affiliate-channel-health ${account.status.toLowerCase()}`}>
              {statusLabel}
            </span>
            {onSetPreferred && (
              <button
            ...
            {(onTransfer ||
              (channel === "WHATSAPP" &&
                account.status !== GQL.WhatsAppAccountStatus.Connected &&
              ...
              <div className="affiliate-channel-row-actions">
                {channel === "WHATSAPP" &&
                  ...
                  onReconnect && (
                    <button
                      className="btn btn-primary btn-sm affiliate-channel-reconnect"
                      type="button"
                ...
                {onTransfer && (
                  <button
                    className="affiliate-channel-transfer-button"
                    type="button"
                    onClick={() => onTransfer(channel, account)}
                  >
                    {t("ecommerce.affiliateTeam.transferAccount")}
                  </button>
                )}
            ...
            {ownerOptions && onOwnerChange && (
              <label className="affiliate-channel-owner-field">
                <span>{t("ecommerce.affiliateTeam.assignDeveloper")}</span>
                <Select
                  value={account.businessDeveloperId ?? UNASSIGNED_ID}
                  onChange={(value) => onOwnerChange(channel, account.id, value)}
...
function ChannelWorkspaceCard({
  ...
  return (
    <section
      className={`affiliate-bd-channel-card is-${channel.toLowerCase()} ${connecting ? "is-expanded" : ""}`}
    >
      <header className="affiliate-bd-channel-card-head">
        <div className="affiliate-bd-channel-brand">
          ...
          <div>
            ...
            <small>
              {channel === "WHATSAPP"
                ? t("ecommerce.affiliateTeam.whatsappAccounts")
                : t("ecommerce.affiliateTeam.emailAccounts")}
            </small>
          </div>
        </div>
        <div className="affiliate-bd-channel-card-actions">
          <span
            className={`affiliate-bd-channel-signal ${healthyCount === accounts.length && accounts.length > 0 ? "is-healthy" : ""}`}
          >
            <i aria-hidden="true" />
          ...
          {canConnect && (
            <button
              className={`btn btn-secondary btn-sm affiliate-channel-connect-button ${connecting ? "is-expanded" : ""}`}
              type="button"
              onClick={onConnect}
      ...
      <div className="affiliate-bd-channel-card-body">
        {accounts.length > 0 ? (
          ...
          <div className="affiliate-bd-channel-card-empty">
            {t("ecommerce.affiliateTeam.noChannels")}
          </div>
        )}
      </div>
    </section>
