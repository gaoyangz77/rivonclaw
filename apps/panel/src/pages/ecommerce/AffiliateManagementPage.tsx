...
} from "./components/CreatorDetailScopeControl.js";
import { useCreatorScopeConversation } from "./components/useCreatorScopeConversation.js";
import {
  matchesCreatorShop,
...
function mergeById<T>(
  items: T[],
  identity: (item: T) => string | null | undefined = (item) => (item as { id?: string | null }).id,
): T[] {
...
export function hydrateAffiliateProposalProjection(
  projection: {
    proposal: unknown;
  ...
  authoritativeProposal?: GQL.ActionProposal,
): GQL.ActionProposal {
  ...
  return {
    ...
    creatorProfile: affiliateSnapshot(
      projection.creatorProfile ?? (proposal as any).creatorProfile,
    ),
    productSummary: affiliateSnapshot(
      projection.productSummary ?? (proposal as any).productSummary,
    ),
...
type AffiliateWorkspaceStore = {
  ...
  upsertAffiliateCreatorRelationship?: (
    relationship: GQL.AffiliateCreatorRelationship | null | undefined,
  ) => void;
  upsertAffiliateCreatorProfile?: (
    profile: GQL.AffiliateCreatorIdentity | null | undefined,
  ) => void;
  upsertAffiliateProductSummary?: (product: GQL.EcomProductSummary | null | undefined) => void;
  upsertAffiliateSampleApplicationRecord?: (
    sample: GQL.SampleApplicationRecord | null | undefined,
  ) => void;
...
function productContextFromProjection(
...
export function affiliateProposalPageCursor(
  stack: AffiliateProposalPageCursorStack,
): string | null {
...
export function sortAffiliateProposalsNewestFirst(
  ...
  return [...proposals].sort((left, right) => {
    const createdDifference =
      proposalTimestamp(right.createdAt) - proposalTimestamp(left.createdAt);
...
export function groupAgentWorkBundles(proposals: GQL.ActionProposal[]): AgentWorkBundle[] {
  ...
  for (const proposal of proposals) {
    ...
    const current =
      !existing || proposal.revisionNumber > existing.proposal.revisionNumber
        ? proposal
        : existing.proposal;
    bundles.set(rootProposalId, {
      rootProposalId,
      proposal: current,
      revisionHistory: [...historyById.values()].sort(
        (left, right) =>
...
export function affiliateModelStagePresentation(
  ...
  const entry =
    availability.find(
      (candidate) => candidate.modelFamily === family && candidate.modelStage === stage,
    ) ?? null;
  const ready =
    (entry?.status === "READY" || entry?.status === "FALLBACK") && entry.contractStatus === "MATCH";
...
export const AffiliateIntelligencePage = observer(function AffiliateIntelligencePage() {
  ...
  const shops = entityStore.shops.filter(
    (shop) => shop.services?.affiliateService?.enabled === true,
  );
  ...
  const insightRows = useMemo<AffiliateInsightRow[]>(() => {
    ...
    for (const subject of insightSubjects) {
      const scopes: AffiliateInsightModelScope[] =
        subject.kind === "user" ? ["user"] : ["user", "region", "shop"];
      for (const modelScope of scopes) {
        ...
        rows.push({
          ...
          modelScope,
          availability: (cached?.availability ?? []) as AffiliateModelAvailabilityView[],
          automaticSelection: (cached?.automaticSelection ??
            null) as GQL.AffiliateExpectedSalesAutomaticSelection | null,
  ...
  useEffect(() => {
    if (
      ...
      !insightSubjects.some((subject) => subject.key === selectedScopeKey)
    ) {
  ...
  return (
    <AffiliatePageFrame className="affiliate-workbench affiliate-intelligence-page">
      <AffiliatePageHeader
        className="affiliate-intelligence-hero"
        data-tutorial-id="affiliate-intelligence-header"
        eyebrow={t("ecommerce.affiliateWorkspace.intelligenceKicker")}
        title={t("ecommerce.affiliateWorkspace.mlInsightsTitle")}
        subtitle={t("ecommerce.affiliateWorkspace.mlInsightsHint")}
        actions={
          <AffiliateToolbar className="affiliate-intelligence-controls">
            <button
...
export const AffiliateWorkbenchPage = observer(function AffiliateWorkbenchPage() {
  ...
  const [selectedEntityTarget, setSelectedEntityTarget] =
    useState<AffiliateWorkbenchEntityOpenTarget | null>(null);
  const [workbenchEntityRefreshRevision, setWorkbenchEntityRefreshRevision] = useState(0);
  ...
  const activeProposalPageBuffer =
    proposalPageBuffer.queryKey === proposalQueryKey
      ? proposalPageBuffer
      : emptyAffiliateProposalPageBuffer(proposalQueryKey);
  ...
  } = useQuery<AffiliateActionProposalPageData, { input: ReadAffiliateActionProposalPageInput }>(
    AFFILIATE_ACTION_PROPOSALS_QUERY,
    {
      variables: {
        input: {
  ...
  useEffect(() => {
    ...
    setEscalationOffset(
      Math.max(
        0,
        Math.floor(Math.max(0, totalCount - 1) / AFFILIATE_ESCALATION_PAGE_SIZE) *
          AFFILIATE_ESCALATION_PAGE_SIZE,
      ),
    );
  ...
  const replaceProposalPage = useCallback(
    (queryKey: string, page: AffiliateActionProposalPageData["affiliateActionProposalPage"]) => {
  ...
  const visibleProposalItems = sortAffiliateProposalsNewestFirst(
    filterActionProposals(
      proposalItemsFromQuery.filter((proposal) => !proposalType || proposal.type === proposalType),
      attentionSearch,
      shopLabel,
    ),
  );
  ...
  async function decideProposal(
    ...
    status: GQL.ActionProposalStatus,
    note?: string,
    sampleReviewOverride?: GQL.ActionProposalSampleReviewOverrideInput,
  ): Promise<boolean> {
    ...
    try {
      const creatorRelationshipId =
        proposal.creatorRelationshipId ?? proposal.sourceWorkBoundary?.creatorRelationshipId;
      ...
      setProposalPageBuffer((current) =>
        current.queryKey !== proposalQueryKey
          ...
          : {
              ...current,
              items: applyAffiliateProposalChange(
                current.items,
                optimisticProposal,
                decisionFilters,
              ),
      ...
      if (updatedProposal) {
        ...
        setSelectedAgentWorkBundle((current) => {
          ...
          const updatedRootProposalId =
            updatedProposal.revisionRootProposalId || updatedProposal.id;
    ...
    <AffiliatePageFrame className="affiliate-workbench tk-v1-workbench">
      ...
      <div className="affiliate-workbench-panel">
          ...
          <div className="affiliate-workbench-panel-head affiliate-agent-workspace-controls">
            ...
            >
              ...
              {workbenchTab === "ESCALATIONS" ? null : (
                <TkChoiceSelect
                  label={t("ecommerce.affiliateWorkspace.typeFilter")}
              ...
              <TkField
                label={t("ecommerce.affiliateWorkspace.searchFilter")}
                className="affiliate-filter-field-search affiliate-attention-search-field"
            ...
            <TkButton
              ...
              loading={workbenchTab === "ESCALATIONS" ? escalationsLoading : proposalsLoading}
            >
        ...
        <div className="affiliate-attention-active-list">
          {workbenchTab === "SAMPLES" || workbenchTab === "MESSAGES" ? (
            ...
            <div className="affiliate-proposal-empty" data-tutorial-id="affiliate-attention-queue">
                ...
                {loadedProposals.length > 0 ? (
                  <span>
                    {t("ecommerce.affiliateWorkspace.proposalPagination.pageRange", {
                      start:
                        activeProposalPageStack.pageIndex * activeProposalPageStack.pageSize + 1,
                      end:
      ...
      {selectedEntityTarget ? (
        <CreatorRelationshipDetailModal
          ...
          onClose={() => setSelectedEntityTarget(null)}
        />
      ) : null}
    </AffiliatePageFrame>
  );
...
function AffiliateMlInsightsPanel({
  ...
  const selectedSubject =
    ...
    subjects.find((subject) =>
      rows.some((row) => row.subjectKey === subject.key && row.availability.length > 0),
    ) ??
    subjects[0] ??
    null;
      ...
      <div className="affiliate-intelligence-main">
        {selectedSubject.kind === "shop" ? (
          <AffiliateModelSourceSwitch
            ...
            automaticSelection={
              selectedRows.find((row) => row.automaticSelection)?.automaticSelection ?? null
            }
        ...
        {availability.length === 0 ? (
          <div
            className="affiliate-intelligence-empty"
          ...
          <div
            className="affiliate-model-stage-grid"
            data-tutorial-id="affiliate-intelligence-analysis"
          >
            <AffiliateModelStageCard availability={availability} stage="UNIFIED" />
          </div>
        )}
      </div>
    </div>
...
function AffiliateProductionModelDashboard({
  ...
  const claimBody =
    liftPercent != null && liftPercent > 0
  ...
  return (
    <>
      ...
      <div
        className="affiliate-intelligence-claim-section"
        data-tutorial-id="affiliate-intelligence-analysis"
      >
        <div className="affiliate-intelligence-comparison">
          <div className="affiliate-intelligence-card-head">
            ...
            <div className="affiliate-intelligence-card-aside">
              {liftPercent != null ? (
                <div
                  className={`affiliate-intelligence-lift-badge${liftPercent < 0 ? " affiliate-intelligence-lift-badge-negative" : ""}`}
                >
          ...
          <div className="affiliate-intelligence-evidence-grid">
              ...
              <div className="affiliate-intelligence-evidence-card data-card-hover affiliate-intelligence-evidence-card-probability">
                <strong>
                  <AffiliateMetricLabel
                    ...
                    tooltip={t(
                      "ecommerce.affiliateWorkspace.intelligenceOutperformanceProbabilityTooltip",
                    )}
                  />
                </strong>
            ...
            {dataFoundationLevel ? (
              <div className="affiliate-intelligence-evidence-card data-card-hover affiliate-intelligence-evidence-card-foundation">
                ...
                <span>
                  {t(
                    `ecommerce.affiliateWorkspace.intelligenceDataFoundationLevels.${dataFoundationLevel.toLowerCase()}`,
                  )}
                </span>
              </div>
            ) : null}
...
function AffiliateThresholdComparisonPanel({
  ...
  const maximumCount = Math.max(historicalQualifiedCount ?? 0, modelQualifiedCount ?? 0, 1);
  const liftPercent =
    comparison.qualifiedCreatorLiftRatio == null
      ? null
      ...
      <div className="affiliate-intelligence-comparison affiliate-intelligence-comparison-secondary">
        <div className="affiliate-intelligence-card-head">
          <div className="affiliate-intelligence-card-title">
            ...
            <p>
              {translate("ecommerce.affiliateWorkspace.intelligenceClaimReachBody", {
      ...
      <AffiliateClaimDistributionPanel
        ...
        series={[
          ...
          },
          {
            ...
            expectedTotal:
              comparison.belowThresholdCount ??
...
function AffiliateModelStageCard({
  ...
  const presentation = affiliateModelStagePresentation(availability, "EXPECTED_SALES", stage);
  const summary = presentation.evaluationSummary;
  const comparisonAvailable = Boolean(presentation.ready && summary?.comparisonAvailable);
  return (
    <section
      className={`affiliate-model-stage-card affiliate-model-stage-card-${stage.toLowerCase()}`}
    >
      <header>
        <div>
...
function AffiliateModelSourceSwitch({
  ...
  return (
    <div className="affiliate-intelligence-model-source">
      <span className="affiliate-intelligence-model-source-label">
        {t("ecommerce.affiliateWorkspace.intelligenceModelSourceSelector")}
      </span>
      <TkTabs
        variant="rail"
        className="affiliate-intelligence-model-source-options"
        label={t("ecommerce.affiliateWorkspace.intelligenceModelSourceSelector")}
        items={rows.map((item) => {
          const modelState = affiliateExpectedSalesModelAvailabilityState(item.row?.availability);
          const automaticallySelected =
            automaticSelection?.requestedTenantScope?.toLowerCase() === item.key;
          const status =
            modelState.status === "ready"
              ? t("ecommerce.affiliateWorkspace.modelReady")
              : modelState.status === "fallback"
                ? affiliateModelFallbackLabel(modelState.effectiveTenantScope, t)
                : item.row?.failed
                  ? t("ecommerce.affiliateWorkspace.intelligenceModelUnavailable")
                  : t("ecommerce.affiliateWorkspace.intelligenceNoModel");
          return {
            ...
            description: `${status} · ${item.description}${
              automaticallySelected
                ? ` · ${t("ecommerce.affiliateWorkspace.intelligenceAutomaticallySelected")}`
                : ""
            }`,
            tone:
              modelState.status === "ready"
                ...
                : modelState.status === "fallback"
                  ? ("warning" as const)
                  : ("default" as const),
            buttonProps: { disabled: !item.row },
          };
        })}
        value={activeModelScope}
        onChange={(value) => onChange(value as AffiliateInsightModelScope)}
      />
    </div>
  );
}
...
function filterActionProposals(
  ...
  return proposals.filter((proposal) =>
    actionProposalSearchText(proposal, shopLabel).includes(query),
  );
}
...
function actionProposalSearchText(
  ...
  const values = [
    ...
    ...(proposal.messageIntent?.parts.flatMap((part) => [
      part.text,
      part.productId,
      part.fileName,
    ]) ?? []),
...
function filterAffiliateCollaborations(
  ...
  return records.filter((record) =>
    affiliateCollaborationSearchText(record, shopLabel).includes(query),
  );
}
...
export const AffiliateCreatorsPage = observer(function AffiliateCreatorsPage() {
  ...
  const [manualTagMatchMode, setManualTagMatchMode] = useState<GQL.TagMatchMode>(
    GQL.TagMatchMode.Any,
  );
  const [selectedSystemTags, setSelectedSystemTags] = useState<GQL.AffiliateCreatorSystemTag[]>([]);
  const [systemTagMatchMode, setSystemTagMatchMode] = useState<GQL.TagMatchMode>(
    GQL.TagMatchMode.Any,
  );
  ...
  const projectionHistoryIncomplete =
    projectionHealth?.datasets.some((dataset) => !dataset.complete) ?? false;
  const projectionDatasetLabel = (dataset: string) =>
    t(PROJECTION_DATASET_I18N_KEY[dataset] ?? dataset, { defaultValue: dataset });
  const projectionStatusLabel = (status: string) =>
    t(PROJECTION_STATUS_I18N_KEY[status] ?? status, { defaultValue: status });
  ...
  useEffect(() => {
    ...
    const available = new Set(
      manualTagCatalogSignature ? manualTagCatalogSignature.split(",") : [],
    );
    ...
    <AffiliatePageFrame className="affiliate-workbench">
        ...
        actions={
          ...
          >
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? t("common.loading") : t("ecommerce.shopDrawer.affiliate.refreshProposals")}
            </button>
          </AffiliateToolbar>
        }
      />
      ...
      {projectionHealth && !projectionHealth.ready ? (
        ...
        >
          ...
          <span>
            {t("ecommerce.affiliateWorkspace.projectionLastSynced")}:{" "}
            {formatDate(
              projectionHealth.datasets
      ...
      <div className="affiliate-workbench-panel">
        ...
        <AffiliateCreatorFilterGroups
          ...
          systemTagDefinitions={systemTagDefinitions}
          systemTagMatchMode={systemTagMatchMode}
        ...
        {loading && creatorItems.length === 0 ? (
          ...
          <div className="affiliate-proposal-empty" data-tutorial-id="affiliate-creators-results">
            ...
            {creatorPage > 1 || hasMoreCreators ? (
              ...
              >
                ...
                <div className="affiliate-collaboration-pagination-actions">
                  ...
                  <span className="affiliate-collaboration-page-pill">{creatorPage}</span>
                  <button
                    className="btn btn-secondary"
                    type="button"
...
function CreatorRelationshipCompactCard({
  ...
  const sampleStatusDescription = sampleStatus
    ...
    : null;
  const relationshipShopIds = Array.from(
    new Set(
      [
...
export function AffiliateCollaborationCard({
  collaboration,
  shopLabel,
  productSummary,
  productSummaries,
  ...
  const { t } = useTranslation();
  const creatorIds = collaboration.creatorIds ?? [];
  const creatorOpenIds = collaboration.creatorOpenIds ?? [];
  const productIds = collaboration.productIds ?? [];
  const products = collaboration.products ?? [];
  const creatorCount = creatorIds.length || creatorOpenIds.length;
  const productCount = productIds.length;
  const targetCommissionRates = products
    .filter(
      (product) =>
        !["DELETED", "DELETING"].includes(product.collaborationStatus?.toUpperCase() ?? ""),
    )
    .map((product) => affiliateBpsPercentValue(product.commission?.rate));
  const targetAdsCommissionRates = products
    .filter(
      (product) =>
        !["DELETED", "DELETING"].includes(product.collaborationStatus?.toUpperCase() ?? ""),
    )
  ...
  const visibleProductIds = variant === "embedded" ? productIds : productIds.slice(0, 1);
  const productForId = (productId: string | undefined) =>
    productSummaries?.find((product) => product.productId === productId) ??
    (productSummary?.productId === productId ? productSummary : null);
  ...
  return (
    <article
      className={affiliateEntityCardClassName(
        ...
        isInteractive,
        [
          "affiliate-collaboration-card",
          "affiliate-collaboration-record-card",
          variant === "embedded" ? "affiliate-entity-card-embedded" : "",
          isInteractive ? "affiliate-collaboration-card-interactive" : "",
        ]
          .filter(Boolean)
          .join(" "),
      )}
      data-tutorial-id={variant === "listing" ? "affiliate-history-results" : undefined}
        ...
        <div className="affiliate-creator-text">
          ...
          <div className="affiliate-work-item-meta">
            {contextLabels.length > 0 ? (
              <span>{contextLabels.join(" · ")}</span>
            ) : (
              <span>
                {formatProposalTime(
                  collaboration.platformUpdatedAt ?? collaboration.lastObservedAt,
                )}
              </span>
            )}
        ...
        <div className="affiliate-relationship-work-card-priority">
          ...
          {collaboration.type === GQL.AffiliateCollaborationType.Target ? (
            <>
              ...
              <RelationshipMetric
                label={t(
                  "ecommerce.affiliateWorkspace.collaborationOperations.adsCommissionPercent",
                )}
            ...
            <RelationshipMetric
              ...
              value={
                collaboration.commissionRate == null
                  ? "—"
                  : formatPercent(collaboration.commissionRate)
              }
            />
          )}
        </div>
...
function AffiliateCollaborationDetailModal({
  ...
  return (
    <>
      <AffiliateDetailModal
        className="affiliate-platform-collaboration-detail-modal"
        ariaLabel={t("ecommerce.affiliateWorkspace.collaborationOperations.detailsTitle")}
        onClose={onClose}
      >
        <header className="modal-header affiliate-platform-collaboration-modal-header">
          <div className="affiliate-collaboration-modal-title-block">
            <h2>
              {collaboration?.name ||
                t("ecommerce.affiliateWorkspace.collaborationOperations.detailsTitle")}
            </h2>
            {collaboration ? (
              <p>
                <span
                  className={`affiliate-platform-collaboration-type-mark is-${collaboration.type.toLowerCase()}`}
                >
                  {formatAffiliateEnumLabel(collaboration.type)}
                </span>
          ...
          {error ? (
            ...
            <>
              <section className="affiliate-platform-collaboration-hero">
                <div className="affiliate-platform-collaboration-hero-identity">
                  <span
                    className={`affiliate-platform-collaboration-status is-${collaboration.status.toLowerCase()}`}
                  >
                    {formatAffiliateEnumLabel(collaboration.status)}
                  </span>
                  <div className="affiliate-platform-collaboration-hero-copy">
                    <h3>
                      {collaboration.type === GQL.AffiliateCollaborationType.Open
                        ? t(
                            "ecommerce.affiliateWorkspace.collaborationOperations.openProductProgram",
                          )
                        : t(
                            "ecommerce.affiliateWorkspace.collaborationOperations.targetInvitation",
                          )}
                    </h3>
                    <p>
                      {collaboration.type === GQL.AffiliateCollaborationType.Open
                        ? t(
                            "ecommerce.affiliateWorkspace.collaborationOperations.openProductProgramHint",
                          )
                        : t(
                            "ecommerce.affiliateWorkspace.collaborationOperations.targetInvitationHint",
                          )}
                    </p>
                  </div>
                </div>
                <dl className="affiliate-platform-collaboration-detail-summary">
                  <div>
                    <dt>{t("ecommerce.affiliateWorkspace.collaborationOperations.creators")}</dt>
                    <dd>
                      {formatInteger(detail.creators.length || collaboration.creatorOpenIds.length)}
                    </dd>
                  </div>
                  <div>
                    <dt>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.sampleApplications")}
                    </dt>
                    <dd>{formatInteger(detail.sampleApplications.length)}</dd>
                  </div>
                  <div>
                    <dt>{t("ecommerce.affiliateWorkspace.labels.relatedProduct")}</dt>
                    <dd>{formatInteger(collaboration.productIds.length)}</dd>
                  </div>
                  <div className="is-time">
                    <dt>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.lastObserved")}
                    </dt>
                    <dd>{formatProposalTime(collaboration.lastObservedAt)}</dd>
                  </div>
                </dl>
              </section>

              <div className="affiliate-platform-collaboration-workspace">
                <main className="affiliate-platform-collaboration-primary-panel">
                  <section className="affiliate-platform-collaboration-linked-section is-products">
                    ...
                    {collaboration.productIds.length > 0 ? (
                      ...
                      <div className="affiliate-platform-collaboration-empty-row">
                        <span aria-hidden="true">—</span>
                        <p>{t("ecommerce.affiliateWorkspace.productContextMissing")}</p>
                      </div>
                    )}
                  </section>
                ...
                <aside className="affiliate-platform-collaboration-context-panel">
                  <section className="affiliate-platform-collaboration-linked-section is-creators">
                    <div className="affiliate-platform-collaboration-section-heading">
                    ...
                    {detail.creators.length > 0 ? (
                      <div className="affiliate-platform-collaboration-identity-list">
                        {detail.creators.map((creator) => (
                          <div
                            className="affiliate-platform-collaboration-identity"
                            key={creator.id}
                          >
                            <CreatorAvatarImage
                      ...
                      <div className="affiliate-platform-collaboration-empty-row">
                        ...
                        <p>
                          {collaboration.type === GQL.AffiliateCollaborationType.Open
                            ? t(
                                "ecommerce.affiliateWorkspace.collaborationOperations.noExpandedOpenCreators",
                              )
                  ...
                  <section className="affiliate-platform-collaboration-linked-section is-samples">
                    <div className="affiliate-platform-collaboration-section-heading">
                      <h3>
                        {t(
                          "ecommerce.affiliateWorkspace.collaborationOperations.samplesAndFulfillment",
                        )}
                      </h3>
                      <small>{formatInteger(detail.sampleApplications.length)}</small>
                    </div>
                    {detail.sampleApplications.length > 0 ? (
                      <div className="affiliate-entity-card-list">
                        {detail.sampleApplications.map((sample) => (
                          <SampleApplicationSummaryCard
                            ...
                            productSummary={
                              detail.productSummaries.find(
                                (entry) =>
                                  entry.shopId === sample.shopId &&
                                  entry.product.productId === sample.productId,
                              )?.product ?? null
                            }
                      ...
                      <div className="affiliate-platform-collaboration-empty-row">
                        ...
                        <p>
                          {t(
                            "ecommerce.affiliateWorkspace.collaborationOperations.noSampleApplications",
                          )}
                        </p>
                      </div>
                    )}
              ...
              {editing ? (
                <aside
                  ...
                  aria-label={t(
                    "ecommerce.affiliateWorkspace.collaborationOperations.editConfiguration",
                  )}
                >
                  <div className="affiliate-platform-editor-drawer-head">
                    <strong>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.editConfiguration")}
                    </strong>
                    <button
                  ...
                  <div className="affiliate-platform-editor-drawer-body">
                    {collaboration.type === GQL.AffiliateCollaborationType.Open ? (
...
function targetProductDrafts(
  collaboration: GQL.AffiliateCollaboration,
): AffiliateTargetProductDraft[] {
...
function collaborationSampleRuleIsEnabled(
  rule: GQL.EcomOpenCollaborationSampleRule | null | undefined,
): boolean {
    ...
    <section className="affiliate-platform-collaboration-config-snapshot">
        ...
        <div className="affiliate-platform-product-commission-snapshot">
          {targetProductDrafts(collaboration).map((product) => (
            <div className="affiliate-platform-product-commission-row" key={product.productId}>
              ...
              <span>
                {t("ecommerce.affiliateWorkspace.collaborationOperations.commissionPercent")}:{" "}
                {affiliateCommissionPercentRange([product.commissionPercent])}
              </span>
              <span>
                {t("ecommerce.affiliateWorkspace.collaborationOperations.adsCommissionPercent")}:{" "}
                {affiliateCommissionPercentRange([product.adsCommissionPercent])}
              </span>
            </div>
          ))}
        </div>
      ...
      {collaboration.type === GQL.AffiliateCollaborationType.Open ? (
        <div className="affiliate-platform-collaboration-guardrail">
          <InfoIcon />
          <span>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.commissionLockedHint")}
          </span>
        </div>
      ) : null}
    </section>
...
function AffiliateOpenCollaborationEditor({
  ...
  const [sampleQuota, setSampleQuota] = useState(
    rule?.sampleQuota == null ? "" : String(rule.sampleQuota),
  );
  ...
  const [minimumFollowerCount, setMinimumFollowerCount] = useState(
    rule?.thresholds?.minimumFollowerCount == null
      ? ""
      : String(rule.thresholds.minimumFollowerCount),
  );
  const [minimumGmv, setMinimumGmv] = useState(
    rule?.thresholds?.minimumGmv == null ? "" : String(rule.thresholds.minimumGmv),
  );
  const [avgEcVideoViews, setAvgEcVideoViews] = useState(
    rule?.thresholds?.avgEcVideoViews == null ? "" : String(rule.thresholds.avgEcVideoViews),
  );
  const [categoryIds, setCategoryIds] = useState(rule?.thresholds?.categoryIds?.join(", ") ?? "");
  const [predictedFulfillmentRank, setPredictedFulfillmentRank] = useState(
    rule?.thresholds?.predictedFulfillmentRank ?? "",
  );
  ...
  return (
    <form
      className="affiliate-platform-collaboration-editor"
      onSubmit={(event) => void submit(event)}
    >
      <div className="affiliate-platform-collaboration-editor-heading">
        <div>
          <span>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.platformConfiguration")}
          </span>
          <h3>{t("ecommerce.affiliateWorkspace.collaborationOperations.editOpenSampleRule")}</h3>
        </div>
        <span className="affiliate-platform-write-badge">
          {t("ecommerce.affiliateWorkspace.collaborationOperations.writesImmediately")}
        </span>
      </div>
      <div className="affiliate-platform-collaboration-guardrail">
        <InfoIcon />
      ...
      <label className="affiliate-platform-toggle-field">
        ...
        <span>
          <strong>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.enableSampleRule")}
          </strong>
          <small>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.enableSampleRuleHint")}
          </small>
        </span>
      </label>
      {enabled ? (
        <>
          <div className="affiliate-platform-form-grid">
            ...
            >
              <input
                ...
                min={0}
                value={sampleQuota}
                onChange={(event) => setSampleQuota(event.target.value)}
              />
            </AffiliateOperationField>
            <label className="affiliate-platform-toggle-field affiliate-platform-toggle-field-compact">
              ...
              <span>
                <strong>
                  {t("ecommerce.affiliateWorkspace.collaborationOperations.unlimitedSampleWindow")}
                </strong>
              </span>
            </label>
            {!unlimited ? (
              <>
                <AffiliateOperationField
                  label={t("ecommerce.affiliateWorkspace.collaborationOperations.startTime")}
                >
                  <input
                    className="input-full"
                    type="datetime-local"
          ...
          <div className="affiliate-platform-editor-subsection">
            ...
            <div className="affiliate-platform-form-grid affiliate-platform-form-grid-three">
              <AffiliateOperationField
                label={t("ecommerce.affiliateWorkspace.collaborationOperations.minimumFollowers")}
              >
                <input
                  className="input-full"
                  type="number"
                  ...
                  onChange={(event) => setMinimumFollowerCount(event.target.value)}
                />
              </AffiliateOperationField>
              <AffiliateOperationField
                label={t("ecommerce.affiliateWorkspace.collaborationOperations.minimumGmv")}
              >
                <input
                  className="input-full"
                  type="number"
                  min={0}
                  value={minimumGmv}
                  onChange={(event) => setMinimumGmv(event.target.value)}
                />
              </AffiliateOperationField>
              <AffiliateOperationField
                label={t("ecommerce.affiliateWorkspace.collaborationOperations.averageVideoViews")}
              >
                <input
                  className="input-full"
                  type="number"
                  min={0}
                  value={avgEcVideoViews}
                  onChange={(event) => setAvgEcVideoViews(event.target.value)}
                />
              </AffiliateOperationField>
              <AffiliateOperationField
                label={t("ecommerce.affiliateWorkspace.collaborationOperations.categoryIds")}
                hint={t("ecommerce.affiliateWorkspace.collaborationOperations.delimitedHint")}
              >
                <input
                  className="input-full"
                  value={categoryIds}
              ...
              <AffiliateOperationField
                label={t("ecommerce.affiliateWorkspace.collaborationOperations.fulfillmentRank")}
              >
                <input
                  className="input-full"
                  value={predictedFulfillmentRank}
                  onChange={(event) => setPredictedFulfillmentRank(event.target.value)}
                />
              </AffiliateOperationField>
            </div>
          </div>
      ...
      <div className="affiliate-platform-editor-actions">
        <span>
          {t("ecommerce.affiliateWorkspace.collaborationOperations.immediateProjectionHint")}
        </span>
        <button className="btn btn-primary" type="submit" disabled={saveRuleState.loading}>
          {saveRuleState.loading
            ? t("common.loading")
            : t("ecommerce.affiliateWorkspace.collaborationOperations.saveChanges")}
        </button>
      </div>
    </form>
  );
...
function AffiliateTargetCollaborationEditor({
  ...
  const activeCreatorOpenIds = collaboration.targetCreators
    .filter(
      (creator) =>
        !["DELETED", "DELETING"].includes(creator.collaborationStatus?.toUpperCase() ?? ""),
    )
  ...
  const [products, setProducts] = useState<AffiliateTargetProductDraft[]>(
    targetProductDrafts(collaboration),
  );
  const [email, setEmail] = useState(collaboration.sellerContactInfo?.email ?? "");
  const [phoneNumber, setPhoneNumber] = useState(
    collaboration.sellerContactInfo?.phoneNumber ?? "",
  );
  ...
  const [hasFreeSample, setHasFreeSample] = useState<boolean | null>(
    collaboration.freeSampleRule?.hasFreeSample ?? null,
  );
  const [isSampleApprovalExempt, setIsSampleApprovalExempt] = useState<boolean | null>(
    collaboration.freeSampleRule?.isSampleApprovalExempt ?? null,
  );
  ...
  const missingSafetyFields = [
    ...
    affiliateDelimitedIdentifiers(creatorOpenIds).length === 0
      ? t("ecommerce.affiliateWorkspace.collaborationOperations.creatorOpenIds")
      : null,
    products.length === 0 ||
    products.some((product) => !product.productId.trim() || !product.commissionPercent.trim())
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (missingSafetyFields.length > 0) {
      showToast(
        t("ecommerce.affiliateWorkspace.collaborationOperations.missingSafetyFields"),
        "error",
      );
      return;
    }
    try {
      ...
      const partialFailure =
        result.data?.updateAffiliateTargetCollaboration.providerResult.updateFailed;
      showToast(
        partialFailure
    ...
    >
      <div className="affiliate-platform-collaboration-editor-heading">
        <div>
          ...
          <h3>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.editTargetCollaboration")}
          </h3>
        </div>
        <span className="affiliate-platform-write-badge">
          {t("ecommerce.affiliateWorkspace.collaborationOperations.writesImmediately")}
      ...
      {missingSafetyFields.length > 0 ? (
        <div className="affiliate-platform-collaboration-data-warning" role="alert">
          <strong>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.editBlockedTitle")}
          </strong>
          <span>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.editBlockedHint", {
              fields: missingSafetyFields.join(", "),
            })}
          </span>
        </div>
      ) : null}
      <div className="affiliate-platform-form-grid">
        ...
        >
          <textarea
            className="input-full"
            rows={4}
            value={creatorOpenIds}
            onChange={(event) => setCreatorOpenIds(event.target.value)}
            required
          />
        </AffiliateOperationField>
      </div>

      <AffiliateTargetProductRows
        products={products}
      ...
      <div className="affiliate-platform-editor-subsection">
        ...
        <div className="affiliate-platform-form-grid affiliate-platform-form-grid-three">
          <AffiliateOperationField
            label={t("ecommerce.affiliateWorkspace.collaborationOperations.email")}
          >
            <input
              className="input-full"
              type="email"
          ...
          <AffiliateOperationField
            label={t("ecommerce.affiliateWorkspace.collaborationOperations.phoneNumber")}
          >
            <input
              className="input-full"
              value={phoneNumber}
      ...
      <div className="affiliate-platform-editor-actions">
        ...
        <button
          ...
          type="submit"
          disabled={updateTargetState.loading || missingSafetyFields.length > 0}
        >
          {updateTargetState.loading
            ? t("common.loading")
            : t("ecommerce.affiliateWorkspace.collaborationOperations.saveChanges")}
        </button>
      </div>
    ...
    <div className="affiliate-platform-editor-subsection">
      <div className="affiliate-platform-editor-subsection-heading">
        <div>
          ...
          <small>
            {t("ecommerce.affiliateWorkspace.collaborationOperations.fullProductListHint")}
          </small>
        </div>
        <button
          className="btn btn-secondary"
          ...
          onClick={() =>
            onChange([
              ...products,
              { productId: "", commissionPercent: "", adsCommissionPercent: "" },
            ])
          }
        >
      ...
      <div className="affiliate-platform-product-editor-list">
        {products.map((product, index) => (
          <div
            className="affiliate-platform-product-editor-row"
            key={`${product.productId}:${index}`}
          >
            <AffiliateOperationField
              label={t("ecommerce.affiliateWorkspace.collaborationOperations.productId")}
            >
              <input
            ...
            >
              <input
                ...
                max={80}
                step={0.01}
                value={product.commissionPercent}
                onChange={(event) => onUpdate(index, { commissionPercent: event.target.value })}
                required
              />
            </AffiliateOperationField>
            <AffiliateOperationField
            ...
            >
              <input
                ...
                max={80}
                step={0.01}
                value={product.adsCommissionPercent}
                onChange={(event) => onUpdate(index, { adsCommissionPercent: event.target.value })}
              />
            </AffiliateOperationField>
            <button
...
function AffiliateCollaborationCreateModal({
    ...
    try {
      if (mode === "OPEN") {
        ...
        if (!result.data?.createAffiliateTargetCollaboration.collaboration) {
          showToast(
            t("ecommerce.affiliateWorkspace.collaborationOperations.targetCreateConflict"),
            "warning",
          );
          return;
        }
  ...
  return (
    <Modal
      isOpen
      onClose={onClose}
      title={t("ecommerce.affiliateWorkspace.collaborationOperations.newCollaboration")}
      hideHeader
      padding="none"
      className="affiliate-collaboration-modal affiliate-platform-collaboration-create-modal"
        ...
        <div className="modal-header affiliate-platform-collaboration-modal-header">
          <div className="affiliate-collaboration-modal-title-block">
            <div className="affiliate-platform-collaboration-kicker">
              {t("ecommerce.affiliateWorkspace.collaborationOperations.platformOperation")}
            </div>
        ...
        <div className="affiliate-platform-collaboration-create-body">
          <div className="affiliate-platform-collaboration-create-rail">
            ...
            <TkSegmented
              items={[
                {
                  ...
                  label: (
                    <span>
                      <strong>Open</strong>
                      <small>
                        {t("ecommerce.affiliateWorkspace.collaborationOperations.openCreateChoice")}
                      </small>
                    </span>
                  ),
                },
                {
                  ...
                  label: (
                    <span>
                      ...
                      <small>
                        {t(
                          "ecommerce.affiliateWorkspace.collaborationOperations.targetCreateChoice",
                        )}
                      </small>
                    </span>
                  ),
              ...
              value={mode}
              onChange={(value) => setMode(value as "OPEN" | "TARGET")}
              label={t("ecommerce.affiliateWorkspace.collaborationOperations.newCollaboration")}
            />
            <div className="affiliate-platform-collaboration-rail-note">
              ...
              <span>
                {mode === "OPEN"
                  ...
                  : t("ecommerce.affiliateWorkspace.collaborationOperations.targetCreateRailHint")}
              </span>
            </div>
          </div>
          <div className="affiliate-platform-collaboration-create-form">
            {mode === "OPEN" ? (
              <>
                <div className="affiliate-platform-collaboration-editor-heading">
                  <div>
                    ...
                    <h3>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.enableProduct")}
                    </h3>
                  </div>
                  <span className="affiliate-platform-write-badge">
                    {t("ecommerce.affiliateWorkspace.collaborationOperations.writesImmediately")}
                ...
                <div className="affiliate-platform-form-grid">
                  ...
                  >
                    <input
                      className="input-full input-mono"
                      value={openProductId}
                      onChange={(event) => setOpenProductId(event.target.value)}
                      required
                    />
                  </AffiliateOperationField>
                  <AffiliateOperationField
                    label={t(
                      "ecommerce.affiliateWorkspace.collaborationOperations.commissionPercent",
                    )}
                    hint={t(
                      "ecommerce.affiliateWorkspace.collaborationOperations.commissionRangeHint",
                    )}
                  >
                    <input
                      className="input-full"
                      ...
                      max={80}
                      step={0.01}
                      value={openCommissionPercent}
                      onChange={(event) => setOpenCommissionPercent(event.target.value)}
                      required
                    />
                  </AffiliateOperationField>
                </div>
                <div className="affiliate-platform-collaboration-data-warning is-neutral">
                  <strong>
                    {t(
                      "ecommerce.affiliateWorkspace.collaborationOperations.commissionLockedTitle",
                    )}
                  </strong>
                  <span>
                    {t(
                      "ecommerce.affiliateWorkspace.collaborationOperations.commissionLockedCreateHint",
                    )}
                  </span>
                </div>
              </>
              ...
              <>
                <div className="affiliate-platform-collaboration-editor-heading">
                  <div>
                    <span>Target Collaboration</span>
                    <h3>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.createInvitation")}
                    </h3>
                  </div>
                  <span className="affiliate-platform-write-badge">
                    {t("ecommerce.affiliateWorkspace.collaborationOperations.writesImmediately")}
                ...
                <div className="affiliate-platform-form-grid">
                  ...
                  <AffiliateOperationField
                    className="affiliate-platform-operation-field-wide"
                    label={t("ecommerce.affiliateWorkspace.collaborationOperations.message")}
                  >
                    <textarea
                      className="input-full"
                      rows={3}
                  ...
                  >
                    <textarea
                      className="input-full input-mono"
                      rows={4}
                      value={targetCreatorOpenIds}
                      onChange={(event) => setTargetCreatorOpenIds(event.target.value)}
                      required
                    />
                  </AffiliateOperationField>
                </div>
                ...
                <div className="affiliate-platform-binary-grid">
                  ...
                  <AffiliateBinaryChoice
                    label={t(
                      "ecommerce.affiliateWorkspace.collaborationOperations.sampleApprovalExempt",
                    )}
                    value={isSampleApprovalExempt}
                    onChange={setIsSampleApprovalExempt}
                  />
        ...
        <div className="affiliate-platform-create-actions">
          ...
          <div>
            ...
            <button className="btn btn-primary" type="submit" disabled={submitting}>
              {submitting
                ? t("common.loading")
                : t("ecommerce.affiliateWorkspace.collaborationOperations.createOnTikTok")}
            </button>
          </div>
        </div>
      </form>
...
function AffiliateOpenCollaborationSettingsModal({
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      showToast(
        t("ecommerce.affiliateWorkspace.collaborationOperations.settingsSaveSuccess"),
        "success",
      );
      await refetch();
    } catch (mutationError) {
  ...
  return (
    <Modal
      ...
      hideHeader
      padding="none"
      className="affiliate-platform-settings-modal affiliate-platform-auto-add-modal"
      backdropClassName="affiliate-creator-detail-backdrop"
        ...
        <div className="affiliate-platform-settings-body">
          <section className="affiliate-platform-settings-scope">
            <div className="affiliate-platform-settings-step">
              <span>01</span>
              <div>
                <strong>{t("ecommerce.affiliateWorkspace.collaborationOperations.shop")}</strong>
                <small>
          ...
          {loading ? (
            ...
            <>
              <section className="affiliate-platform-settings-rule">
                <div className="affiliate-platform-settings-step">
                  <span>02</span>
                  <div>
                    <strong>
                      {t("ecommerce.affiliateWorkspace.collaborationOperations.autoAddProducts")}
                    </strong>
                    <small>
                      {t(
                        "ecommerce.affiliateWorkspace.collaborationOperations.autoAddProductsHint",
                      )}
                    </small>
                  </div>
                </div>
                <label className="affiliate-platform-settings-switch">
                  <input
                    type="checkbox"
                    checked={enabled}
                    onChange={(event) => setEnabled(event.target.checked)}
                ...
                <AffiliateOperationField
                  className="affiliate-platform-settings-commission"
                  label={t(
                    "ecommerce.affiliateWorkspace.collaborationOperations.defaultCommissionPercent",
                  )}
                  hint={t(
                    "ecommerce.affiliateWorkspace.collaborationOperations.commissionRangeHint",
                  )}
                >
                  <div className="affiliate-platform-settings-percent-input">
                    <input
                      className="input-full"
                      type="number"
                      min={1}
              ...
              <div className="affiliate-platform-collaboration-data-warning is-neutral">
                ...
                <span>
                  {t("ecommerce.affiliateWorkspace.collaborationOperations.settingsScopeHint")}
                </span>
              </div>
            </>
          )}
        ...
        <div className="affiliate-platform-create-actions">
          ...
          <div>
            ...
            <button
              className="btn btn-primary"
              type="submit"
              disabled={loading || saveSettingsState.loading || !commissionPercent}
            >
              {saveSettingsState.loading
                ? t("common.loading")
                : t("ecommerce.affiliateWorkspace.collaborationOperations.saveSettings")}
            </button>
          </div>
        </div>
      </form>
...
function RelationshipPlatformCollaborationCard({
  ...
  const sourceLabels = item.sources.map((source) =>
    source === GQL.AffiliateRelationshipPlatformCollaborationSource.TargetMembership
      ? t("ecommerce.affiliateWorkspace.platformCollaborationTargetMembership", {
          defaultValue: "Target membership",
        })
      : t("ecommerce.affiliateWorkspace.platformCollaborationSampleReference", {
...
function buildRelationshipTimelineEntries(
  ...
  return items.map((item) => {
    ...
    return {
      id: item.id,
      type:
        item.kind === GQL.AffiliateRelationshipTimelineItemKind.TimePassed
...
export type RelationshipTimelineLane = "creator" | "operator" | "system";

export function relationshipTimelineLane(
  item: GQL.AffiliateRelationshipTimelineItem,
): RelationshipTimelineLane {
  if (item.message?.direction === GQL.AffiliateCreatorMessageDirection.Creator) return "creator";
  if (item.message?.direction === GQL.AffiliateCreatorMessageDirection.Seller) return "operator";
  const event = item.businessEvent ?? item.actionEvent;
  const actorRole = item.actorRole ?? event?.actorRole;
  if (actorRole === GQL.AffiliateLifecycleActorRole.Creator) return "creator";
  if (
    actorRole === GQL.AffiliateLifecycleActorRole.Staff ||
    actorRole === GQL.AffiliateLifecycleActorRole.Agent
  ) {
    return "operator";
  }
  return "system";
...
function relationshipTimelineDetail(
  ...
  if (
    ...
    normalizeTimelineText(event.displaySummary) !==
      normalizeTimelineText(formatAffiliateEnumLabel(event.eventType))
  ) {
    lines.push(event.displaySummary);
  }
  if (item.timePassed) {
    lines.push(
      t("ecommerce.affiliateWorkspace.timePassedHint", {
...
function RelationshipTimelineEntry({ entry }: { entry: RelationshipTimelineEntryModel }) {
  ...
  const samplePayload =
    entry.cardPayload?.kind === "sample" && entry.cardPayload.id
      ? {
      ...
      : null;
  return (
    <div className={`affiliate-timeline-row affiliate-timeline-row-${entry.lane}`} key={entry.id}>
      ...
      <div className="affiliate-timeline-entry">
        ...
        <div className="affiliate-timeline-event-card">
          ...
          {entry.detail ? <div className="affiliate-work-item-preview">{entry.detail}</div> : null}
          {samplePayload ? (
...
export const AffiliateHistoryPage = observer(function AffiliateHistoryPage() {
  ...
  const [historyTypeFilter, setHistoryTypeFilter] = useState<HistoryTypeFilter>(
    GQL.AffiliateCollaborationType.Open,
  );
  ...
  const searchedItems = filterAffiliateCollaborations(
    collaborations,
    historySearch,
    shopLabel,
  ).filter((record) =>
    affiliateCollaborationMatchesHistoryStatusFilter(record, historyStatusFilter),
  );
  const historyTypeOptions = useMemo(
    () => [
      {
        value: GQL.AffiliateCollaborationType.Open,
        label: formatAffiliateEnumLabel(GQL.AffiliateCollaborationType.Open),
      },
      {
        value: GQL.AffiliateCollaborationType.Target,
        label: formatAffiliateEnumLabel(GQL.AffiliateCollaborationType.Target),
      },
      {
        value: "ALL",
        label: t("ecommerce.affiliateWorkspace.allCollaborationTypes", {
          defaultValue: "All collaboration types",
        }),
      },
    ],
    [t],
  ...
  const historyPageCount = Math.max(
    1,
    Math.ceil(visibleItems.length / CREATOR_RELATIONSHIP_WORK_PAGE_SIZE),
  );
  ...
  const pageProductRefs = useMemo(
    () =>
      mergeById(
        pagedVisibleItems.flatMap((collaboration) =>
          collaboration.productIds.map((productId) => ({
            shopId: collaboration.shopId,
            productId,
          })),
  ...
  const pageStart =
    visibleItems.length === 0 ? 0 : (historyPage - 1) * CREATOR_RELATIONSHIP_WORK_PAGE_SIZE + 1;
    ...
    <AffiliatePageFrame className="affiliate-workbench">
      ...
      <div className="affiliate-workbench-panel">
        <div className="affiliate-workbench-panel-head">
          ...
          <div className="affiliate-attention-toolbar" data-tutorial-id="affiliate-history-filters">
            ...
            <label className="affiliate-filter-field">
              <span>
                {t("ecommerce.affiliateWorkspace.collaborationOperations.collaborationType")}
              </span>
              <Select
                ...
                className="affiliate-status-select"
                ariaLabel={t(
                  "ecommerce.affiliateWorkspace.collaborationOperations.collaborationType",
                )}
              />
            </label>
        ...
        {error ? (
          ...
          <div className="affiliate-proposal-empty" data-tutorial-id="affiliate-history-results">
            {t("ecommerce.affiliateWorkspace.emptyHistory")}
          </div>
        ) : (
          <>
            <div className="affiliate-collaboration-list affiliate-platform-collaboration-list">
              {pagedVisibleItems.map((record) => (
                <AffiliateCollaborationCard
                  ...
                  productSummary={
                    pageProductSummaries.find(
                      (entry) =>
                        entry.shopId === record.shopId &&
                        entry.product.productId === record.productIds[0],
                    )?.product ?? null
                  }
            ...
            {visibleItems.length > CREATOR_RELATIONSHIP_WORK_PAGE_SIZE ? (
              <div
                className="affiliate-collaboration-pagination"
                aria-label={t("ecommerce.affiliateWorkspace.pagination")}
              >
                <span className="affiliate-collaboration-pagination-summary">
...
function affiliateCreatorMessageKey(message: AffiliateConversationMessage): string {
...
function affiliateCreatorMessageTimestamp(message: AffiliateConversationMessage): number {
  const value =
    message.createdAt ??
...
export function sortAffiliateCreatorMessagesOldestFirst(
  items: AffiliateConversationMessage[],
): AffiliateConversationMessage[] {
  return [...items].sort(
    (left, right) =>
      affiliateCreatorMessageTimestamp(left) - affiliateCreatorMessageTimestamp(right),
  );
}
...
function AffiliateCreatorMessageRow({
  ...
  return (
    <div
      className={`affiliate-conversation-message-row affiliate-conversation-message-${directionKey}`}
    >
      ...
      {hasCardRefs ? (
        <div className="affiliate-conversation-card-stack">
          ...
          {sampleRefs.map((ref) => (
            <AffiliateCreatorMessageSampleRefCard
              key={`sample:${ref.platformApplicationId}`}
              refItem={ref}
            />
          ))}
          {targetRefs.map((ref) => (
            <AffiliateCreatorMessageTargetRefCard
              key={`target:${ref.platformTargetCollaborationId}`}
              refItem={ref}
            />
          ))}
        </div>
    ...
    return (
      <div className="affiliate-conversation-card affiliate-conversation-target-card">
        ...
        <div className="affiliate-conversation-card-body">
          ...
          <span>
            {[part.mimeType, part.sizeBytes != null ? formatFileSize(part.sizeBytes) : null]
              .filter(Boolean)
              .join(" · ")}
          </span>
          ...
          {part.attachmentRef ? (
            <button
              ...
              onClick={() =>
                void downloadAffiliateAttachment(part, creatorRelationshipId, setDownloading)
              }
            >
    ...
    <div className="affiliate-conversation-card affiliate-conversation-target-card">
      ...
      <div className="affiliate-conversation-card-body">
        ...
        <div className="affiliate-conversation-card-meta">
          ...
          {productCount > 0 ? (
            <span>
              {t("ecommerce.affiliateWorkspace.conversation.productCount", { count: productCount })}
            </span>
          ) : null}
        </div>
      </div>
...
function parsePlatformCardPayload(value: string): AffiliateCreatorMessageRawCardPayload | null {
  ...
  try {
    ...
    const targetId =
      stringRecordValue(parsed, "target_collaboration_id") ??
      stringRecordValue(parsed, "invitation_group_id");
    if (targetId) return { id: targetId, kind: "target" };
    const sampleId =
      stringRecordValue(parsed, "application_id") ?? stringRecordValue(parsed, "apply_id");
...
function formatConversationProductPrice(
...
function formatConversationMoney(
...
export type AffiliateProposalManualTagRow = {
  ...
  systemTag: GQL.AffiliateCreatorSystemTag | null;
...
export function proposalManualTagRows(
  ...
  return sources
    .filter(({ intent }) => Boolean(intent.manualTagId || intent.systemTag))
    .map(({ key, intent }) => ({
      key,
      operation: intent.operation,
      manualTagId: intent.manualTagId ?? null,
      systemTag: intent.systemTag ?? null,
      tagName: intent.manualTagId ? (nameById.get(intent.manualTagId) ?? null) : null,
...
function renderCreatorTagIntentSummary(
  ...
  const displayName = intent.systemTag
    ? creatorSystemTagLabel(t, intent.systemTag)
    : (tagName ?? intent.manualTagId ?? t("ecommerce.affiliateWorkspace.manualTags.deletedTag"));
  return t(
      ...
      <div className="affiliate-manual-tag-change-list">
        {rows.map((row) => (
          <div className="affiliate-manual-tag-change" key={row.key}>
            ...
            <strong>
              {row.systemTag
                ? creatorSystemTagLabel(t, row.systemTag)
                : (row.tagName ?? t("ecommerce.affiliateWorkspace.manualTags.deletedTag"))}
            </strong>
            {row.systemTag ? (
              <span className="affiliate-system-tag-scope">
                {t("ecommerce.affiliateWorkspace.systemTags.proposalScope")}
              </span>
            ) : null}
            <span className="affiliate-manual-tag-change-target">
              {t("ecommerce.affiliateWorkspace.manualTags.proposalTarget", {
...
function renderAgentWorkRecommendationTitle(
  ...
  return t("ecommerce.affiliateWorkspace.sampleReviewActions.summary", {
    ...
    rejectCount: summary.rejectCount - ignoreCount,
    ignoreCount,
  });
}
...
function proposalHasMessageIntent(proposal: GQL.ActionProposal): boolean {
  return Boolean(
    proposal.messageIntent || (proposal.steps ?? []).some((step) => Boolean(step.messageIntent)),
  );
}
      ...
      <table className="affiliate-agent-work-table">
        ...
        <thead>
          <tr>
            ...
            <th scope="col">{t("ecommerce.affiliateWorkspace.agentWorkTable.creator")}</th>
            <th scope="col">
              {t("ecommerce.affiliateWorkspace.agentWorkTable.creatorMetrics.title")}
            </th>
        ...
        <tbody>
          {bundles.map((bundle) => {
            ...
            const creatorSecondaryName =
              creatorNickname && normalizeTikTokUsername(creatorNickname) !== creatorUsername
                ? creatorNickname
                : null;
              ...
              >
                ...
                <td className="affiliate-agent-work-table-creator-metrics">
                  <div className="affiliate-agent-work-creator-metric-grid">
                    <span>
                      <small>
                        {t("ecommerce.affiliateWorkspace.agentWorkTable.creatorMetrics.followers")}
                      </small>
                      <strong>{formatCount(proposal.creatorFollowerCount) ?? "—"}</strong>
                    </span>
                    <span>
                      <small>
                        {t("ecommerce.affiliateWorkspace.agentWorkTable.creatorMetrics.avgViews")}
                      </small>
                    ...
                    <span>
                      <small>
                        {t(
                          "ecommerce.affiliateWorkspace.agentWorkTable.creatorMetrics.engagementRate",
                        )}
                      </small>
                      <strong>
                        {formatPerformanceRate(proposal.creatorEngagementRate) ?? "—"}
                      </strong>
                    </span>
                    <span>
                      <small>
                        {t(
                          "ecommerce.affiliateWorkspace.agentWorkTable.creatorMetrics.shoppableVideos",
                        )}
                      </small>
                ...
                <td className="affiliate-agent-work-table-type">
                  <span
                    className={`affiliate-agent-work-type-${proposal.type.toLowerCase().replace(/_/g, "-")}`}
                  >
                ...
                <td className="affiliate-agent-work-table-work">
                  <div
                    className="affiliate-agent-work-table-actions"
...
function AgentWorkBundleDetailModal({
  ...
  return (
    <AffiliateDetailModal
      backdropClassName="affiliate-agent-work-detail-backdrop"
      className="affiliate-agent-work-detail-modal"
    ...
    >
      <TkModalHeader
        ...
        title={<span id={titleId}>{t("ecommerce.affiliateWorkspace.agentWorkDetail.title")}</span>}
        description={
          <>
      ...
      <div className="affiliate-agent-work-detail-body">
        <AgentWorkReviewContext
          ...
          timelineItems={(reviewTimelineData?.affiliateRelationshipTimeline?.items ?? []).filter(
            (item) => item.relatedIds.actionProposalId !== proposal.id,
          )}
          previousAgentWork={(reviewProposalHistoryData?.affiliateActionProposalPage?.items ?? [])
            .filter(
              (item) =>
                item.id !== proposal.id &&
                proposalTimestamp(item.createdAt) <= proposalTimestamp(proposal.createdAt),
            )
            .slice(0, 4)}
          loading={
            reviewRelationshipLoading || reviewTimelineLoading || reviewProposalHistoryLoading
          }
          failed={Boolean(
            reviewRelationshipError || reviewTimelineError || reviewProposalHistoryError,
          )}
    ...
    >
      ...
      <section className="affiliate-agent-review-context-section">
        <div className="affiliate-agent-review-context-section-head">
          ...
          <span>
            {t("ecommerce.affiliateWorkspace.agentWorkDetail.currentBusinessContextHint")}
          </span>
        </div>
        {reviewContextHasCurrentFacts(relationshipDetail) ? (
          <div className="affiliate-agent-review-metrics">
            <RelationshipMetric
              ...
              value={
                relationshipDetail?.lastContactedAt
              ...
              {previousAgentWork.map((item) => (
                <article key={item.id}>
                  <div>
                    ...
                    <span>
                      {t(`ecommerce.affiliateWorkspace.proposalFilters.${item.status}`, {
...
export function AgentWorkBundleCard({
  proposal,
  ...
  const creatorName = proposal.creatorProfile
    ...
    : t("ecommerce.affiliateWorkspace.unknownCreator");
  const creatorHandle = proposal.creatorProfile
    ? creatorTikTokHandle(proposal.creatorProfile)
    : null;
  const creatorPlatformId = proposal.creatorProfile
  ...
  const executionDescription =
    sampleReviewRows.length > 0 ? null : renderProposalExecutionDescription(proposal, t);
  ...
  const predictionSnapshot =
    ...
    proposal.type === GQL.ActionProposalType.ReviewSampleApplication
      ...
      : null;
  const resolvedVariant = variant === "full" ? "listing" : variant;
  ...
  const openPrimaryTarget = () => {
    if (canOpenRelationshipWork && detailItem && onOpenRelationshipWork)
  ...
  const revisionEditor =
    canDecide && revisionOpen ? (
      <div className="affiliate-proposal-revision-box" onClick={(event) => event.stopPropagation()}>
        <label
          className="affiliate-proposal-revision-label"
          htmlFor={`proposal-revision-${proposal.id}`}
        >
        ...
        <div className="affiliate-proposal-revision-foot">
          <span>{t("ecommerce.shopDrawer.affiliate.proposalRevisionNoteHint")}</span>
  ...
  const decisionActions = canDecide ? (
    <div className="affiliate-work-item-actions">
      {revisionOpen ? (
        ...
        <>
          {canRejectOverride && decidingProposal ? (
            <TkButton variant="secondary" disabled>
              {t("ecommerce.affiliateWorkspace.sampleReviewActions.change")}
            </TkButton>
          ) : canRejectOverride ? (
            <TkMenu
              label={t("ecommerce.affiliateWorkspace.sampleReviewActions.change")}
              triggerLabel={t("ecommerce.affiliateWorkspace.sampleReviewActions.change")}
              placement="top-end"
              items={sampleReviewOverrideOptions(proposal).map((option) => ({
                id: sampleReviewDisposition(option),
                label: t(
                  `ecommerce.affiliateWorkspace.sampleReviewActions.changeTo.${sampleReviewDisposition(option)}`,
                ),
                description:
                  sampleReviewDisposition(option) === "IGNORE"
                    ? t("ecommerce.affiliateWorkspace.sampleReviewActions.ignoreHint")
                    : undefined,
                onSelect: () => {
                  void onReject?.(proposal, option);
                },
              }))}
            />
          ) : null}
      ...
      >
        <div className="affiliate-proposal-row-shell">
          ...
          <div
            ...
            aria-label={
              reviewLayout ? t("ecommerce.affiliateWorkspace.agentWorkDetail.title") : undefined
            }
          ...
          >
            ...
            {proposal.operatorSummary ? (
              <div className="affiliate-card-section-copy affiliate-proposal-row-summary">
                {proposal.operatorSummary}
              </div>
            ) : null}
            ...
            <div className="affiliate-proposal-row-context">
                ...
                <section className="affiliate-card-section affiliate-card-bundled-message-section">
                  ...
                  {messagePreview ? (
                    ...
                    <div className="affiliate-card-section-copy affiliate-message-content-cleared">
                      {t(
                        "ecommerce.affiliateWorkspace.proposalExecutionDescriptions.SEND_MESSAGE_CONTENT_CLEARED",
                      )}
                    </div>
                  ) : (
                    <div className="affiliate-card-section-copy">
              ...
              <ProposalPredictionComparison snapshot={predictionSnapshot} />
          ...
          <aside
            className="affiliate-proposal-row-decision"
  ...
  return (
    ...
    >
      <div className="affiliate-work-item-head">
        ...
        <div className="affiliate-work-item-badges">{statusBadge}</div>
      </div>
          ...
          <li
            ...
            className={
              version.id === currentProposalId
                ? "affiliate-work-bundle-revision-current"
                : undefined
            }
          >
            <div className="affiliate-work-bundle-revision-meta">
              ...
              <span>
                {t(`ecommerce.affiliateWorkspace.proposalFilters.${version.status}`, {
...
export function summarizeSampleProposalReviewRows(rows: AffiliateSampleProposalReviewRow[]): {
  approveCount: number;
  rejectCount: number;
} {
  return rows.reduce(
...
export function sampleReviewDisposition(intent: {
  decision?: GQL.AffiliateSampleReviewDecision | null;
  executionMode?: GQL.AffiliateSampleReviewExecutionMode | null;
}): "APPROVE" | "REJECT" | "IGNORE" {
  if (intent.executionMode === GQL.AffiliateSampleReviewExecutionMode.AllowPlatformExpiry) {
    return "IGNORE";
  }
  return intent.decision === GQL.AffiliateSampleReviewDecision.Approve ? "APPROVE" : "REJECT";
}

export function sampleReviewOverrideOptions(
  proposal: GQL.ActionProposal,
): GQL.ActionProposalSampleReviewOverrideInput[] {
  if (proposalSampleDecisionOverrideTarget(proposal) == null) return [];
  const source = proposal.steps?.length ? proposal.steps[0]! : proposal;
  const suggested = sampleReviewDisposition(source.sampleReviewIntent!);
  return [
    {
  ...
  ].filter((option) => sampleReviewDisposition(option) !== suggested);
}
...
export function proposalSampleDecisionOverrideTarget(
  ...
  if (executionMode === GQL.AffiliateSampleReviewExecutionMode.AllowPlatformExpiry) {
...
function actionProposalDecisionNote(
  t: ReturnType<typeof useTranslation>["t"],
  proposal: GQL.ActionProposal,
  status: GQL.ActionProposalStatus,
  note?: string,
...
function actionProposalDecisionSuccessMessage(
  t: ReturnType<typeof useTranslation>["t"],
  proposal: GQL.ActionProposal,
  status: GQL.ActionProposalStatus,
  override?: GQL.ActionProposalSampleReviewOverrideInput,
...
function isPureSampleReviewProposalSource(source: {
  ...
  return (
    source.type === GQL.ActionProposalType.ReviewSampleApplication &&
...
export function proposalSampleReviewRows(
    ...
    sampleSteps.length > 0
      ? sampleSteps
          .filter(
            (
              step,
            ): step is GQL.ActionProposalStep & {
              sampleReviewIntent: GQL.ActionProposalSampleReviewIntent;
            } => Boolean(step.sampleReviewIntent),
          )
          .map((step) => ({
        ...
        ? [
            {
              ...
              sampleApplicationRecordId:
                ...
                proposal.sampleApplicationRecordId ??
                null,
              productId: proposal.productId ?? proposal.sampleApplicationRecord?.productId ?? null,
  ...
  return sources.map((source) => {
    const snapshot = findPredictionSnapshotForSampleSource(snapshots, source, sources.length === 1);
    const productId =
      source.productId ??
    ...
    >
      ...
      <div className="affiliate-sample-decision-list">
        {rows.map((row, index) => {
          ...
          const evidence = evidenceState?.kind === "EVIDENCE" ? evidenceState.evidence : null;
          const highlightTarget = evidence ? predictionEvidenceHighlightTarget(evidence) : "NONE";
          const unavailableLabel = t(
            "ecommerce.affiliateWorkspace.sampleDecisionBundle.unavailable",
          );
...
function ProposalPredictionComparison({
  ...
  if (!evidenceState || evidenceState.kind !== "EVIDENCE") {
    ...
    return (
      <section
        className="affiliate-prediction-comparison"
        aria-label={t("ecommerce.affiliateWorkspace.predictionComparison.title")}
      >
  ...
  const humanDecisionLabel = humanDecisionValue
    ...
    : humanDecisionSignal.status === GQL.AffiliateModelSignalStatus.Ready
      ...
      : (predictionSignalFallbackLabel(humanDecisionSignal, unavailableLabel) ?? unavailableLabel);
  const probability =
    typeof humanDecisionValue?.approvalProbability === "number"
    ...
    >
      ...
      <div className="affiliate-prediction-comparison-grid">
        ...
        >
          ...
          {probability ? (
            <small>
              {t("ecommerce.affiliateWorkspace.predictionComparison.humanApprovalProbability", {
                probability,
              })}
            </small>
          ) : null}
...
export type AffiliatePredictionHighlightTarget = "EXPECTED_SALES" | "HUMAN_DECISION" | "NONE";
...
export function SampleApplicationSummaryCard({
  sampleApplication,
  productSummary,
  shopId,
  allowProductLoad = false,
  embedded = false,
  variant,
}: {
  ...
  const detailFields = [
    ...
    },
    {
      ...
      value:
        sampleApplication.platformCollaborationId ?? sampleApplication.affiliateCollaborationId,
    },
    {
    ...
    },
    {
      ...
      value:
        sampleApplication.latestObservedContentViewCount != null
    ...
    },
    {
      ...
      value: sampleApplication.deliveredAt
        ? formatProposalTime(sampleApplication.deliveredAt)
        : null,
    },
  ].filter(
  ...
  const resolvedVariant = variant ?? (embedded ? "embedded" : "listing");
  const isEmbedded = resolvedVariant === "embedded" || embedded;
    ...
    <article
      className={affiliateEntityCardClassName(
        ...
        [
          ...
          isEmbedded ? "affiliate-collaboration-sample-card-embedded" : "",
    ...
    >
      <div className="affiliate-collaboration-sample-card-head">
        <div>
          ...
          <AffiliateSampleShopIdentity shopId={sampleApplication.shopId ?? shopId} />
          ...
          <div className="affiliate-entity-card-identifiers">
            ...
            <PlatformIdCopy
              value={sampleApplication.platformApplicationId}
              labelKey="ecommerce.affiliateWorkspace.copySamplePlatformId"
            />
          </div>
        </div>
        <div className="affiliate-collaboration-sample-status">
          {sampleApplication.reviewDisposition === GQL.AffiliateSampleReviewDisposition.SoftRejected
      ...
      <div className="affiliate-collaboration-sample-grid">
        ...
        <SampleApplicationFact
          ...
          value={
            trackingNumber || t("ecommerce.affiliateWorkspace.sampleApplication.noTrackingYet")
          }
        />
        <SampleApplicationFact
        ...
        <div className="affiliate-collaboration-sample-details">
          <div className="affiliate-collaboration-sample-details-grid">
            {detailFields.map((field) => (
              <SampleApplicationFact key={field.label} label={field.label} value={field.value} />
            ))}
          </div>
        </div>
      ) : null}
...
type AffiliateSampleApplicationStateQueryData = {
  affiliateSampleApplicationState: GQL.AffiliateSampleApplicationStatePayload;
};

type AffiliateSampleApplicationStateQueryVariables = {
  creatorRelationshipId: string;
  sampleApplicationRecordId: string;
};

/**
 * One current agenda item with entity-aware disclosure.
 *
 * The relationship detail query is the fast path: when it already supplied the
 * Sample Application, expanding this card performs no request. A missing entity
 * is hydrated by its stable relationship + record IDs only after disclosure,
 * keeping the overview resilient without turning the relationship list into an
 * N+1 query surface.
 */
function RelationshipAgendaCard({
  agenda,
  creatorRelationshipId,
  sampleApplication,
  productSummary,
  ...
  agenda: GQL.AffiliateRelationshipAgendaItem;
  creatorRelationshipId: string;
  sampleApplication?: GQL.SampleApplicationRecord | null;
  productSummary?: GQL.EcomProductSummary | null;
  shopLabel: ShopDisplayLabel;
  ...
  const hasSampleEntity = Boolean(
    sampleApplicationRecordId && sampleApplication?.id === sampleApplicationRecordId,
  );
  const [detailsOpen, setDetailsOpen] = useState(Boolean(sampleApplicationRecordId));
  const [loadSampleApplication, sampleState] = useLazyQuery<
    AffiliateSampleApplicationStateQueryData,
    AffiliateSampleApplicationStateQueryVariables
  >(AFFILIATE_SAMPLE_APPLICATION_STATE_QUERY, {
    fetchPolicy: "cache-first",
  });

  useEffect(() => {
    if (!detailsOpen || !sampleApplicationRecordId || hasSampleEntity || sampleState.called) {
      return;
    }
    void loadSampleApplication({
      variables: { creatorRelationshipId, sampleApplicationRecordId },
    });
  }, [
    creatorRelationshipId,
    detailsOpen,
  ...
  const hydratedSample =
    sampleState.data?.affiliateSampleApplicationState.sampleApplication ??
    sampleApplication ??
    null;
  const hasEntityDisclosure = Boolean(sampleApplicationRecordId);

  return (
    <section
      className={`affiliate-relationship-work-current-work affiliate-relationship-agenda-card${
        detailsOpen && hasEntityDisclosure ? " is-expanded" : ""
      }`}
    >
      <div className="affiliate-relationship-work-current-work-main">
        ...
        <h3>
          {t(`ecommerce.affiliateWorkspace.workKinds.${agenda.workKind}`, {
            defaultValue: formatAffiliateEnumLabel(agenda.workKind),
          })}
        </h3>
        <p>
          {agenda.reasons
            ...
            .join(" · ") ||
            affiliateWorkspaceEnumLabel(t, "requiredActions", agenda.requiredAction)}
        </p>
      </div>
      <div className="affiliate-relationship-agenda-card-controls">
        <RelationshipStatusBadge
          display={{
            primary: affiliateWorkspaceEnumLabel(t, "requiredActions", agenda.requiredAction),
            secondary: affiliateWorkspaceEnumLabel(t, "agendaSourceTypes", agenda.sourceType),
          }}
          tone={relationshipStatusTone(relationshipProcessingStatusFromAgendaOwner(agenda.owner))}
        />
        {hasEntityDisclosure ? (
          <button
            className="affiliate-relationship-agenda-disclosure"
            type="button"
      ...
      {detailsOpen && hasEntityDisclosure ? (
        <div className="affiliate-relationship-agenda-entity">
          {sampleState.loading && !hydratedSample ? (
            <div className="affiliate-relationship-agenda-entity-state">{t("common.loading")}</div>
          ) : null}
          {sampleState.error && !hydratedSample ? (
            <div className="affiliate-relationship-agenda-entity-state is-error" role="alert">
              <span>{sampleState.error.message}</span>
              <button
                className="btn btn-secondary btn-sm"
                type="button"
                onClick={() => void sampleState.refetch()}
              >
                {t("common.retry", { defaultValue: "Retry" })}
              </button>
            </div>
          ) : null}
          {!sampleState.loading && !sampleState.error && !hydratedSample ? (
            <div className="affiliate-relationship-agenda-entity-state">
              {t("ecommerce.affiliateWorkspace.sampleApplication.none")}
            </div>
          ) : null}
          {hydratedSample ? (
            <SampleApplicationSummaryCard
              sampleApplication={hydratedSample}
              productSummary={productSummary}
              shopId={agenda.shopId ?? hydratedSample.shopId}
              allowProductLoad
              variant="compact"
            />
          ) : null}
...
function ProposalProductSummary({
  proposal,
  label,
}: {
  proposal: GQL.ActionProposal;
...
function relationshipWorkItemFromProposal(
  ...
  const relationshipId =
    hydratedProposal.creatorRelationshipId ??
    hydratedProposal.sourceWorkBoundary?.creatorRelationshipId;
  if (!relationshipId) return null;
  const projectionCollaborations = (projection?.affiliateCollaborations ??
    []) as GQL.AffiliateCollaboration[];
  const projectionPendingProposals = (
    (projection?.actionProposals ?? []) as GQL.ActionProposal[]
  ).filter((item) => item.status === GQL.ActionProposalStatus.Pending);
  const focusCollaboration = (hydratedProposal.affiliateCollaboration ??
    projectionCollaborations.find(
      (record) => record.id === hydratedProposal.affiliateCollaborationId,
    ) ??
    projectionCollaborations[0] ??
    null) as GQL.AffiliateCollaboration | null;
  const relationship =
    projection?.creatorRelationship ?? hydratedProposal.creatorRelationship ?? null;
  ...
  return {
    ...
    stateUpdatedAt:
      relationship?.stateUpdatedAt ?? hydratedProposal.updatedAt ?? hydratedProposal.createdAt,
    ...
    productContext:
      productContextFromProposal(hydratedProposal) ??
      productContextFromProjection(proposalProjection ?? projection),
...
export function reconcileAgendaProcessingStatusWithPendingProposals(
  ...
  if (hasPendingProposals && status === GQL.AffiliateRelationshipProcessingStatus.AgentRequired) {
    return GQL.AffiliateRelationshipProcessingStatus.StaffRequired;
  }
  return status;
}
...
function relationshipDetailFromWorkItem(
  ...
  return {
    ...
    shopState:
      item.creatorRelation?.shopStates?.find((state) => state.shopId === item.shopId) ?? null,
    managementItem: null,
    workItems: [item],
  };
}
...
function productContextFromProposal(
  ...
  const productId =
    proposal.affiliateCollaboration?.productIds[0] ?? getProposalActionProductId(proposal);
  if (!productId) return null;
  return {
    productId,
...
function CreatorContactPanel({ relationshipId }: { relationshipId: string }) {
  ...
  const [saveEmail, saveEmailState] = useMutation<
    { setAffiliateCreatorEmail: GQL.AffiliateCreatorRelationship },
  ...
  const emailContact = contactState?.channelContacts.find(
    (contact) => contact.channel === GQL.AffiliateMessageChannel.Email,
  );
  const whatsAppContact = contactState?.channelContacts.find(
    (contact) => contact.channel === GQL.AffiliateMessageChannel.Whatsapp,
  );

  useEffect(() => {
    setEmail(emailContact?.creatorEmail ?? "");
    setWhatsApp(whatsAppContact?.creatorPhone ?? "");
  }, [emailContact?.creatorEmail, whatsAppContact?.creatorPhone]);

  async function updateEmail(): Promise<void> {
    const nextEmail = email.trim();
    if (!nextEmail || saveEmailState.loading) return;
    try {
      await saveEmail({
        variables: { input: { creatorRelationshipId: relationshipId, email: nextEmail } },
      });
      await refetch();
      showToast(t("ecommerce.affiliateTeam.saved"), "success");
    } catch (mutationError) {
      showToast(
        mutationError instanceof Error ? mutationError.message : t("ecommerce.updateFailed"),
        "error",
      );
    }
  }

  async function updateWhatsApp(): Promise<void> {
    const creatorPhone = whatsApp.trim();
    if (!creatorPhone || saveWhatsAppState.loading) return;
    try {
      await saveWhatsApp({
        variables: {
          input: { creatorRelationshipId: relationshipId, creatorPhone },
        },
      });
      await refetch();
      showToast(t("ecommerce.affiliateTeam.saved"), "success");
  ...
  return (
    <section className="affiliate-relationship-work-side-card affiliate-creator-contact-panel">
      <div className="affiliate-relationship-work-side-card-head">
        <span>{t("ecommerce.affiliateWorkspace.relationshipContacts")}</span>
        {loading ? <small>{t("common.loading")}</small> : null}
      </div>
      {error ? (
        <button
          className="affiliate-inline-link-button"
        ...
        <>
          <label className="affiliate-creator-contact-field">
          ...
          <div className="affiliate-creator-contact-health">
            <span className={contactState?.hasUsableEmailContact ? "is-ready" : ""}>
              {t("auth.email")} ·{" "}
              {contactState?.hasUsableEmailContact ? t("common.yes") : t("common.no")}
            </span>
            <span className={contactState?.hasUsableWhatsAppContact ? "is-ready" : ""}>
              WhatsApp · {contactState?.hasUsableWhatsAppContact ? t("common.yes") : t("common.no")}
            </span>
          </div>
        </>
      )}
...
export function CreatorRelationshipDetailModal(
  props: ComponentProps<typeof CreatorRelationshipDetailContent>,
) {
  const id =
    props.relationshipId ??
    props.item?.creatorRelation?.id ??
    props.item?.workItems?.[0]?.relationshipId ??
    "";
  // A different Creator starts with global scope and cannot inherit drafts or page cursors.
  return <CreatorRelationshipDetailContent key={id} {...props} />;
}

function CreatorRelationshipDetailContent({
  item,
  relationshipId: relationshipIdOverride,
  selectedShopId,
  initialTab = "overview",
  ...
  const [activeTab, setActiveTab] = useState<
    "profile" | "overview" | "samples" | "platform" | "conversation" | "activity"
  >(initialTab);
  const [scopeShopId, setScopeShopId] = useState("");
  const [pendingScopeShopId, setPendingScopeShopId] = useState<string | null>(null);
  const [contextInspectorOpen, setContextInspectorOpen] = useState(() =>
    typeof window === "undefined" || typeof window.matchMedia !== "function"
      ? true
      : window.matchMedia("(min-width: 1081px)").matches,
  );
  const [contextInspectorSection, setContextInspectorSection] = useState<
    "overview" | "contacts" | "management"
  >("overview");
  ...
  const [optimisticallyDecidedProposalIds, setOptimisticallyDecidedProposalIds] = useState<
    Set<string>
  >(() => new Set());
  const tabPanelRef = useRef<HTMLDivElement>(null);
  const [composerText, setComposerText] = useState("");
  const [composerChannel, setComposerChannel] = useState<"AUTO" | GQL.AffiliateMessageChannel>(
    "AUTO",
  );
  ...
  const [sampleReviewCommand, setSampleReviewCommand] = useState<{
    ...
    reviewDispositionRevision: number;
    kind: "APPROVE" | "PLATFORM_REJECT" | "SOFT_REJECT" | "REOPEN";
  } | null>(null);
  const [sampleRejectReason, setSampleRejectReason] = useState<GQL.AffiliateSampleRejectReason>(
  ...
  useEffect(() => {
    ...
    const syncInspectorWithViewport = () => setContextInspectorOpen(desktopInspector.matches);
    syncInspectorWithViewport();
    desktopInspector.addEventListener("change", syncInspectorWithViewport);
    return () => desktopInspector.removeEventListener("change", syncInspectorWithViewport);
  }, []);
  useEffect(() => {
  ...
  const performance =
    authoritativeProfile?.performance ??
  ...
  useEffect(() => {
    if (developerData)
      affiliateWorkspace.replaceAffiliateBusinessDevelopers(
        developerData.affiliateBusinessDevelopers,
      );
  ...
  const [removeRelationshipProtection, removeRelationshipProtectionState] = useMutation(
    REMOVE_AFFILIATE_CREATOR_RELATIONSHIP_PROTECTION_MUTATION,
  );
  ...
  const ownershipBusy =
    assignDeveloperState.loading ||
    protectRelationshipState.loading ||
    removeRelationshipProtectionState.loading;
  const includedShopIds =
    relationshipDetail?.includedShopIds ?? rawShopStates.map((state) => state.shopId);
  const allShopActivitySummaries =
    relationshipDetail?.shopActivitySummaries ??
    includedShopIds.map((shopId) => ({
      shopId,
      lastContactedAt:
        rawShopStates.find((state) => state.shopId === shopId)?.lastContactedAt ?? null,
  ...
  const shopActivitySummaries = allShopActivitySummaries.filter((summary) =>
    matchesCreatorShop(summary.shopId, scopeShopId),
  );
  const scopedShopSummary = scopeShopId ? shopActivitySummaries[0] : null;
  // Store summaries are TOTAL counts, not active-only counts. Keep labels consistent in both views.
  const scopedSampleCount = scopeShopId
    ? scopedShopSummary?.sampleApplicationCount
    : relationshipDetail?.counts.sampleApplicationCount;
  const scopedCollaborationCount = scopeShopId
    ? scopedShopSummary?.platformCollaborationCount
    : relationshipDetail?.counts.platformCollaborationCount;
  const scopedPendingProposalCount = scopeShopId
    ? scopedShopSummary?.pendingProposalCount
    : relationshipDetail?.counts.pendingProposalCount;
  const scopedLastContactedAt = scopeShopId
    ? scopedShopSummary?.lastContactedAt
    : relationshipDetail?.lastContactedAt;
  const scopedLastBusinessActivityAt = scopeShopId
    ? scopedShopSummary?.lastBusinessActivityAt
    : relationshipDetail?.lastBusinessActivityAt;
  const scopeKey = `${relationshipId ?? ""}:${scopeShopId}`;
  ...
  useEffect(() => {
    if (scopeShopId) {
      setComposerShopId(scopeShopId);
    } else if (includedShopIds.length === 1) {
      setComposerShopId(includedShopIds[0]);
    } else if (!includedShopIds.includes(composerShopId)) {
      setComposerShopId(includedShopIds.includes(selectedShopId) ? selectedShopId : "");
    }
  }, [composerShopId, includedShopIds, selectedShopId, scopeShopId]);

  function applyScope(nextShopId: string): void {
    setPendingScopeShopId(null);
    setScopeShopId(nextShopId);
    setComposerShopId(
      nextShopId || (includedShopIds.includes(selectedShopId) ? selectedShopId : ""),
    );
    setComposerText("");
    setComposerSubject("");
  ...
  function changeScope(nextShopId: string): void {
    if (
      nextShopId === scopeShopId ||
      sendingMessage ||
      uploadingAttachments ||
      decidingProposalId ||
      reviewSampleState.loading ||
      reopenSampleState.loading ||
      (nextShopId && !includedShopIds.includes(nextShopId))
    )
      return;
    if (composerText.trim() || composerSubject.trim() || stagedAttachments.length) {
      setPendingScopeShopId(nextShopId);
    } else applyScope(nextShopId);
  }
  ...
  >(REVIEW_AFFILIATE_SAMPLE_APPLICATION_MUTATION);
  const [reopenSoftRejectedAffiliateSampleApplication, reopenSampleState] = useMutation<
  ...
  const [decideRelationshipActionProposal] = useMutation<
    { decideActionProposal: GQL.ActionProposal },
    { input: GQL.DecideActionProposalInput }
  >(DECIDE_ACTION_PROPOSAL_MUTATION);
  ...
  const relationshipSampleApplications = sortAffiliateSamplesPendingFirst(
    mergeById([
      ...(item?.workItems ?? []).flatMap((workItem) => [
        ...
        ...(workItem.workItem?.sampleApplicationRecord
          ? [workItem.workItem.sampleApplicationRecord]
          : []),
      ]),
      ...(samplePage?.items ?? []),
    ]).filter((sample) => matchesCreatorShop(sample.shopId, scopeShopId)),
  );
  const platformCollaborations = (platformPage?.items ?? []).filter((entry) =>
    matchesCreatorShop(entry.collaboration.shopId, scopeShopId),
  );
  // The list/work-item payload already carries the exact pending proposal that
  // produced the queue row. Use it as an immediate entity snapshot while the
  // detail query catches up; the query result wins when both contain the same ID.
  const relationshipProposals = mergeById([
    ...(item?.workItems ?? []).flatMap((workItem) => [
      ...workItem.pendingProposals,
      ...(workItem.focusedProposal ? [workItem.focusedProposal] : []),
    ]),
    ...(proposalPage?.items ?? []),
  ]).filter((proposal) => matchesCreatorProposalShop(proposal, scopeShopId));
  const visiblePendingProposals = relationshipProposals.filter(
    (proposal) =>
      proposal.status === GQL.ActionProposalStatus.Pending &&
      !optimisticallyDecidedProposalIds.has(proposal.id),
  );
  const historicalProposals = relationshipProposals.filter(
  ...
  const productSummaryFor = (shopId: string, productId: string | null | undefined) =>
    productSummaries.find(
      (entry) => entry.shopId === shopId && entry.product.productId === productId,
    )?.product ?? null;
  const relationshipAgenda = (relationship?.agendaItems ?? []).filter(
    (agenda) =>
      matchesCreatorShop(agenda.shopId, scopeShopId) &&
      (!agenda.shopId || includedShopIds.includes(agenda.shopId)),
  );
  const relationshipSummary = scopeShopId
    ? creatorScopeWorkSummary(relationshipAgenda)
    : relationship?.workSummary;
  ...
  const currentTitle = visiblePendingProposals[0]
    ...
    : relationshipAgenda[0]
      ...
      : !scopeShopId && management?.needsAttention
        ? t("ecommerce.affiliateWorkspace.creatorNeedsAttention")
  ...
  const newestConversationMessageKey = conversationMessages.at(-1)
    ...
    : null;
  useLayoutEffect(() => {
    if (activeTab === "conversation" && conversationScrollRelationshipRef.current !== scopeKey) {
      conversationScrollRelationshipRef.current = scopeKey;
      conversationAutoScrollPendingRef.current = true;
    }
    if (
      activeTab !== "conversation" ||
      !conversationAutoScrollPendingRef.current ||
      conversationLoading ||
    ...
    conversationAutoScrollPendingRef.current = false;
  }, [activeTab, conversationLoading, newestConversationMessageKey, scopeKey]);
  ...
  async function submitComposerMessage(): Promise<void> {
    if (!relationshipId || !composerShopId || (scopeShopId && composerShopId !== scopeShopId))
      return;
    const parts: GQL.AffiliateOutboundMessagePartInput[] = [];
    if (composerText.trim())
      parts.push({ kind: GQL.AffiliateMessagePartKind.Text, text: composerText.trim() });
    parts.push(
    ...
    try {
      const result = await sendAffiliateCreatorMessage({
        variables: {
          input: {
      ...
      if (
        delivery?.status === GQL.AffiliateDeliveryStatus.Failed ||
        delivery?.status === GQL.AffiliateDeliveryStatus.PartiallySent
      ) {
  ...
  async function executeSampleReviewCommand(): Promise<void> {
    if (!sampleReviewCommand) return;
    try {
      if (sampleReviewCommand.kind === "REOPEN") {
        await reopenSoftRejectedAffiliateSampleApplication({
          variables: {
            input: {
              sampleApplicationRecordId: sampleReviewCommand.sampleApplicationRecordId,
              creatorRelationshipId: sampleReviewCommand.creatorRelationshipId,
              projectionRevision: sampleReviewCommand.projectionRevision,
              reviewDispositionRevision: sampleReviewCommand.reviewDispositionRevision,
            },
          },
        });
      } else {
        const rejecting = sampleReviewCommand.kind !== "APPROVE";
        await reviewAffiliateSampleApplication({
          variables: {
            input: {
              sampleApplicationRecordId: sampleReviewCommand.sampleApplicationRecordId,
              creatorRelationshipId: sampleReviewCommand.creatorRelationshipId,
      ...
      onWorkbenchEntityChanged?.();
      showToast(t("ecommerce.affiliateWorkspace.workbench.sampleReviewSaved"), "success");
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error), "error");
    }
  }
  ...
  >(AFFILIATE_RELATIONSHIP_TIMELINE_QUERY, {
    variables: {
      input: {
        creatorRelationshipId: relationshipId ?? "",
        shopIds: scopeShopId ? [scopeShopId] : undefined,
  ...
  async function decideRelationshipProposal(
    proposal: GQL.ActionProposal,
    status: GQL.ActionProposalStatus,
    note?: string,
    sampleReviewOverride?: GQL.ActionProposalSampleReviewOverrideInput,
    ...
    setOptimisticallyDecidedProposalIds((current) => new Set(current).add(proposal.id));

    const restorePendingProposal = () => {
      setOptimisticallyDecidedProposalIds((current) => {
    ...
    try {
      if (onDecideProposal) {
        const handled = await onDecideProposal(proposal, status, note, sampleReviewOverride);
        if (!handled) {
        ...
        const result = await decideRelationshipActionProposal({
          variables: {
            input: {
              ...
              decision: {
                decidedAt,
                note: actionProposalDecisionNote(t, proposal, status, note, sampleReviewOverride),
              },
            },
          },
        });
      ...
      setDecidingProposalId((current) => (current === proposal.id ? null : current));
    }
  }

  const relationshipTimeline = relationshipTimelineData?.affiliateRelationshipTimeline;
  const { data: globalManagementTimelineData, refetch: refetchGlobalManagementTimeline } = useQuery<
    { affiliateRelationshipTimeline: GQL.AffiliateRelationshipTimelinePayload },
    { input: GQL.AffiliateRelationshipTimelineInput }
  >(AFFILIATE_RELATIONSHIP_TIMELINE_QUERY, {
    variables: {
      input: { creatorRelationshipId: relationshipId ?? "", limit: AFFILIATE_TIMELINE_PAGE_SIZE },
    },
    fetchPolicy: "cache-and-network",
    skip: !relationshipId || !scopeShopId || contextInspectorSection !== "management",
  });
  const managementTimelineItems = scopeShopId
    ? (globalManagementTimelineData?.affiliateRelationshipTimeline.items ?? [])
    : (relationshipTimeline?.items ?? []);
  const lastManualTagChange = latestManualTagChange(managementTimelineItems);
  const lastSystemTagChange = latestSystemTagChange(managementTimelineItems);
  const canLoadOlderActivity = Boolean(
    relationshipTimeline?.hasOlder && relationshipTimeline.olderCursor,
  );
  const activityEntries = buildRelationshipTimelineEntries(
    creatorScopeTimeline(relationshipTimeline?.items ?? [], scopeShopId),
  ...
  }, [activeTab, activityEntries.length, scopeKey]);
  const tabItems = [
    {
      id: "overview" as const,
      label: t("ecommerce.affiliateWorkspace.relationshipPanelCurrentWork"),
      count:
        (scopedPendingProposalCount ?? visiblePendingProposals.length) || relationshipAgenda.length,
    },
    {
    ...
    {
      id: "samples" as const,
      label: t("ecommerce.affiliateWorkspace.relationshipPanelSamples", {
        defaultValue: "Samples & fulfillment",
      }),
      count: scopedSampleCount ?? relationshipSampleApplications.length,
    },
    {
      id: "platform" as const,
      label: t("ecommerce.affiliateWorkspace.relationshipPanelPlatformCollaborations", {
        defaultValue: "Platform collaborations",
      }),
      count: scopedCollaborationCount ?? platformCollaborations.length,
    },
    {
      id: "profile" as const,
      label: t("ecommerce.affiliateWorkspace.relationshipPanelProfile", {
    ...
    },
    {
      ...
      count: scopeShopId
        ...
        : (relationshipDetail?.counts.lifecycleEventCount ?? activityEntries.length),
    },
  ];

  async function loadOlderConversationMessages(): Promise<void> {
    if (!canLoadOlderConversation || !relationshipId) return;
    ...
    window.requestAnimationFrame(() => {
      ...
      const currentViewport = conversationViewportRef.current;
      if (!currentViewport) return;
      currentViewport.scrollTop =
        previousScrollTop + Math.max(0, currentViewport.scrollHeight - previousScrollHeight);
    });
  }

  function loadOlderActivity(): void {
    if (!relationshipTimeline?.hasOlder || !relationshipTimeline.olderCursor || !relationshipId)
      return;
    ...
    void platformQuery.fetchMore({
      variables: {
        input: {
          ...
          shopId: scopeShopId || null,
          limit: 25,
          cursor: platformPage.nextCursor,
        },
      },
      updateQuery: (previous, { fetchMoreResult }) => {
        if (!fetchMoreResult) return previous;
  ...
  async function applyRelationshipProtectionChange(): Promise<void> {
    ...
    try {
      if (relationshipProtection) {
        await removeRelationshipProtection({
          variables: { creatorRelationshipId: relationshipId },
        });
      } else {
  ...
  if (relationshipDetailError && !relationshipDetail) {
    return (
      <AffiliateDetailModal
        backdropClassName="affiliate-creator-detail-backdrop"
        className="affiliate-collaboration-modal"
        onClose={onClose}
      >
        <AffiliateQueryErrorState
          error={relationshipDetailError}
          onRetry={() => void refetchRelationshipDetail()}
        />
      </AffiliateDetailModal>
    );
  }
  ...
  return (
    <>
      <AffiliateDetailModal
        backdropClassName="affiliate-creator-detail-backdrop"
        className="affiliate-collaboration-modal affiliate-relationship-detail-modal affiliate-relationship-work-modal"
        ariaLabel={name}
        onClose={onClose}
      >
        <div className="modal-header affiliate-relationship-work-modal-header affiliate-creator-scope-header">
            ...
            <div className="affiliate-collaboration-modal-title-block">
              <div className="affiliate-relationship-detail-title-row">
                ...
                <span
                  ...
                  title={
                    scopeShopId
                      ? `${t("ecommerce.affiliateWorkspace.creatorScope.selectedShop")} · ${t("ecommerce.affiliateWorkspace.sampleTierColumnLabel")}`
                      : t("ecommerce.affiliateWorkspace.sampleTierColumnHint")
                  }
                >
          ...
          <div className="affiliate-relationship-work-modal-header-actions affiliate-creator-scope-actions">
            <CreatorDetailScopeControl
              value={scopeShopId}
            ...
            >
              {t("ecommerce.affiliateWorkspace.relationshipInformation")}
            </button>
            <button
              className="modal-close-btn"
              type="button"
        ...
        <div className="affiliate-collaboration-modal-body affiliate-relationship-workspace-body">
          <AffiliateContextInspector
            className="affiliate-collaboration-context-pane"
            ...
            headerContent={
              <TkTabs
                items={[
                  {
                    id: "overview",
                    label: t("ecommerce.affiliateWorkspace.relationshipInspectorOverview"),
                  },
                  {
                    id: "contacts",
                    label: t("ecommerce.affiliateWorkspace.relationshipInspectorContacts"),
                  },
                  {
                    id: "management",
                    label: t("ecommerce.affiliateWorkspace.relationshipInspectorManagement"),
                  },
                ]}
                value={contextInspectorSection}
                onChange={(value) =>
                  setContextInspectorSection(value as "overview" | "contacts" | "management")
                }
                label={t("ecommerce.affiliateWorkspace.relationshipInformation")}
              />
            }
          >
            {contextInspectorSection === "overview" ? (
              <section className="affiliate-relationship-work-side-card affiliate-relationship-work-side-card-primary">
                <div className="affiliate-relationship-work-side-card-head">
                  ...
                  <TkBadge>
                    {t("ecommerce.affiliateWorkspace.creatorScope.globalInformation")}
                  </TkBadge>
                  {profile ? (
                    <button
                      className="affiliate-inline-link-button"
                ...
                <div className="affiliate-relationship-work-side-meta">
                  ...
                  <span>
                    {t("ecommerce.affiliateWorkspace.creatorDetail.followers")}:{" "}
                    {formatCount(performance?.followerCount)}
                  </span>
                ...
                {marketplaceBio ? (
                  <p className="affiliate-relationship-creator-bio">{marketplaceBio}</p>
                ) : null}
              </section>
            ) : null}
                ...
                <div className="affiliate-relationship-protection-control">
                  ...
                  <strong>
                    {relationshipProtection
                      ? t("ecommerce.affiliateWorkspace.relationshipProtected")
                      : t("ecommerce.affiliateWorkspace.relationshipAiEnabled")}
                  </strong>
                  <button
                    className="btn btn-secondary btn-sm"
                    type="button"
                  ...
                  {relationshipProtection?.note ? (
                    <small>{relationshipProtection.note}</small>
                  ) : null}
                </div>
                {relationshipOwner?.agentAssistanceMode ===
                GQL.AffiliateAgentAssistanceMode.HumanOnly ? (
                  <small>
                    {t("ecommerce.affiliateWorkspace.relationshipHumanOnlyHint", {
                      name: relationshipOwner.displayName,
                    })}
                  </small>
                ) : null}
              </section>
            ...
            {contextInspectorSection === "management" && relationshipId ? (
              <AffiliateCreatorManualTagEditor
                ...
                onChanged={() => {
                  ...
                  if (scopeShopId) void refetchGlobalManagementTimeline();
                }}
              />
            ) : null}
            {contextInspectorSection === "overview" ? (
              <section className="affiliate-relationship-work-side-card">
                ...
                {relationshipProtection ? (
                  <div className="affiliate-relationship-protection-banner">
                    {t("ecommerce.affiliateWorkspace.protectionDispatchBlocked", {
                      defaultValue:
                        "This Creator is protected. The work remains visible for staff, but AI dispatch is blocked.",
                    })}
                  </div>
                ) : null}
                <RelationshipStatusBadge
                  display={relationshipStatusDisplay}
                ...
                <div className="affiliate-relationship-work-side-facts">
                  <SampleApplicationFact
                    ...
                    value={scopedPendingProposalCount ?? visiblePendingProposals.length}
                  />
                  <SampleApplicationFact
                    label={t("ecommerce.affiliateWorkspace.relationshipPanelSamples")}
                    value={scopedSampleCount ?? "—"}
                  />
                  <SampleApplicationFact
                    label={t(
                      "ecommerce.affiliateWorkspace.relationshipPanelPlatformCollaborations",
                      {
                        defaultValue: "Active platform collaborations",
                      },
                    )}
                    value={scopedCollaborationCount ?? "—"}
                  />
                  <SampleApplicationFact
                    label={`${t("ecommerce.affiliateWorkspace.creatorBlocked")} · ${t("ecommerce.affiliateWorkspace.creatorScope.globalInformation")}`}
                  ...
                  <SampleApplicationFact
                    ...
                    value={scopedLastContactedAt ? formatProposalTime(scopedLastContactedAt) : "—"}
                  />
                  <SampleApplicationFact
                    label={t("ecommerce.affiliateWorkspace.creatorLastBusinessActivityAt", {
                      defaultValue: "Last business activity",
                    })}
                    value={
                      scopedLastBusinessActivityAt
                        ? formatProposalTime(scopedLastBusinessActivityAt)
                        : "—"
                    }
                  />
                </div>
            ...
            {contextInspectorSection === "overview" && shopActivitySummaries.length > 0 ? (
              <section className="affiliate-relationship-work-side-card">
                <div className="affiliate-relationship-work-side-card-head">
                ...
                <div className="affiliate-relationship-shop-state-list">
                  {shopActivitySummaries.slice(0, 4).map((summary) => (
                    <div className="affiliate-relationship-shop-state" key={summary.shopId}>
                      ...
                      <span className="affiliate-relationship-shop-tier">
                        ...
                        {creatorSampleTierDisplay(
                          t,
                          rawShopStates.find((state) => state.shopId === summary.shopId)
                            ?.sampleTier,
                        )}
                      </span>
                      <span>
                        {t("ecommerce.affiliateWorkspace.creatorLastContactedAt", {
                          defaultValue: "Last contacted",
                        })}
                        :{" "}
                      ...
                      <span>
                        {t("ecommerce.affiliateWorkspace.creatorLastBusinessActivityAt", {
                          defaultValue: "Last business activity",
                        })}
                        :{" "}
                        {summary.lastBusinessActivityAt
                          ? formatProposalTime(summary.lastBusinessActivityAt)
                          : t("ecommerce.affiliateWorkspace.creatorScope.noRecentActivity")}
                      </span>
                      <span>
                        {t("ecommerce.affiliateWorkspace.creatorScope.shopCounts", {
                  ...
                  {shopActivitySummaries.length > 4 ? (
                    <div className="affiliate-relationship-shop-state affiliate-relationship-shop-state-more">
                      {t("ecommerce.affiliateWorkspace.relationshipMoreShopStates", {
                        count: shopActivitySummaries.length - 4,
                      })}
                    </div>
                  ) : null}
                </div>
          ...
          <section className="affiliate-collaboration-work-pane">
            ...
            />
            <div
              className={`affiliate-collaboration-tab-panel${
            ...
            >
              {activeTab === "profile" ? (
                <>
                  <CreatorGlobalInformation profile />
                  <CreatorProfilePanel
                    profile={profile}
              ...
              {activeTab === "overview" ? (
                <div className="affiliate-relationship-work-overview-panel">
                  {relationshipDetailLoading && !relationshipDetail ? (
                    <AffiliateLoadingState />
                  ) : null}
                  {proposalQuery.error ? (
                    <AffiliateQueryErrorState
                  ...
                  {relationshipAgenda.length === 0 && visiblePendingProposals.length === 0 ? (
                    <section className="affiliate-relationship-work-current-work">
                      ...
                      <div className="affiliate-relationship-work-current-work-actions">
                        <div>
                          <span>{t("ecommerce.affiliateWorkspace.relationshipPanelSamples")}</span>
                          <strong>{scopedSampleCount ?? "—"}</strong>
                          <small>
                            {scopeShopId
                              ? t("ecommerce.affiliateWorkspace.creatorScope.selectedShop")
                              : t("ecommerce.affiliateWorkspace.relationshipAcrossShops")}
                          </small>
                        </div>
                        <div>
                          <span>
                            {t("ecommerce.affiliateWorkspace.relationshipPanelCommunication")}
                          </span>
                          <strong>{conversationMessages.length}</strong>
                          <small>
                            {t("ecommerce.affiliateWorkspace.relationshipCommunicationHint")}
                          </small>
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="affiliate-relationship-work-overview-section">
                      ...
                      <div className="affiliate-relationship-work-overview-proposal-list">
                        {relationshipAgenda.map((agenda) => {
                          const sampleApplication = agenda.sampleApplicationRecordId
                            ? relationshipSampleApplications.find(
                                (sample) => sample.id === agenda.sampleApplicationRecordId,
                              )
                            : null;
                          return (
                            <RelationshipAgendaCard
                              key={agenda.key}
                              agenda={agenda}
                              creatorRelationshipId={relationshipId ?? ""}
                              sampleApplication={sampleApplication}
                              productSummary={productSummaryFor(
                                agenda.shopId ?? sampleApplication?.shopId ?? "",
                                agenda.productId ?? sampleApplication?.productId,
                              )}
                              shopLabel={
                                agenda.shopId
                    ...
                    <section className="affiliate-relationship-work-overview-section">
                      ...
                      <div className="affiliate-relationship-work-overview-proposal-list">
                        {visiblePendingProposals.map((proposal) =>
                          creatorProposalRequiresGlobalView(proposal, scopeShopId) ? (
                            <TkAlert
                              key={proposal.id}
                              title={t(
                                "ecommerce.affiliateWorkspace.creatorScope.crossShopProposal",
                              )}
                              actions={
                                <TkButton variant="secondary" onClick={() => changeScope("")}>
                                  {t("ecommerce.affiliateWorkspace.creatorScope.backToGlobal")}
                                </TkButton>
                              }
                            >
                              {t("ecommerce.affiliateWorkspace.creatorScope.crossShopProposalHint")}
                            </TkAlert>
                          ) : (
                            <AgentWorkBundleCard
                              key={proposal.id}
                              ...
                              shopLabel={{
                                ...(scopeShopId
                                  ? relationshipShopName(scopeShopId)
                                  : {
                                      text: t(
                                        "ecommerce.affiliateWorkspace.relationshipAcrossShops",
                                      ),
                  ...
                  {historicalProposals.length > 0 ? (
                    <section className="affiliate-relationship-work-overview-section affiliate-relationship-proposal-history-section">
                      <button
                        className="affiliate-relationship-proposal-history-toggle"
                        type="button"
                      ...
                      >
                        <span>
                          <strong>
                            {t("ecommerce.affiliateWorkspace.relationshipProposalHistory")}
                          </strong>
                          <small>{historicalProposals.length}</small>
                        </span>
                        <span
                          className="affiliate-relationship-proposal-history-chevron"
                          aria-hidden="true"
                        >
                          ⌄
                        </span>
                        ...
                        <div className="affiliate-relationship-work-overview-proposal-list">
                          {historicalProposals.map((proposal) =>
                            creatorProposalRequiresGlobalView(proposal, scopeShopId) ? (
                              ...
                              >
                                {t(
                                  "ecommerce.affiliateWorkspace.creatorScope.crossShopProposalHint",
                                )}
                              </TkAlert>
                            ) : (
                              <AgentWorkBundleCard
                  ...
                  {proposalPage?.hasMore ? (
                    <button className="btn btn-secondary" type="button" onClick={loadMoreProposals}>
              ...
              {activeTab === "samples" ? (
                <div className="affiliate-relationship-work-collaboration-detail-list affiliate-relationship-sample-list">
                  {sampleQuery.error ? (
                    <AffiliateQueryErrorState
                  ...
                  {sampleQuery.loading && relationshipSampleApplications.length === 0 ? (
                    <AffiliateLoadingState />
                  ) : null}
                  {relationshipSampleApplications.map((sample) => {
                    const canPlatformReview =
                      sample.sampleWorkStatus === GQL.SampleWorkStatus.RequestPendingReview;
                    const commandBase = {
                      sampleApplicationRecordId: sample.id,
                      creatorRelationshipId: sample.creatorRelationshipId,
                      projectionRevision: sample.projectionRevision,
                      reviewDispositionRevision: sample.reviewDispositionRevision,
                    };
                    return (
                      <TkPanel
                        className="affiliate-workbench-sample-review-card"
                        variant="framed"
                        padding="sm"
                        key={sample.id}
                      >
                          ...
                          GQL.AffiliateSampleReviewDisposition.SoftRejected ? (
                            canPlatformReview ? (
                              <TkButton
                                variant="secondary"
                                onClick={() =>
                                  setSampleReviewCommand({ ...commandBase, kind: "REOPEN" })
                                }
                              >
                  ...
                  {!sampleQuery.loading &&
                  !sampleQuery.error &&
                  relationshipSampleApplications.length === 0 ? (
                    <div className="affiliate-proposal-empty">
                      {t("ecommerce.affiliateWorkspace.sampleApplication.none")}
                    </div>
                  ) : null}
                  {samplePage?.hasMore ? (
                    <button className="btn btn-secondary" type="button" onClick={loadMoreSamples}>
                      {t("common.loadMore", { defaultValue: "Load more" })}
                    </button>
                  ) : null}
                </div>
              ...
              {activeTab === "platform" ? (
                <div className="affiliate-relationship-work-collaboration-detail-list affiliate-relationship-platform-list">
                  {platformQuery.error ? (
                    <AffiliateQueryErrorState
                  ...
                  {platformQuery.loading && platformCollaborations.length === 0 ? (
                    <AffiliateLoadingState />
                  ) : null}
                  {platformCollaborations.map((entry) => (
                    <RelationshipPlatformCollaborationCard
                      ...
                      productSummaries={(entry.collaboration.productIds ?? [])
                        .map((productId) =>
                          productSummaryFor(entry.collaboration.shopId, productId),
                        )
                        .filter((product): product is GQL.EcomProductSummary => Boolean(product))}
                    />
                  ))}
                  {!platformQuery.loading &&
                  !platformQuery.error &&
                  platformCollaborations.length === 0 ? (
                    <div className="affiliate-proposal-empty">
                      {t("ecommerce.affiliateWorkspace.relationshipWorkNoCollaborations")}
                    </div>
                  ) : null}
                  {platformPage?.hasMore ? (
                    <button
                      className="btn btn-secondary"
                      type="button"
                      onClick={loadMorePlatformCollaborations}
                    >
                      {t("common.loadMore", { defaultValue: "Load more" })}
                    </button>
                  ) : null}
              ...
              {activeTab === "conversation" ? (
                <div className="affiliate-conversation-tab">
                  <div className="affiliate-conversation-preview" ref={conversationViewportRef}>
                    {scopeShopId ? (
                      <p className="affiliate-creator-scope-note">
                        {t("ecommerce.affiliateWorkspace.creatorScope.sharedMessagesHint")}
                      </p>
                    ) : null}
                    {canLoadOlderConversation ? (
                      <TkButton
                        variant="secondary"
                        className="affiliate-conversation-load-more"
                    ...
                    {conversationError ? (
                      <AffiliateQueryErrorState
                        error={conversationError}
                        onRetry={() => void refetchConversationMessages()}
                      />
                    ) : null}
                    {conversationError ? null : conversationLoading &&
                      conversationMessages.length === 0 ? (
                      ...
                      <div className="affiliate-proposal-empty">
                        {t(
                          canLoadOlderConversation
                            ? "ecommerce.affiliateWorkspace.creatorScope.noLoadedMessages"
                            : "ecommerce.affiliateWorkspace.conversation.noMessages",
                        )}
                      </div>
                    ) : (
                      <>
                  ...
                  <div className="affiliate-message-composer">
                    <div className="affiliate-message-composer-head">
                      <div>
                        <strong>{t("ecommerce.affiliateWorkspace.messageComposerTitle")}</strong>
                        <span>{t("ecommerce.affiliateWorkspace.messageComposerPartsHint")}</span>
                      </div>
                    </div>
                    <div className="affiliate-message-composer-route">
                      <Select
                        value={composerShopId}
                        onChange={setComposerShopId}
                        disabled={Boolean(scopeShopId)}
                        ariaLabel={t("ecommerce.affiliateWorkspace.selectMessageShop")}
                        options={(scopeShopId ? [scopeShopId] : includedShopIds).map((shopId) => {
                      ...
                      <Select
                        ...
                        options={[
                          {
                            value: "AUTO",
                            label: t("ecommerce.affiliateWorkspace.messageComposerDefaultChannel"),
                          },
                          {
                          ...
                          },
                          {
                            ...
                            label: t("ecommerce.affiliateWorkspace.messageChannels.EMAIL"),
                          },
                          {
                            value: GQL.AffiliateMessageChannel.PlatformChat,
                            label: t("ecommerce.affiliateWorkspace.messageChannels.PLATFORM_CHAT"),
                          },
                        ]}
                      ...
                      {composerChannel === GQL.AffiliateMessageChannel.Email ? (
                        <input
                          ...
                          placeholder={t(
                            "ecommerce.affiliateWorkspace.messageComposerEmailSubject",
                          )}
                        />
                      ) : null}
                    </div>
                          ...
                          <div className="affiliate-message-staged-item" key={asset.draftAssetId}>
                                ...
                                <input
                                  ...
                                  onChange={(event) =>
                                    setStagedAttachments((current) =>
                                      current.map((item) =>
                                        item.draftAssetId === asset.draftAssetId
                            ...
                            <button
                              ...
                              onClick={() =>
                                setStagedAttachments((current) =>
                                  current.filter(
                                    (item) => item.draftAssetId !== asset.draftAssetId,
                                  ),
                                )
                              }
                            >
                    ...
                    <div className="affiliate-message-composer-footer">
                      <div className="affiliate-message-composer-footer-tools">
                        <label className="btn btn-secondary btn-sm affiliate-message-file-button">
                          {uploadingAttachments
                            ? t("common.loading")
                            : t("ecommerce.affiliateWorkspace.messageComposerAddFiles")}
                          <input
                            type="file"
                            multiple
                        ...
                        <span>
                          {t("ecommerce.affiliateWorkspace.messageComposerAttachmentCount", {
              ...
              {activeTab === "activity" ? (
                <div className="affiliate-collaboration-timeline">
                  <div className="affiliate-timeline-legend" aria-hidden="true">
                    <span className="is-creator">
                      {t("ecommerce.affiliateWorkspace.historyActors.CREATOR_ACTION")}
                    </span>
                    <span className="is-system">
                      {t("ecommerce.affiliateWorkspace.historyActors.SYSTEM_RECORD")}
                    </span>
                    <span className="is-operator">
                      {t("ecommerce.affiliateWorkspace.historyActors.STAFF_ACTION")}
                    </span>
                  </div>
                  {relationshipTimelineError ? (
                    <AffiliateQueryErrorState
                      error={relationshipTimelineError}
                      onRetry={() => void refetchRelationshipTimeline()}
                    />
                  ) : null}
                  {canLoadOlderActivity ? (
                    <button
                  ...
                  {relationshipTimelineLoading && activityEntries.length === 0 ? (
                    ...
                    activityEntries.map((entry) => (
                      <RelationshipTimelineEntry key={entry.id} entry={entry} />
                    ))
                  ) : (
                    <div className="affiliate-proposal-empty">
      ...
      <ConfirmDialog
        isOpen={pendingScopeShopId !== null}
        title={t("ecommerce.affiliateWorkspace.creatorScope.changeView")}
        message={t("ecommerce.affiliateWorkspace.creatorScope.discardDraftHint")}
        confirmLabel={t("ecommerce.affiliateWorkspace.creatorScope.discardAndSwitch")}
        cancelLabel={t("common.cancel")}
        onCancel={() => setPendingScopeShopId(null)}
        onConfirm={() => {
          if (pendingScopeShopId !== null) applyScope(pendingScopeShopId);
        }}
      />
      <Modal
      ...
      >
        <div className="affiliate-workbench-review-dialog">
          ...
          sampleReviewCommand?.kind === "SOFT_REJECT" ? (
            <>
              <label className="affiliate-filter-field">
                <span>{t("ecommerce.affiliateWorkspace.workbench.rejectReason")}</span>
                <Select
                  value={sampleRejectReason}
                  onChange={(value) =>
                    setSampleRejectReason(value as GQL.AffiliateSampleRejectReason)
                  }
                  options={Object.values(GQL.AffiliateSampleRejectReason).map((reason) => ({
                    value: reason,
                    label: t(`ecommerce.affiliateWorkspace.rejectReasons.${reason}`),
                  }))}
                />
              </label>
              {sampleRejectReason === GQL.AffiliateSampleRejectReason.Other ? (
                <textarea
                  className="form-input"
                  value={sampleRejectExplanation}
                  onChange={(event) => setSampleRejectExplanation(event.target.value)}
                  placeholder={t("ecommerce.affiliateWorkspace.workbench.rejectExplanation")}
                  rows={3}
                />
              ) : null}
            </>
          ...
          <div className="modal-actions">
            ...
            <button
              className={`btn ${sampleReviewCommand?.kind === "PLATFORM_REJECT" ? "btn-danger" : "btn-primary"}`}
              type="button"
              disabled={
                reviewSampleState.loading ||
                reopenSampleState.loading ||
              ...
              onClick={() => void executeSampleReviewCommand()}
            >
              {reviewSampleState.loading || reopenSampleState.loading
                ? t("common.loading")
                : t("common.confirm")}
            </button>
          </div>
      ...
      <ConfirmDialog
        ...
        title={
          pendingOwnershipConfirmation?.kind === "OWNER"
...
function CreatorProfilePanel({
  ...
  const categorySummary = categoryIds?.length ? categoryIds.slice(0, 8).join(", ") : null;
  ...
  return (
    <div className="affiliate-creator-profile-panel">
      <div className="affiliate-creator-profile-panel-hero affiliate-creator-profile-toolbar">
        <div className="affiliate-creator-profile-toolbar-copy">
          <span>{t("ecommerce.affiliateWorkspace.relationshipPanelProfile")}</span>
          <strong>{t("ecommerce.affiliateWorkspace.creatorDetail.marketplacePerformance")}</strong>
        </div>
        <div className="affiliate-creator-profile-freshness">
          <span
            className={`affiliate-creator-freshness-badge${payload?.refreshErrorCode ? " warning" : ""}`}
          >
            {freshnessLabel}
          </span>
          <button
          ...
          >
            {loading
              ? t("common.loading")
              : t("ecommerce.affiliateWorkspace.creatorDetail.refresh", {
      ...
      {payload?.refreshErrorMessage ? (
        <div className="affiliate-creator-profile-warning">
          <strong>
            {t("ecommerce.affiliateWorkspace.creatorDetail.refreshFailed", {
      ...
      <div className="affiliate-creator-profile-dashboard">
        <div className="affiliate-creator-profile-main">
          <section className="affiliate-creator-detail-section affiliate-creator-profile-performance">
            <div className="affiliate-card-section-label">
        ...
        <aside className="affiliate-creator-profile-sidebar">
          <section className="affiliate-creator-detail-section affiliate-creator-profile-facts">
            <div className="affiliate-card-section-label">
              {t("ecommerce.affiliateWorkspace.creatorDetail.profileFacts")}
            </div>
            <div className="affiliate-creator-detail-grid affiliate-creator-profile-fact-grid">
              ...
              />
              <CreatorDetailMetric
                label={t("ecommerce.affiliateWorkspace.creatorDetail.market")}
              ...
              />
              <CreatorDetailMetric
                label={t("ecommerce.affiliateWorkspace.creatorDetail.sourceShop")}
        ...
        <div className="affiliate-creator-profile-footer-grid">
          <section className="affiliate-creator-detail-section">
            <div className="affiliate-card-section-label">
          ...
          <section className="affiliate-creator-detail-section">
            ...
            <div className="affiliate-creator-detail-copy">
              {categorySummary
                ? t("ecommerce.affiliateWorkspace.creatorDetail.categorySummary", {
                    categories: categorySummary,
                  })
                : t("ecommerce.affiliateWorkspace.creatorDetail.noSignals")}
            </div>
          </section>
          <section className="affiliate-creator-detail-section affiliate-creator-profile-identifiers">
            <div className="affiliate-card-section-label">
            ...
            <div className="affiliate-creator-detail-id-list">
              ...
              {profile.creatorImId ? (
                <CreatorDetailCopyRow
                  label={t("ecommerce.affiliateWorkspace.creatorDetail.creatorImId")}
              ...
              {profile.profileTtUri ? (
                <CreatorDetailCopyRow
                  label={t("ecommerce.affiliateWorkspace.creatorDetail.profileUri")}
...
function formatPerformanceMoney(
  ...
  if (metric.minimumAmount != null && metric.maximumAmount != null) {
    const minText =
      formatCreatorMoney(String(metric.minimumAmount), metric.currency) ??
      String(metric.minimumAmount);
    const maxText =
      formatCreatorMoney(String(metric.maximumAmount), metric.currency) ??
...
function CreatorDetailCopyRow({
  ...
  return (
    <div
      className={`affiliate-creator-detail-id-row${muted ? " affiliate-creator-detail-id-row-muted" : ""}`}
    >
      <span>{label}</span>
      <CopyInlineValue
        value={value}
...
function CopyInlineValue({
  ...
  async function copyValue(event: MouseEvent<HTMLButtonElement>) {
    ...
    try {
      ...
      showToast(
        err instanceof Error ? err.message : t("ecommerce.affiliateWorkspace.copyFailed"),
        "error",
      );
    }
  }
...
export function getProposalActionProductId(proposal: GQL.ActionProposal | null): string | null {
  ...
  const directProductId =
    proposal.messageIntent?.parts.find((part) => part.productId)?.productId ??
  ...
  for (const step of proposal.steps ?? []) {
    const stepProductId =
      step.messageIntent?.parts.find((part) => part.productId)?.productId ??
...
function findProposalPredictionSnapshot(
  ...
  if (!snapshots.length) return null;
  return (
    sortPredictionSnapshotsByCaptureTime(snapshots as AffiliatePredictionSnapshotView[])[0] ?? null
  );
}
...
export function latestSystemTagChange(
  items: readonly GQL.AffiliateRelationshipTimelineItem[],
): CreatorSystemTagChange | null {
  let latest: CreatorSystemTagChange | null = null;
  for (const item of items) {
    const eventType = item.businessEvent?.eventType ?? item.actionEvent?.eventType ?? null;
    if (
      eventType !== GQL.AffiliateLifecycleEventType.CreatorSystemTagAdded &&
      eventType !== GQL.AffiliateLifecycleEventType.CreatorSystemTagRemoved
    ) {
      continue;
    }
    if (latest && latest.occurredAt >= item.occurredAt) continue;
    latest = {
      occurredAt: item.occurredAt,
      added: eventType === GQL.AffiliateLifecycleEventType.CreatorSystemTagAdded,
      actorType: item.actorType ?? null,
      summary: item.summary,
    };
  }
...
function getProposalMessagePreview(proposal: GQL.ActionProposal): string | null {
  const directText = proposal.messageIntent?.parts
    .find((part) => part.kind === GQL.AffiliateMessagePartKind.Text)
    ?.text?.trim();
  if (directText) return directText;
  for (const step of proposal.steps ?? []) {
    const text = step.messageIntent?.parts
      .find((part) => part.kind === GQL.AffiliateMessagePartKind.Text)
      ?.text?.trim();
    if (text) return text;
  }
...
function renderProposalPreview(
  ...
  if (proposal.sampleReviewIntent) {
    return t("ecommerce.shopDrawer.affiliate.sampleReviewPreview", {
      decision: t(
        `ecommerce.shopDrawer.affiliate.sampleReviewDecisions.${proposal.sampleReviewIntent.decision}`,
        {
  ...
  if (proposal.sampleShipmentIntent) {
    return t("ecommerce.shopDrawer.affiliate.sampleShipmentPreview", {
      applicationId:
        proposal.sampleShipmentIntent.platformApplicationId ??
