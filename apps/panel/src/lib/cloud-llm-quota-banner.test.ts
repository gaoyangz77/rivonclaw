...
describe("resolveCloudLlmQuotaBannerState", () => {
  it("does not warn when the active key is not RivonClaw cloud LLM", () => {
    expect(
      resolveCloudLlmQuotaBannerState(
  ...
  it("returns an error when the billing decision reports quota exceeded", () => {
    expect(resolveCloudLlmQuotaBannerState(cloudKey, overview([], "QUOTA_EXCEEDED"))).toMatchObject(
      { severity: "error" },
    );
  });
  ...
  it("ignores unrelated quota windows", () => {
    expect(
      resolveCloudLlmQuotaBannerState(cloudKey, overview([{ window: "DAY", remainingPercent: 0 }])),
    ).toBeNull();
  });
});
