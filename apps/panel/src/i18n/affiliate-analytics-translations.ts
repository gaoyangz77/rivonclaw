...
const englishAnalytics = {
  ...
  coverage: {
    ...
    boundary: "Series start on {{date}} — the first day all {{shops}} shops with data were present. Earlier days were measured over fewer shops, so they are not comparable with later ones.",
    partialDays: "{{count}} earlier days exist in this window, but each covers only part of the shop set.",
    showPartial: "Show the partial range anyway",
    hidePartial: "Hide the partial range",
    excludeLimiting: "Exclude the {{count}} latest-starting shops",
    limitingShops: "Latest to start: {{shops}}",
    dashedNote: "The partial range is drawn dashed and faint. Its shop set changes from day to day, so it records what was collected — it is not a trend to compare.",
  ...
  reachout: {
    ...
    horizonNote: "Each horizon counts only invitations old enough for it, so the rate is comparable across horizons while the raw response counts are not.",
  ...
  postApproval: {
    ...
    axis: "Cohort axis: the application date · measured in units",
    approvedApplications: "Approved applications",
    orderRate: "Applications with an order",
    ...
    unitsPerApprovedProjected: "{{value}} projected units per approved application",
    cohortsTitle: "Units by cohort day",
    cohortsNote: "{{count}} cohorts are still accruing; their projected top-up is drawn in a lighter tone above the units already observed.",
    actualUnitsSeries: "Units observed",
    projectedUnitsSeries: "Projected remaining",
    maturationTitle: "Unit maturation curve",
    maturationNote: "Share of 60-day units realised by each lag day. This curve is the completion factor behind the projection above, so the two reconcile by construction.",
    maturationSeries: "Cumulative share",
    lagDay: "D+{{count}}",
    basisCohorts: "Basis cohorts: {{count}}",
  },
...
const englishTutorial = {
  ...
  postApprovalBody: "Units never go missing, so this stage uses units rather than GMV. A young cohort carries a lighter projected top-up drawn from the maturation curve beside it.",
...
export const AFFILIATE_ANALYTICS_TRANSLATIONS = {
  ...
  zh: resource({
    ...
    coverage: {
      ...
      boundary: "曲线自 {{date}} 起绘制——这是有数据的 {{shops}} 家店铺全部到齐的第一天。更早的日子只统计到其中一部分店铺，与后面的日子不可比。",
      partialDays: "该时间窗内还有 {{count}} 天，但每天都只覆盖了部分店铺。",
      showPartial: "仍要查看不完整区间",
      hidePartial: "隐藏不完整区间",
      excludeLimiting: "排除最晚开始的 {{count}} 家店铺",
      limitingShops: "最晚开始的店铺：{{shops}}",
      dashedNote: "不完整区间以虚线、浅色绘制。它的店铺集合逐日变化，只说明当时采到了什么，不能当作趋势比较。",
      partialSeries: "不完整区间",
    ...
    reachout: {
      ...
      horizonNote: "每个时长只统计已满足该时长的定邀，因此只有回应率可跨时长比较，原始回应数不可以。",
    ...
    postApproval: {
      ...
      projectedUnits: "预计件数",
      unitsPerApprovedProjected: "预计每个已通过申请 {{value}} 件",
      cohortsTitle: "按申请日的件数",
      cohortsNote: "{{count}} 个队列仍在累积，其预计补足量以浅色叠加在已观测件数之上。",
      actualUnitsSeries: "已观测件数",
      projectedUnitsSeries: "预计补足",
      maturationTitle: "件数成熟曲线",
      maturationNote: "60 天件数在各滞后天数上的累计占比。该曲线就是上方预测所用的完成度因子，两图按构造一致。",
      maturationSeries: "累计占比",
      lagDay: "D+{{count}}",
      basisCohorts: "参与统计的队列数：{{count}}",
    },
    ...
    postApprovalBody: "件数不会缺失，因此这一段用件数而非 GMV。年轻队列会叠加一段浅色预计补足量，其依据就是旁边的成熟曲线。",
  ...
  th: resource({ title: "การวิเคราะห์ Affiliate", subtitle: "สามส่วนแบบคอฮอร์ต แต่ละส่วนมีแกนเวลาของตนเอง: การเชิญ การอนุมัติตัวอย่าง และจำนวนชิ้นหลังอนุมัติ", overview: "ภาพรวม", region: "ภูมิภาคร้าน", allRegions: "ทุกภูมิภาค", selectedShops: "เลือก {{count}} ร้าน", startDate: "วันที่เริ่ม", endDate: "วันที่สิ้นสุด", window: "ช่วงคอฮอร์ต", windowDays: "{{count}} วันล่าสุด", granularity: "ความละเอียดเวลา", refresh: "รีเฟรช", platformTitle: "ผลงานแพลตฟอร์ม", sampleTitle: "การแปลงตัวอย่าง", explore: { title: "สำรวจ", contract: "สัญญา", chooseContract: "เลือกหนึ่งสัญญา", metrics: "เมตริก", groupBy: "จัดกลุ่มตาม", filters: "ตัวกรองมิติ", resultTable: "ตารางผลลัพธ์" } }, {
    welcomeTitle: "สามคอฮอร์ต สามแกนเวลา", welcomeBody: "ภาพรวมติดตามเส้นทางเดียวกันในสามช่วง แต่ละช่วงระบุวันที่คอฮอร์ตที่ใช้นับ", scopeTitle: "เลือกขอบเขตธุรกิจ", scopeBody: "เลือกร้านที่ได้รับอนุญาตและช่วง 30 / 60 / 90 วัน ตัวเลขพอร์ตทางขวาไม่เปลี่ยนตามช่วงเวลา", reachoutTitle: "การเชิญ นับตามวันที่เชิญ", reachoutBody: "เทียบข้ามช่วงเวลาได้เฉพาะอัตราเท่านั้น คอฮอร์ตล่าสุดจะเป็นสีเทาระหว่างที่การตอบกลับยังทยอยเข้ามา", approvalTitle: "การอนุมัติ นับตามวันที่ยื่นคำขอ", approvalBody: "ทุกคำขอจะจบลงที่อนุมัติ ผู้ขายปฏิเสธ เกินกำหนดฝั่งเรา หรือยังดำเนินการอยู่", postApprovalTitle: "หลังอนุมัติ วัดเป็นจำนวนชิ้น", postApprovalBody: "จำนวนชิ้นไม่เคยขาดหาย จึงใช้แทน GMV และคอฮอร์ตใหม่จะมีส่วนคาดการณ์สีอ่อนต่อท้าย", exploreTitle: "สร้างคำค้นที่ตรงตามสัญญา", exploreBody: "ระบบจะปิดชุด Aggregate ที่ไม่รองรับก่อนสั่งรัน", resultsTitle: "เจาะลึกโดยไม่เปลี่ยนความหมาย", resultsBody: "กราฟและตารางใช้ข้อมูลเดียวกัน และคำนวณอัตราจากตัวตั้งกับตัวหารรวม"
  }, "การวิเคราะห์"),
} as const;
