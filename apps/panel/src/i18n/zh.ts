export default {
  ...
  permissions: {
    title: "文件权限",
    description: "DlxAI通过验证所有工具调用来强制执行文件访问权限。你的工作区文件夹始终可访问。在下方添加其他文件夹。",
    addPath: "添加路径",
    browsePath: "浏览...",
  ...
  settings: {
    ...
    autoLaunch: {
      title: "启动",
      toggle: "登录时自动启动 DlxAI",
      hint: "",
      failedToSave: "保存启动设置失败：",
    },
    telemetry: {
      title: "遥测与隐私",
      description: "通过分享匿名使用统计数据帮助我们改进DlxAI。我们重视您的隐私。",
      toggle: "发送匿名使用统计数据",
    ...
    dataDir: {
      title: "数据目录",
      label: "OpenClaw 状态目录",
      hint: "Agent 记忆、文档和技能的存储目录。修改后需重启 DlxAI 生效。",
      default: "默认",
      custom: "自定义",
      current: "当前",
      change: "修改",
      reset: "恢复默认",
      restartNotice: "请重启 DlxAI 以使更改生效。",
  ...
  tutorial: {
    ...
    settings: {
      ...
      welcomeBody: "在这里你可以自定义 DlxAI 的各项行为。每个部分控制应用的不同方面，让我们逐一了解每个选项。",
    ...
    apps: {
      ...
      invitationCodeBody: "你的面板访问令牌，在入驻时提供。用于将你的 DlxAI 实例与云服务进行身份验证。如果之前已保存过，后续编辑时此字段可以留空。",
    ...
    account: {
      ...
      identityTitle: "你的身份",
      identityBody: "你的头像（根据姓名首字母生成）、显示名称和邮箱地址。邮箱是你注册或登录 DlxAI 时使用的地址。",
      logoutTitle: "退出登录",
      ...
      memberSinceBody: "你的账户创建日期。帮助你了解使用 DlxAI 的时长。",
      ...
      modulesSectionBody: "模块是可选的功能扩展包，为 DlxAI 添加特定领域的专业能力。每个模块会解锁额外的工具、页面和集成，服务于特定的工作流程。在此处启用或禁用模块来定制你的工作空间。",
